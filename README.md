...
- **Link-more-characters on the apply form**: applicants see which of their characters recruiters will review (with a Main badge) and can link more alts inline via SeAT's add-character SSO, returning to the form afterward. The form urges registering every character/alt, and a returning member with characters HR already knows but hasn't re-authed gets a blinking warning listing the missing ones
