...
- **Former Members** (**director**): a frozen record of people who left. Captured on the day of departure rather than computed afterwards, because leavers commonly revoke their ESI key and their history stops being readable. Holds tenure, where they went, whether they left or were purged, tier and status as they stood, and contribution over that stint alone; notes, intel and history are linked rather than copied. Whether somebody is a former member is *derived*, so a rejoin needs no write: they reappear on the roster and go straight back to Members and Players, while their archived record stays as the history of the membership that ended. `hr-manager:backfill-member-archives` reconstructs records for people who left before this existed, badged as reconstructed
...
| `hr-manager:backfill-member-archives` | on-demand | **Opt-in**: reconstruct Former Member records for departures predating the archive, from SeAT's corporation histories. Everything written is badged *reconstructed* (partial by definition) and never overwrites a record captured at departure. Supports `--corporation`, `--since`, `--dry-run` |
...
| `member_archives` | frozen record of each completed membership (one row per stint), for the Former Members page |
