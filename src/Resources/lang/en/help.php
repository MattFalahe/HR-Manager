...
return [
    ...
    'feat_p_cap_6' => '<strong>Squad cleanup</strong>: the player profile lists the account\'s SeAT squad memberships, separated into <strong>manual / hidden</strong> (removable) and <strong>auto</strong> (managed by SeAT). A one-button <em>Remove from these squads</em> clears only the manual / hidden ones, mirroring SeAT\'s native kick, so with SeAT Connector installed the matching Discord roles drop immediately. Auto squads are shown for information only and left alone (SeAT would just re-add an eligible member from filters); they resolve once the player no longer matches, for example after leaving the corp. Director-only, recorded on the history timeline.',
    ...
    'discord_connector_applicants_step2' => '<strong>Or grant it yourself</strong>: in <em>SeAT &rarr; Access Management</em>, give registered characters (or whichever group your applicants fall into) the Connector view permission. A public or registered-user mapping is the simplest way to reach everyone who logs in.',
