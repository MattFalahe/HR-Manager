...
return [
    ...
    'feat_pt_tracking_body'    => 'Every application gets an unguessable 48-character base62 slug at submit time. The no-auth URL <code>/recruit/track/{token}</code> shows the applicant: their character portrait + name, the corp they applied to, current status badge, a clean status timeline (transitions + timestamps only, NO recruiter comments), the joined-corp outcome panel once decided, and their own submitted answers. Notes / handlers / eligibility internals are never shown. When the corp enables self-withdrawal (Settings → General), the applicant also gets a <strong>Withdraw application</strong> button while their application is still open (a CSRF-protected, token-authenticated POST). The post-submission confirmation page surfaces the URL in a copy-friendly input.',
