...
return [
    ...
    'feat_assessment'           => 'Applicant assessment',
    'feat_assessment_desc'      => 'Automated green / amber / red recruiter verdict from corp-hopping, NPC-parking, age, sec status, watchlist, zKill PvP + SP.',
    'feat_counterintel'         => 'Counter-intelligence',
    'feat_counterintel_desc'    => 'Blacklist / whitelist with account-wide screening on submit — auto-flags a spy\'s other characters, cross-corp / alliance scan, and a full character dossier.',
    'feat_alignment'            => 'Role &amp; title alignment',
    'feat_alignment_desc'       => 'Do a player\'s alts hold the same in-game corp titles + roles as their main? Drift between main and alts (and between SeAT and in-game) is flagged.',
    'feat_activity_log'         => 'Activity log',
    'feat_activity_log_desc'    => 'Opt-in director audit trail — who viewed and did what: page views, privilege grants, decisions, security actions, notifications. 180-day retention.',
    'feat_token_coverage'       => 'ESI token &amp; scope coverage',
    'feat_token_coverage_desc'  => 'Every member\'s ESI key graded OK / missing-scopes / lost / never-linked, with loss alerts and a corp coverage card — spot a dead or delinked key fast.',
    'feat_ecosystem'            => 'Suite integration',
    'feat_ecosystem_desc'       => 'Richer with Manager Core: wallet / mining / blueprint / structure / buyback / FC signals feed the profiles and Corp Health. Fully standalone without it.',
    ...
    'feat_rf_cap_6' => '<strong>Register-all-characters on the apply form</strong>: the applicant sees which characters recruiters will review and is told to link <em>every</em> character and alt (not just their main) via SeAT\'s add-character SSO. A <strong>returning member</strong> who has characters HR already knows (from the player-identity record) but has not re-authed gets a blinking red warning listing the missing ones, so they re-add everything before submitting. A warning, never a hard gate.',
    ...
    'feat_aw_cap_6' => '<strong>Undo an accidental status change</strong> (director-only): revert the last status change on an application. The mistake is <em>voided</em> rather than deleted &mdash; it stays in the internal history struck-through with who reverted it and why, but is <strong>hidden from the applicant\'s tracking timeline</strong>, and the status is restored to what it was. So a misclicked <em>Rejected</em> never lingers on the applicant\'s page. Requires a written reason (audited), works on terminal states too (that is the main case), and undoes one step at a time. Reverting an <em>Accepted</em> warns first: it restores the status but does NOT roll back what the acceptance already triggered (Discord onboarding, access grants, notifications already sent).',
    ...
    'feat_mpv_members_body'    => 'Reads SeAT\'s authoritative <code>corporation_members</code> table (populated by a director-scoped <code>read_corporation_membership.v1</code> ESI token) so it shows EVERY character in the corp, registered or not. Names fall back through <code>character_infos</code> &rarr; <code>universe_names</code> cache &rarr; <code>"Character #N"</code> so even unregistered alts show real names when SeAT has resolved them. Each row links to a detail page with assessment data, wallet activity, in-game titles + roles, recent PvP, Discord identity, and notes. <strong>No director token?</strong> The roster is sparse — an opt-in <strong>EveWho back-fill</strong> (<em>Settings &rarr; Features</em>) fills the gap from EveWho\'s public corp list: name-only rows badged <em>EveWho</em>, de-duplicated against anyone SeAT already knew, seeded on first view and refreshed by a daily cron. A character SeAT can\'t resolve has no profile page, so its View button is hidden. EveWho is a third-party aggregator (can list departed members, miss recent joins), so the authoritative fix is still a director token.',
    'feat_mpv_players_heading' => 'Players — per-human rollup',
    'feat_mpv_players_body'    => 'One row per SeAT user account, aggregating every linked character. Tier resolved from the highest-tier registered alt; activity, wallet flags, and Corp Health verdicts roll up across the whole person. The alt grid badges each character <strong>active / idle / inactive (a dormant alt) / unknown</strong> beside its token badge — with an account roll-up (N active / idle / dormant) — so you can see which alts are actually used. The history timeline names the specific character each event is about, and director-tier actions (LOA, mark for purge, clear status) sit alongside. A separate amber panel flags <strong>possible alt links</strong> &mdash; a claim by a director that this player is (or has) somebody else\'s alt &mdash; shown whether or not anyone here is blacklisted, with its verdict and a marker when the character on the other end is itself blacklisted. That one informs rather than accuses; it stays unproven until the nightly reconciler settles it. An <strong>active blacklist entry on any character of the account</strong> raises a warning banner above everything else on the profile, with the severity, scope, reason and a link to the dossier — the one status that should stop a director before they act on the rest. Blacklist only, and active only: a cleared entry is history and appears on the timeline instead. The heavy per-alt panels are cached and can be <strong>pre-warmed in the background</strong> (opt-in, <em>Settings &rarr; Features</em>) so a many-alt profile opens fast. Use this for HR decisions about a person rather than one alt at a time.<br><br><strong>Player identity + merging.</strong> Each account also has an HR <em>player identity</em> — HR\'s own record of the human, which survives account-takeover reassignments. When one person has accidentally made <em>two</em> SeAT accounts you can <strong>merge</strong> their identities from the target\'s profile, which re-points every character mapping and makes HR treat them as one human (alt-link claims, for instance, then resolve as confirmed). <strong>It changes nothing in SeAT:</strong> SeAT still holds two accounts, so both keep a row on the Players list — the merged one is badged and its profile banners where its characters went, who merged it and when. To consolidate for real, a server admin moves the characters onto one account in <em>SeAT &rarr; Users &rarr; edit user &rarr; Transfer</em>; HR follows on its own once they share an account.',
    ...
    'feat_mpv_token_heading'  => 'Token + scope compliance',
    'feat_mpv_token_body'     => 'Each member row carries a token-health badge: <strong>Token OK</strong>, <strong>Missing scopes</strong>, <strong>Token lost</strong>, or <strong>Never linked</strong>. Pick a SeAT SSO scope profile as your corp requirement under <em>Settings &rarr; SSO &amp; Scopes &rarr; Member token requirement</em> and HR measures every member token against it (the Missing scopes badge names the exact scopes a token lacks). The corp-wide rollup, with a coverage bar and the lost / never-linked lists, lives on the <em>Corp Health &rarr; Overview</em> Token &amp; scope coverage card, and an opt-in weekly digest (<code>hr-manager:token-coverage-digest</code>) can post the summary to a webhook. Leave the profile as None to check only that a token exists.',
    ...
    'feat_ch_tiers_body'    => 'Five tiers map players to responsibility levels: <strong>L-1 Applicant</strong> (excluded from classification), <strong>L0 Member</strong>, <strong>L1 Junior Officer</strong>, <strong>L2 Senior Officer</strong>, <strong>L3 Director</strong>. Each tier has an activity threshold in days; cross it and the classifier downgrades the player. Defaults are 90/30/14/14 days respectively. Configure per-corp overrides in <strong>Settings &rarr; Tiers</strong>, or — <strong>only with the SeAT Connector framework installed</strong> — map a Discord role to a tier so new senior officers auto-pick up the right threshold. Connector is optional: without it, set tiers manually or skip them and the classifier uses the L0 default.',
    'feat_ch_classifier_heading' => 'The classifier',
    'feat_ch_classifier_body'    => 'A nightly cron (<code>hr-manager:classify-players</code>) walks every active player and buckets them: <strong>Active</strong> (within threshold), <strong>At Risk</strong> (inside threshold but trending down), <strong>Inactive</strong> (past threshold), <strong>Dead Weight</strong> (significantly past threshold with no positive signals). Inactive directors get a separate critical-tier alert because corp survival depends on responsive leadership. Click "Run Now" on the Corp Health page to re-run on demand. Classification is <strong>per human</strong> (one verdict per SeAT account), and the whole console — the at-risk table, Director roster, Economy cards and access map — folds a person\'s characters into one row headed by their main, expandable to the per-character detail. <strong>Any active character makes the person active</strong>: activity is the most recent signal across every character on the account, so the days-inactive figure is the <em>lowest</em> of their alts, not the highest. Someone flying their main daily is never classified inactive (or alerted on) because a spare alt has sat unused for months. The same rule governs the <strong>Inactive directors</strong> panel — an active account is not an absent director, however dusty one of its director characters is, and each row reports that person\'s most recently flown director character. Director characters skipped that way are counted in a note above the table so a genuinely dormant one stays visible.',
    'feat_ch_signals_heading' => 'Wallet signals (CWM round-3)',
    'feat_ch_signals_body'    => 'When Corp Wallet Manager is installed, the classifier folds in wallet flags: <code>STL</code> stalled, <code>NEG</code> net negative, <code>TAX</code> tax compliance &lt;50%, <code>VTX</code> tax compliance &lt;30%, <code>SWD</code> silent wallet director (a director still logging in but with no corp wallet activity attributed to them — its own notification category, off by default, and suppressed entirely when CWM reports no attributed data for the corp, since "no data" cannot be told apart from "did nothing"), <code>LYL</code> loyalty hold (long tenure softens classification), <code>ALT</code> active but carrying dormant alts. Signals are weighed on the <strong>account, not the alt</strong> — <code>STL</code> / <code>NEG</code> fire only when the whole account\'s freshest contribution is stale or its summed net is negative, so a dormant cyno / mining alt can\'t drag an active, contributing main down to At Risk; a real tax-compliance collapse still nudges immediately. Plus the Wallet Activity panel on member profile shows lifetime contribution, contribution percentile in corp, top contribution categories, recent entries, monthly trend, and tax compliance bar.',
    ...
    'feat_alignment_section'      => 'Role & Title Alignment (SeAT ↔ in-game, main ↔ alts)',
    'feat_alignment_section_desc' => 'An optional check: do <strong>all of a player\'s characters carry the same in-game corp titles + roles as their main</strong>? It compares each registered alt against the account\'s main and surfaces the drift — the exact thing that lets a forgotten alt keep (or lack) access nobody intended. Off by default; enable it in <strong>Settings &rarr; Features</strong>. Because it needs to know which characters belong to one human, it only covers <strong>registered</strong> characters (an unlinked member has no account to compare against).',
    'feat_alignment_missing'      => '<strong>Missing</strong> — the main holds a title or role an alt lacks (e.g. an alt never got the <em>Member</em> title, so a Connector role or a doctrine grant silently skips it).',
    'feat_alignment_extra'        => '<strong>Extra</strong> — an alt holds a title or role the main does not (e.g. an alt still carrying a Director or Personnel Manager role after a reshuffle — an access-creep / awox risk worth a look).',
    'feat_alignment_where'        => 'Enable in <strong>Settings &rarr; Features</strong>. When on: a <strong>Corp Health &rarr; Alignment</strong> tab (corp-wide summary, misaligned accounts first, lazy-loaded) and an <strong>Alignment panel on the player profile</strong>.',

    // ESI token & scope coverage
    'feat_token_section'      => 'ESI Token & Scope Coverage (active-key tracking)',
    'feat_token_section_desc' => 'HR grades <strong>every member\'s ESI key</strong> so you can see at a glance whose data is actually flowing. Pick a required scope profile (<strong>Settings &rarr; SSO &amp; Scopes &rarr; Member token requirement</strong>) and each member is classified <strong>Token OK / Missing scopes / Token lost / Never linked</strong> — a badge on the Members roster (the insufficient ones name the exact scopes they lack), a <strong>Token &amp; scope coverage</strong> card on Corp Health (counts, coverage bar, lost-this-week, drill-down lists), and an opt-in <strong>weekly digest</strong> to a webhook. Leave the profile as <em>None</em> to only check a token exists at all. It is a pure read of SeAT\'s <code>refresh_tokens</code> against the global scope profiles — no ESI calls, no changes to SeAT.',
    'feat_token_loss_heading' => 'Token-loss Watchdog (security)',
    'feat_token_loss_body'    => 'A member who revokes their SeAT refresh token (delinked, or rejected by CCP after a password change or app de-authorisation, which soft-deletes it either way) severs HR\'s visibility into them while they may still hold corp access. A 10-minute cron (<code>hr-manager:detect-token-loss</code>) turns that into a closed-loop security response: it <strong>always</strong> fires the <strong>SeAT Token Revoked</strong> webhook and records a critical history event, and (opt-in) auto-schedules a security purge that <strong>cancels itself</strong> if the token comes back. Everything below is off by default; switch on the parts you want under <strong>Settings &rarr; Features &rarr; Security Policy</strong>.',
    'feat_tw_cap_1' => '<strong>Choose the trigger</strong>: the account\'s <em>main</em> character losing its token, <em>any</em> character, or a share of the account\'s in-corp characters (25 / 50 / 75 / 100%). A single-character account always triggers, whatever the mode, so a one-character alt can\'t slip through a percentage gate. The default (<em>any</em>) matches the previous behaviour.',
    'feat_tw_cap_2' => '<strong>Director escalation</strong>: a character holding the in-game <strong>Director</strong> role losing its token always triggers (it can\'t hide under a percentage) and fires a distinct dark-red <em>SECURITY BREACH: Director token revoked</em> alert. Full corp access with zero visibility is the worst case.',
    'feat_tw_cap_3' => '<strong>Auto-cancel on restore</strong>: the safety net that makes it safe to leave on. Every run, any auto-purge whose account has re-linked enough to no longer trip the policy is cleared back to active with a green <em>token restored</em> alert. A director\'s manual purge is never auto-cancelled.',
    'feat_tw_cap_4' => '<strong>HR Watchdog notes</strong>: the arc (revoked, auto-scheduled, restored) is narrated as shield-badged <em>HR Watchdog</em> notes on the player\'s notes panel, so the story reads on the profile, not only in the history log.',
    'feat_tw_cap_5' => '<strong>Access reduction</strong> (opt-in): after a grace window from the token-loss event (or immediately, for a director), drop the member from their <strong>manual / hidden</strong> squads so their Connector Discord roles cascade off before the kick. Auto squads and your never-touch exclusions are left alone. HR only <em>removes</em>: a dropped squad is <strong>not</strong> re-added if the token returns, so a director re-adds by hand once cleared (which is why the grace window exists).',
    'feat_tw_cap_6' => '<strong>Accountable override</strong>: cancelling an auto-purge by hand is director-only and requires a written reason, recorded as an override note on the player and in the history. A director countermanding the Watchdog is always on the record.',
    'feat_token_where'        => 'Requirement in <strong>Settings &rarr; SSO &amp; Scopes</strong>; badges on the <strong>Members</strong> roster; coverage card on <strong>Corp Health</strong>; the <strong>SeAT Token Revoked</strong> category + weekly <strong>Token Coverage Digest</strong> in <strong>Settings &rarr; Notifications</strong>. The Token-loss Watchdog\'s auto-response toggles are under <strong>Settings &rarr; Features &rarr; Security Policy</strong>.',
    ...
    'feat_p_cap_7' => '<strong>Closes itself when the player actually leaves.</strong> HR watches for the departure; once confirmed, the purge is closed automatically — it drops off the purge board and the role-strip banner disappears from the profile — without waiting for anyone to tick it off by hand. It also <strong>writes the whole arc to the player</strong>: who scheduled the purge and when, the date it was due against the date they actually went (flagged on time, N days early, or N days late), which corp they went to, whether it came from the security auto-purge, the reason, and whatever was written in the purge notes. That lands as a history event and as a plain-English <strong>HR Purge log</strong> note on the profile, so "who kicked this person, and why?" still has an answer months later.',
    'feat_p_cap_6' => '<strong>Squad cleanup</strong>: the player profile and the purge board list the account\'s SeAT squad memberships, split three ways: <strong>removable</strong> (manual / hidden), <strong>kept</strong> (on your never-touch exclusions list), and <strong>auto</strong> (managed by SeAT). A one-button <em>Remove from these squads</em> clears only the removable ones, mirroring SeAT\'s native kick, so with SeAT Connector installed the matching Discord roles drop immediately. <strong>Opt-in auto cleanup</strong> (Settings &rarr; Squad cleanup, off by default) can clear a purged member\'s removable squads automatically: immediately once they are detected as having left the corp, otherwise at a configurable T-24h / T-12h before the kick date. A <strong>never-touch exclusions list</strong> protects keep-in-touch squads such as Former Member or Alliance access. Auto squads are never touched (SeAT would re-add an eligible member from filters) and resolve once the player no longer matches. Director-only, recorded on the history timeline. The <strong>Token-loss Watchdog</strong> can trigger this same removal earlier on its own opt-in schedule (see its section above); both paths share one fire-once marker, so a member is never double-dropped.',
    ...
    'feat_w_blacklist_body'    => 'Known troublemakers, spies, scammers, bad-faith applicants. When an applicant matches a blacklist entry, a <strong>red gradient banner</strong> renders at the top of the application detail page with the reason, severity (low / medium / high), and added-by info. A <strong>HIGH</strong>-severity entry also <strong>guards acceptance</strong>: the applicant cannot be moved to Accepted until a director clears the entry or overrides with a written justification (folded into the timeline + sent to your security channel). The application still flows in either way, so you keep gathering intel; the guard toggles in Settings &rarr; General. Add by character name or ID, attach a free-text reason, optional auto-expiry date. <strong>Screening runs automatically on submit</strong>: every new application is checked against the blacklist + intel across the WHOLE applying account (main + registered alts), so a blacklisted alt added under a clean main is caught. A hit fires the <strong>Flagged applicant</strong> notification, records it on the history timeline, and &mdash; with the opt-in auto-flag setting (Settings &rarr; General, off by default) &mdash; also blacklists the account\'s other characters so a later corp/alliance scan catches every alt. Those auto-created entries are attributed to <strong>HR Counter-Intelligence</strong> (with a bot icon), not a blank User #0.',
    'feat_w_dossier_heading' => 'Character dossier',
    'feat_w_dossier_body'    => 'Click any character name on the Watchlist page (or the <em>open dossier</em> link on a clipped reason) to open <code>/watchlist/character/{id}</code> &mdash; one page that gathers everything HR holds on that character: the full, untruncated watchlist reasons (active AND cleared), every intel note you are allowed to see, and &mdash; for directors &mdash; the history timeline. Each section honours its own visibility: watchlist entries are scoped to what you can access, the intel section shows only notes visible to you (so a recruiter without intel access simply sees nothing hidden), and history is director-only. Also linked from the Intel page. The <strong>watchlist lifecycle is preserved on that timeline</strong> &mdash; when a character was added, when an application hit, and when it was cleared (each with who did it and the reason) &mdash; so the story survives even after the entry drops off the active list. Re-listing a character who was previously cleared starts a fresh episode and is labelled as a re-listing on the timeline, naming the date the last one ended.<br><br>The dossier resolves the <strong>whole human</strong>, not one character &mdash; following the SeAT account <em>and</em> HR\'s player identity &mdash; so every entry and note across their alts is on one page. An <strong>Alt links</strong> section lists any claim that this character is (or has) an alt, with its current verdict, and a <strong>possible missing alts</strong> panel names characters on that account which nobody has listed &mdash; the alts a director did not know about when the original block was filed. Both appear only when there is something to show. The same missing-alts panel is on the player profile.',
    'feat_w_clear_account_heading' => 'Clearing an account\'s alts together',
    'feat_w_clear_account_body'    => 'When you clear a character whose account has more than one active watchlist entry (a spy whose alts are all listed), HR asks whether to clear <strong>only that character</strong> or <strong>all of them on the account</strong> before prompting for the required reason. Clearing all wipes the whole footprint in one action; each character still gets its own cleared entry + history record, and any entries outside your scope are left untouched.',
    ...
    'feat_w_resolution_heading' => 'Name ↔ ID resolution',
    ...
    'feat_pt_tracking_body'    => 'Every application gets an unguessable 48-character base62 slug at submit time. The no-auth URL <code>/recruit/track/{token}</code> shows the applicant: their character portrait + name, the corp they applied to, current status badge, a clean status timeline (transitions + timestamps only, NO recruiter comments, and any transition a director has reverted as a mistake is hidden here), the joined-corp outcome panel once decided, and their own submitted answers. When <strong>SeAT Connector</strong> is installed but the applicant has not linked their Discord yet, the page also surfaces a <strong>Link your Discord</strong> step so they see that outstanding action while the application is live rather than discovering it at accept time (hidden once linked, and never shown on a rejected / withdrawn application). Notes / handlers / eligibility internals are never shown. When the corp enables self-withdrawal (Settings → General), the applicant also gets a <strong>Withdraw application</strong> button while their application is still open (a CSRF-protected, token-authenticated POST). The corp can optionally <strong>ask for a withdrawal reason</strong> &mdash; and make it required &mdash; which is saved to the director-side application timeline for context and is never shown on this public page. The post-submission confirmation page surfaces the URL in a copy-friendly input.',
    'feat_pt_join_heading' => 'Corp-join detection',
    'feat_pt_join_body'    => 'A background cron (<code>hr-manager:detect-corp-joins</code>, every 30 minutes) confirms accepted applicants actually joined. It reads the join date from the most recent corp-history record for the target corp (so a member who was already in the corp before applying is dated correctly), and falls back to SeAT\'s fresher <code>character_affiliations</code> when the corp-history endpoint has not caught up yet. <code>joined_corp_at</code> is flipped (and corrected if an approximate date was stamped first), and an <code>hr.application.joined_corp</code> event is published to EventBus. Bounded to a 90-day window. The Corp Health page surfaces an "accepted but never joined" backlog count.',
    ...
    'feat_n_cap_1' => '<strong>Per-webhook category toggles</strong>, grouped into labelled boxes (Applications, Security, Corp health / retention &amp; purge, Corp membership, Wallet &amp; tax): application submitted / accepted / rejected / status change, classifier flags (at_risk / inactive / dead_weight), inactive director critical alert, purge reminders (T-7 / T-3 / T-48 / T-0), player status (LOA / marked for purge / <strong>marked for purge, notify the player</strong> / cleared, fired the moment a director sets it), <strong>SeAT token revoked</strong> (security), <strong>token coverage digest</strong> (opt-in weekly), wallet signals (stalled / compliance_dropped / milestone). Each toggle independent. The <em>notify the player</em> purge category @-mentions the flagged player directly instead of the role, for a member-facing channel (needs SeAT Connector to resolve the Discord identity).',
    ...
    'feat_n_cap_6' => '<strong>Token-loss security alert</strong>: the <code>hr-manager:detect-token-loss</code> cron (every 10 minutes) watches SeAT for tracked members whose refresh token has gone, fires the <em>SeAT Token Revoked</em> category to your security channel, and records a critical history event. Because SeAT removes a token both when a member delinks it AND when CCP permanently rejects it (password change, app de-authorization), this catches passive token death, not just deliberate delinks. A character holding the <strong>Director</strong> role gets a louder <em>SECURITY BREACH</em> alert. The alert works on its own; the opt-in <strong>Token-loss Watchdog</strong> (<em>Settings &rarr; Features &rarr; Security Policy</em>) adds the closed-loop auto-purge (with auto-cancel on restore) and optional squad-drop detailed in the Token &amp; scope feature section. The Members page already shows who currently holds a working token via its registered / unregistered count.',
    ...
    'feat_i_buyback_heading' => 'Buyback Manager',
    'feat_i_buyback_body'    => 'HR subscribes to <code>buyback.offer.published</code> (engagement) and <code>buyback.contract.completed</code> (realized contribution) on the EventBus, keyed on the issuer (the contributing member). Each corp\'s buyback is valued through a <strong>per-corp policy</strong> (Settings → Buyback Contribution): counted? / tier (<strong>Direct corp / Community / Personal</strong>) / weight / which corp it is credited to — so an alt or holding corp\'s buyback can count as main-corp support. The default is read from Buyback Manager\'s own target model (own-corp → community, target-corp → direct credited to that corp, designated-character → personal). <strong>Contributions are valued per era</strong>: changing a policy freezes the contributions made so far at their old weights and applies the new policy to new ones (both count), unless you tick <em>Also re-value existing</em> to re-value all history. Surfaces as a buyback panel on the player profile, a buyback card on Corp Health Economy (ISK credited + top contributors), and a positive <code>buyback_hold</code> classifier signal that holds a borderline at-risk member at active. Run <code>hr-manager:backfill-buyback</code> once to seed history.',
    ...
    'access_intro' => 'When a recruiter joins an application\'s handler list, HR Manager can attach a temporary SeAT role granting them view access to the applicant\'s character data &mdash; the pages you choose, from wallet and mail through to any SeAT character page &mdash; in SeAT\'s own UI. The role is scoped strictly to the applicant\'s whole account (main + alts, resolved live from the shared SeAT account); your existing Director and other roles are not touched. Auto-revoked on leave / application close / expiry.',
    ...
    'access_lifecycle_use'   => '<strong>While active</strong>: a panel on the application detail page shows the handler\'s active grant with <strong>one deep-link button per granted permission</strong> (the button set tracks the permission set you configured) into SeAT\'s native character pages, plus a <strong>Re-sync characters</strong> button that refreshes the grant when the applicant registers a new alt or you change the permission set. Each link opens in a new tab; SeAT enforces the permission transparently.',
    ...
    'access_perm_journal'      => 'Wallet journal — every transaction (donations, ratting bounties, market fees, etc.)',
    'access_perm_transactions' => 'Market transactions — buy/sell history with item details and venue',
    ...
    'access_perm_industry'     => 'Industry jobs — current and completed, with blueprint and runs detail',
    ...
    'access_recruiter_ux_body'  => 'A pinned panel at the top of the application detail page titled <strong>Your temporary SeAT access</strong>, with an "expires in" badge in the header. Each of the applicant\'s characters &mdash; main and alts &mdash; gets its own row with one deep-link button per granted permission (so the buttons match the set you configured). A <strong>Re-sync characters</strong> link refreshes the panel if the applicant registered a new alt or you changed the permission set after the grant was created. Clicking a link opens SeAT\'s native page in a new tab &mdash; the recruiter does their due diligence in SeAT\'s own UI, not a copy in HR Manager. The panel disappears when the grant is revoked.',

    // Activity Log (director-only, opt-in accountability trail)
    'audit_help_title'  => 'Activity Log (director-only, opt-in)',
    'audit_help_intro'  => 'A sidebar page &mdash; visible only to directors &mdash; recording who did, and viewed, what inside HR Manager, so a corp keeps an accountable trail. It is <strong>off by default</strong>; enable it in <strong>Settings &rarr; Features</strong>. Nothing is recorded before you turn it on.',
    'audit_help_views'        => '<strong>Views</strong> &mdash; every internal page a recruiter or director opens is logged with actor, time, and target: which application, player, member, dossier or intel record they looked at, and which Corp Health corp + tab. One middleware over the whole plugin captures this, so there are no per-page hooks to miss.',
    'audit_help_actions'      => '<strong>Every action</strong> &mdash; the same middleware records <em>every</em> successful add / edit / delete: applications, players, members, notes, watchlist, intel, templates, recruitment pages, webhooks, tiers and settings. Only failed requests (validation errors, denied) are skipped. Routine changes land under a <strong>Management</strong> category; the important ones below add richer detail.',
    'audit_help_privilege'    => '<strong>Privilege + security</strong> &mdash; recruiter data-access and Connector-link grants + revokes (subject, permissions, expiry, and <em>why</em> on revoke), plus blacklist / whitelist add + clear and intel add + delete &mdash; each naming the character and reason, with deletes snapshotted before the record is gone.',
    'audit_help_decision'     => '<strong>Decisions</strong> &mdash; every application status change (the whole recruitment process), snapshotting the applicant name so the row survives a later purge, every player-lifecycle action (LOA / mark-for-purge / clear / merge / reassign), a distinct entry whenever a HIGH-severity blacklist match is consciously overridden to accept, with the written justification, and every <strong>reverted</strong> status change (which transition was voided, what it was restored to, and the director\'s written reason).',
    'audit_help_notification' => '<strong>Notifications</strong> &mdash; the action-driven notifications that go out (status change, blacklist override), attributed to the acting user. Filter by category / action / actor / date / text, export the current filter to CSV, and jump back to any referenced record.',
    'audit_help_retention_title' => 'Retention',
    'audit_help_retention_body'  => 'Entries are append-only and pruned after <strong>180 days</strong> by a daily job (<code>hr-manager:prune-audit-log</code>), which runs even when the feature is off so a disabled log still drains. IP addresses are stored only as a daily-rotating salted hash &mdash; traceable within a day, never kept raw.',
    ...
    'corp_health_intro' => 'The Corp Health view buckets every player in the corp into one of four categories based on their tier threshold + activity signals. Each bucket gets its own count and drill-down. Buckets are per <strong>person</strong>: a player counts as active if any one of their characters is, and their days-inactive figure is the lowest across their alts &mdash; an unused alt never pulls an active human into a worse bucket. The <strong>Composition</strong> tab additionally carries an <strong>active vs dormant characters</strong> bar, which is deliberately per <strong>character</strong>: it measures how much of the character base is sitting idle (dormant-alt load), not how many people are around, so the two figures answer different questions and will not match.',
    ...
    'flag_swd_desc'        => 'Silent wallet director: a director who is still logging in but has no corp wallet activity attributed to them. Its own notification category (off by default), separate from the Inactive Director alert, and never reports a days-inactive figure &mdash; they are not inactive, just absent from corp finances.',
    ...
    'inactive_director_body'  => 'A separate critical-severity alert fires when an in-game director crosses their L3 threshold. Directors with the keys to your wallet should not be inactive without anyone noticing. A <strong>second, distinct</strong> alert covers the opposite case: a director who <em>is</em> still logging in but has no corp wallet activity attributed to them (needs Corp Wallet Manager). That one has its own category, is off by default, and never reports a days-inactive figure &mdash; the director is not inactive, just absent from corp finances.',
    ...
    // NOTIFICATIONS
    // ============================================================
    // ── New-member onboarding (own Help section, added 1.0.1) ───────────────
    'onboarding_nav'   => 'Onboarding',
    'onboarding_title' => 'New-member onboarding welcome',
    'onboarding_intro' => 'The step straight after someone is accepted and actually joins. HR posts a <strong>member-facing</strong> greeting to a Discord channel that <strong>@-mentions the new member</strong> and walks them through your first steps — where home is, which intel channels to join, where the trade hub is, how to load your bookmarks, and who to ask for help. Every other notification HR sends is written for staff; this one is written for the newcomer. <strong>Opt-in and off by default</strong>, per corporation.',

    'onboarding_who_title' => 'Who gets one',
    'onboarding_who_body'  => 'Genuinely new people only, <strong>once per account</strong>. The joining character can be any of theirs — main or alt — but an existing member\'s alt joining does not trigger it, because that is not a new person. Someone with no SeAT account at all is skipped too: there is no linked identity to @-mention. A leave-and-rejoin inside 30 days will not re-welcome anybody.',

    'onboarding_when_title' => 'When it actually fires',
    'onboarding_when_body'  => 'Not on a fixed timer. HR waits your configured delay (default 30 minutes) and then holds the welcome until the newcomer is genuinely <em>reachable</em>: confirmed in the corp, a webhook subscribing to the category, and — with SeAT Connector installed — a linked Discord identity, so they can actually see the channel the ping lands in. Someone who links Discord a day late still gets welcomed properly. If they leave during the wait the welcome is dropped; if it never becomes deliverable it is abandoned after 7 days. Delivery runs every 5 minutes (<code>hr-manager:send-onboarding-welcomes</code>).',

    'onboarding_setup_title' => 'Setting it up',
    'onboarding_setup_1' => '<strong>Settings &rarr; Onboarding</strong> — turn on the master switch. On its own this greets nobody.',
    'onboarding_setup_2' => 'Pick a corporation and turn on <strong>Run onboarding welcomes for this corporation</strong>. Every corp is off until you enable it individually, so this can never start welcoming people somewhere you did not intend.',
    'onboarding_setup_3' => 'Edit that corp\'s message and tick the Discord roles your <strong>new-member care team</strong> holds.',
    'onboarding_setup_4' => '<strong>Settings &rarr; Webhooks</strong> — turn on the <strong>New-member onboarding welcome</strong> category on the webhook pointing at your welcome channel. Each corp on the Onboarding tab tells you whether a webhook currently covers it.',

    'onboarding_template_title' => 'Writing the message',
    'onboarding_template_body'  => 'Per corporation, in Discord markdown, with a friendly built-in default you can edit or replace. Three variables are filled in for you: <code>{member}</code> @-mentions the newcomer (or names them in plain text when Connector is absent), <code>{corp}</code> is the corporation name, and <code>{care}</code> expands to the @-mentions of the care-team roles you ticked. Leave a corp\'s message at the default and it simply uses the shared default.',

    'onboarding_length_note' => '<strong>Discord caps a message at 2,000 characters</strong> and anything past that is cut off mid-sentence. Remember the variables expand — a mention is roughly 21 characters, and <code>{care}</code> multiplies by the number of roles — so leave headroom. If you are adapting a long in-game mail, expect to cut it down by about half.',

    'onboarding_troubleshoot' => '<strong>Nothing arriving?</strong> Run <code>hr-manager:send-onboarding-welcomes</code> and read the counts. <em>Still waiting</em> is normal for a member who has not linked Discord yet; it also means no webhook subscribes, which is the usual cause. <em>Given up on</em> means welcomes waited 7 days without their conditions being met — check the webhook category first. A welcome is never silently discarded: it is only consumed once it is sent, the member has left, or it expires.',
    // ── Handler-note pings (1.0.1) + pointer to the Onboarding section ──────
    'handler_note_title' => 'Handler note on an application',
    'handler_note_body'  => 'Adding a <strong>public</strong> note to an application @-mentions the <strong>other handlers on that application</strong> — and deliberately <strong>suppresses the webhook\'s role mention</strong>, because the channel is not the audience, the co-handlers are. The author is never pinged, which makes it self-silencing: one handler talking to themselves notifies nobody, and it starts working the moment a second recruiter joins the handler list. <strong>Private notes never notify</strong> — a private note is a recruiter thinking aloud, and pinging somebody about text they may not be allowed to read is worse than silence. Off by default on every webhook; without SeAT Connector it still posts, naming the handlers in plain text.',

    'onboarding_notif_pointer' => 'The one <strong>member-facing</strong> category — a greeting posted to a Discord channel for the newcomer, not for staff. It has its own section: see <strong>Onboarding</strong> in the menu on the left for who gets one, when it actually fires, how to set it up per corporation, and how to write the message.',

    // ── Whole-account listing + suspected alt claims (1.0.1) ────────────────
    'feat_w_altclaims_heading' => 'Listing a whole account, and alt claims',
    'feat_w_altclaims_body'    => 'A spy is a person, not a character, so both forms take a <strong>main plus their possible alts</strong> and file an entry for each with the same reason, severity, scope and alerts.<br><br>Where HR can <em>prove</em> the link — a shared SeAT account, or an HR player identity — an <strong>also add this character\'s known alts</strong> option expands the account for you. It only ever uses proven links, never name similarity or a shared corp, so it cannot span different people, and a character reassigned away in an account takeover is excluded.<br><br>For an outsider there is nothing to expand: <strong>EVE\'s API exposes no account concept at all</strong>, so nothing can prove two characters are one person and the names you supply are recorded as <strong>claims</strong> rather than facts. A nightly pass (<code>hr-manager:reconcile-suspected-alts</code>) then settles them once both characters appear in SeAT — the same account <strong>confirms</strong> the link, two different accounts <strong>refute</strong> it. Refutation is the point: without it a wrong guess leaves somebody blocked forever with nothing to surface the error. A verdict can also change later if the underlying accounts are merged or a character is reassigned.<br><br>Claims live in their own table and are <strong>never</strong> written into HR\'s authoritative player-identity mappings, so an unproven suspicion cannot leak into surfaces that assume certainty. State badges show on the watchlist rows, the verdict is written to the main\'s profile, and a <strong>possible missing alts</strong> panel names characters on the account that nobody has listed.',
    'notifications_title' => 'Notification Categories',
    'notifications_intro' => 'HR Manager fires notifications through Discord / Slack webhooks, controlled on two independent axes. The <strong>Notifications console</strong> (Settings &rarr; Notifications) lists every type in one place, grouped by area, each with a <strong>master on/off</strong> &mdash; off means the type is never sent on ANY channel. The <strong>Webhooks</strong> tab then decides WHICH webhook fires each type. <strong>Watchlist Detection</strong> and <strong>Intel Match</strong> have their own master switches in the console (they used to ride other categories and could not be silenced on their own), and the recurring wallet-alert cadence &mdash; how often HR re-pings while a condition persists &mdash; lives there beside its types. Every type is on by default.',
    ...
    'toggle_handler_note' => 'Handler note on an application — pings only the OTHER handlers on that application, never the channel role (public notes only; off by default)',
    'toggle_silent_wallet_director' => 'Director with no wallet activity — still logging in, but nothing attributed to them (needs Corp Wallet Manager; off by default)',
    ...
    'toggle_loa_marked'        => 'Player marked on LOA',
    'toggle_marked_for_purge'  => 'Player marked for purge',
    'toggle_status_cleared'    => 'Player status cleared',
    'toggle_purge_personal'    => 'Player marked for purge — notify the player (@-mentions the flagged member instead of the role, for a member-facing channel; needs SeAT Connector)',
    'toggle_token_revoked'     => 'SeAT token revoked (security)',
    'toggle_token_coverage'    => 'Weekly token + scope coverage digest',
    'toggle_member_joined'     => 'Member joined (alt of a member, or applied; names the main)',
    'toggle_member_left'       => 'Member left the corp',
    'toggle_join_no_application' => 'Joined without a valid application (security flag)',
    'toggle_member_unregistered' => 'Joined while unregistered — no SeAT account (security flag, auto-clears on registration)',
    'toggle_onboarding_welcome' => 'New-member onboarding welcome (opt-in, member-facing greeting held until the newcomer can actually see the channel)',
    'toggle_flagged_applicant' => 'Flagged applicant — a new application registered a blacklisted / intel-flagged character (security)',
    'toggle_watchlist_detection' => 'Watchlist detection — a blacklisted character found inside a watched corp / alliance (its own switch)',
    'toggle_intel_match'       => 'Intel match — a character with an intel note on file is a current member of a watched corp (its own switch)',
    ...
    'permissions_intro' => 'HR Manager uses a 5-tier permission model. Higher tiers inherit all lower-tier access, except <strong>Personnel Manager</strong> — a targeted add-on you grant <em>alongside</em> Recruiter to hand out the accept/reject decision without the full Director role. Role permissions only gate page access — data is further scoped by corporation_id at the controller level.',

    'permission_level'   => 'Permission Level',
    'permission_access'  => 'Access',
    'perm_view_desc'      => 'Help page only.',
    'perm_recruiter_desc' => 'View and process applications (move through interview stages), manage own notes, character checks, watchlist + intel. The final accept/reject needs Personnel Manager or Director. (Member + player profiles are director-only.)',
    'perm_personnel_desc' => 'Everything a Recruiter can do, <strong>plus accept / reject applications</strong> — the hire/decline decision. Mirrors EVE\'s in-game <em>Personnel Manager</em> role: no member/player profiles, no kicks, no sensitive data. Grant it <strong>alongside</strong> Recruiter to a senior recruiter who should close applications but doesn\'t need Director.',
    'perm_director_desc'  => 'All recruiter access + member + player profiles (sensitive data), Corp Health, manage templates, assign recruiters, accept/reject applications, schedule purges, Activity Log.',
    'perm_admin_desc'     => 'Full control: settings, webhooks, delete applications and templates, diagnostic page.',

    'coherence_recommendation' => 'Recommended: grant <strong>recruiter</strong> to your recruitment team, add <strong>Personnel Manager</strong> to the senior recruiters who close applications, and <strong>director</strong> to corp leadership. Reserve <strong>admin</strong> for SeAT site admins who own the install. HR flags a mismatch on Corp Health when someone can accept applications but lacks the in-game role to complete the invite (or vice-versa).',

    // ============================================================
    // COMMANDS
    // ============================================================
    'commands_title' => 'Artisan Commands',
    'commands_intro' => 'HR Manager ships a set of artisan commands. Most run on cron via SeAT\'s scheduler and need no hand-invocation; a couple are one-time setup commands you run by hand (see init and backfill-buyback below).',

    'cmd_init'              => 'Guided first-run setup: a readiness check (prerequisites + the setup steps to complete) then a sequenced load that populates every dashboard at once instead of waiting for the nightly crons. It deliberately skips the notification passes so a first run is quiet. Run by hand; <code>--check</code> runs the readiness pass only, <code>--force</code> skips the prompt.',
    ...
    'cmd_detect_membership' => 'Diff the corp roster for joins and leaves and notify (classified: alt of a member / applied / no application). Forward-only — the first run per corp seeds the roster silently, so existing members are never announced. Runs every 30 minutes.',
    ...
    'cmd_reconcile_suspected_alts' => 'Settles "possible alt of X" claims made on the blacklist / whitelist / intel forms. HR cannot prove an alt link for someone who was never in SeAT (EVE\'s API exposes no account concept), so those claims are recorded as suspicions. Once BOTH characters appear in SeAT this pass decides: same account confirms the link, different accounts <strong>refute</strong> it and flag the entry that was filed on that assumption. Claims stay open until there is enough data, so most runs decide nothing.',
    'cmd_sync_external_rosters' =>'Refreshes the EveWho roster back-fill: the full multi-page pull a page-load seed can\'t afford, for corps already using it. Fills the gap left when SeAT has no director token for a corp, so the Members page isn\'t near-empty. Opt-in (Settings &rarr; Features &rarr; EveWho roster back-fill); no-ops when off. EveWho is a third-party aggregator, so it can list departed members and miss recent joins — a director token is still the authoritative fix.',
    'cmd_warm_player_profiles' =>'<strong>Why it exists:</strong> a player profile is an end-to-end assessment of a whole account, and building it means fanning out across every alt — per-character activity summaries, role profiles, in-game titles, role alignment, and the blueprint / buyback panels. Doing that fan-out on every page view is what makes a profile feel slow, especially for someone with a lot of alts. This command pre-computes that expensive half and caches it, so opening a profile reads a ready-made bundle instead of rebuilding it.<br><br><strong>Where it fits:</strong> it is the last step of <code>hr-manager:init</code> (when the feature is on), warming a first batch so profiles are fast straight after setup. From there the 30-minute schedule keeps them fresh: each pass rebuilds only the accounts whose data has actually moved, detected with a cheap change-fingerprint, so re-warming a whole roster costs a few indexed queries per unchanged account rather than a full rebuild. Unchanged accounts are still rebuilt at least every 12 hours, so a change the fingerprint cannot see reconciles on its own.<br><br><strong>It is optional</strong> (Settings &rarr; Features &rarr; Pre-warm player profiles, off by default). With it off nothing breaks — the profile builds its bundle on first view and caches it for 5 minutes, so the first open is slow and the next few are quick. With it on, bundles live 24 hours because the schedule owns freshness.<br><br><strong>Expect some load time regardless.</strong> The panels a director <em>acts</em> on — access depth, Discord roles, squads, notes, history — are deliberately never cached, because stale data there would be misleading. They are rebuilt live on every view. So on a modest server, a large account can still take a few seconds to open even fully warmed; pre-warming removes the heavy read-only half, not the whole page.',
    'cmd_send_onboarding_welcomes' => 'Posts queued new-member onboarding welcomes whose delay has elapsed. A welcome is only consumed once it is genuinely resolved — sent, the member has left, or it waited 7 days. Not-synced-yet, no webhook subscribing, no linked Discord identity and transient webhook failures all leave it pending for the next pass, so a welcome is never silently thrown away. Reports sent / dropped / still waiting / given up on. No-op when the feature is off or nothing is due.',
    'cmd_detect_token_loss' =>'The Token-loss Watchdog pass: detects members whose ESI refresh token has gone, fires the security alert + history event, applies the configured auto-purge policy, cancels any of its own purges whose token has been restored, and runs the opt-in squad-drop when due. Runs every 10 minutes.',
    ...
    'cmd_token_coverage'     => 'Opt-in weekly summary of each corp token + scope coverage to any webhook with the Token Coverage Digest category on. Runs Mondays at 09:00. No-op when no webhook subscribes.',
    'cmd_prune_audit_log'   => 'Delete Activity Log entries past the 180-day retention window (override with <code>--days=N</code>). Runs daily at 04:30, and runs even when the Activity Log feature is off so a disabled log still drains rather than lingering. Cheap when there is nothing to remove.',
    'cmd_diagnose'          => 'Print a self-test summary to stdout for support tickets. Run on demand.',
    'cmd_backfill_buyback'  => 'One-time seed of historical Buyback Manager offers and completed contracts into HR\'s buyback activity, so the contribution panels have history immediately. Run by hand after enabling the Buyback integration; live data flows via the EventBus thereafter.',
    ...
    'a_unmapped_tier' => 'Go to <strong>Tiers</strong> and assign the user to the correct tier; the classifier defaults to <code>L0 Member</code> when no explicit mapping exists. Automatic resolution from Discord roles needs the <strong>SeAT Connector</strong> framework — if you don\'t use Connector, assigning tiers by hand here is all you need (or skip tiers entirely and live with the L0 default).',
