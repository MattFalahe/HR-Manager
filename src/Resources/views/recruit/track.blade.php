...
<div class="hr-manager-wrapper">
            ...
            @if($canWithdraw)
                <div class="mt-3 pt-3" style="border-top: 1px solid var(--hr-border, rgba(255,255,255,0.08));">
                    <form method="POST" action="{{ route('hr-manager.recruit.track.withdraw', $application->tracking_token) }}"
                        ...
                        @csrf
                        @if(!empty($reasonEnabled))
                            <div class="form-group" style="max-width: 520px;">
                                <label for="withdrawReason" style="color: var(--hr-text-light); font-size: 0.9rem;">
                                    {{ trans('hr-manager::recruit.withdraw_reason_label') }}
                                    @if(!empty($reasonRequired))
                                        <span style="color: var(--hr-danger, #dc3545);">*</span>
                                    @else
                                        <small style="color: var(--hr-text-muted);">({{ trans('hr-manager::recruit.withdraw_reason_optional') }})</small>
                                    @endif
                                </label>
                                <textarea name="withdraw_reason" id="withdrawReason" class="form-control" rows="3"
                                          maxlength="1000" {{ !empty($reasonRequired) ? 'required' : '' }}
                                          placeholder="{{ trans('hr-manager::recruit.withdraw_reason_placeholder') }}">{{ old('withdraw_reason') }}</textarea>
                            </div>
                        @endif
    ...
    {{-- Outstanding step: link Discord via SeAT Connector. Shown when the
         Connector is installed but this applicant's account has no linked Discord
         identity yet, and the application is still live. --}}
    @if(!empty($showDiscordStep))
        <div class="card card-dark mb-3" style="border-left: 3px solid #5865F2;">
            <div class="card-body">
                <h4 style="color: var(--hr-text-white); margin-bottom: 8px;">
                    <i class="fab fa-discord" style="color: #5865F2;"></i> {{ trans('hr-manager::recruit.track_discord_step_heading') }}
                </h4>
                <p style="color: var(--hr-text-light); margin-bottom: 12px;">{{ trans('hr-manager::recruit.track_discord_step_body') }}</p>
                @if(!empty($connectorLinkUrl))
                    <a href="{{ $connectorLinkUrl }}" target="_blank" rel="noopener" class="btn btn-hr-primary btn-icon">
                        <i class="fab fa-discord"></i> {{ trans('hr-manager::recruit.track_discord_step_btn') }}
                    </a>
                @endif
            </div>
        </div>
    ...
    <div class="card card-dark mb-3">
        ...
        <div class="card-body">
            @forelse($application->publicStatusHistory as $entry)
