...
<style>
    ...
    .hr-manager-wrapper .hr-corp-tabs .nav-tabs .nav-link { padding: 10px 16px; color: var(--hr-text-muted, #9ca3af); border: 1px solid transparent; }
    .hr-manager-wrapper .hr-corp-tabs .nav-tabs .nav-link i { margin-right: 6px; }
    /* Inactive tabs lift slightly on hover so the row reads as clickable. */
    .hr-manager-wrapper .hr-corp-tabs .nav-tabs .nav-link:not(.active):hover {
        color: var(--hr-text-light, #e5e7eb) !important;
        background: rgba(255,255,255,0.04) !important;
        border-color: transparent !important;
    }
    /* Active tab: high-contrast so it's unmistakable which section you're on.
       Tinted indigo fill, bright text, and a solid accent top-bar. Overrides the
       AdminLTE / dark-theme default that rendered the active tab dimmer than the
       inactive ones. */
    .hr-manager-wrapper .hr-corp-tabs .nav-tabs .nav-link.active,
    .hr-manager-wrapper .hr-corp-tabs .nav-tabs .nav-item.show .nav-link {
        color: #ffffff !important;
        font-weight: 600;
        background: rgba(102,126,234,0.22) !important;
        border-color: transparent !important;
        border-top: 3px solid var(--hr-accent, #667eea) !important;
    }
    .hr-manager-wrapper .hr-corp-tabs .nav-tabs .nav-link.active i { color: #8fa3ff !important; }
    ...
    /* Attention badge for access gaps (unregistered / no Discord link). */
    @keyframes hr-access-blink { 0%, 100% { opacity: 1; } 50% { opacity: 0.4; } }
    .hr-manager-wrapper .hr-blink { animation: hr-access-blink 1.15s ease-in-out infinite; }
    @media (prefers-reduced-motion: reduce) { .hr-manager-wrapper .hr-blink { animation: none; } }
    /* Clickable access stat badges + active-filter ring. */
    .hr-manager-wrapper .access-stat-filter { transition: box-shadow 0.1s ease, transform 0.1s ease; user-select: none; }
    .hr-manager-wrapper .access-stat-filter:hover { transform: translateY(-1px); box-shadow: 0 0 0 1px rgba(255,255,255,0.35); }
    .hr-manager-wrapper .access-filter-on { box-shadow: 0 0 0 2px rgba(255,255,255,0.75); font-weight: 600; }
</style>
@endpush
...
<div class="hr-manager-wrapper">
    ...
    <div class="card card-dark card-tabs hr-corp-tabs">
        <div class="card-header p-0 pt-1">
            <ul class="nav nav-tabs">
                {{-- Assessment tabs first (who's contributing / at risk), then the
                     recruitment + purge WORKFLOW tabs at the end. --}}
                <li class="nav-item">
                ...
                @can('hr-manager.director')
                    ...
                    <li class="nav-item">
                        <a class="nav-link {{ $activeTab === 'membership' ? 'active' : '' }}" href="{{ $tabLink('membership') }}">
                            <i class="fas fa-user-plus"></i> {{ trans('hr-manager::corp-health.tab_membership') }}
                            @if(($membershipReviewCount ?? 0) > 0)
                                <span class="badge badge-danger ml-1">{{ $membershipReviewCount }}</span>
                            @endif
                        </a>
                    </li>
                    @if(!empty($roleAlignEnabled))
                        <li class="nav-item">
                            <a class="nav-link {{ $activeTab === 'alignment' ? 'active' : '' }}" href="{{ $tabLink('alignment') }}">
                                <i class="fas fa-user-tag"></i> {{ trans('hr-manager::corp-health.tab_alignment') }}
                            </a>
                        </li>
                    @endif
                    <li class="nav-item">
                        <a class="nav-link {{ $activeTab === 'access' ? 'active' : '' }}" href="{{ $tabLink('access') }}">
                            <i class="fas fa-key"></i> {{ trans('hr-manager::corp-health.tab_access') }}
                        </a>
                    </li>
                @endcan
                <li class="nav-item">
    ...
    {{-- Reusable client-side filter + pagination for long access/alignment
         lists. Defined once here (before the tab panes) so each pane can call
         hrPaginate() after rendering its rows. --}}
    <script>
    window.hrPaginate = function (opts) {
        var rows = Array.prototype.slice.call(document.querySelectorAll(opts.rows));
        if (!rows.length) { return; }
        var badges = opts.badges ? Array.prototype.slice.call(document.querySelectorAll(opts.badges)) : [];
        var attr = opts.attr || 'data-access';
        var sizeSel = opts.sizeSel ? document.getElementById(opts.sizeSel) : null;
        var pager = opts.pager ? document.getElementById(opts.pager) : null;
        var clearBtn = opts.clear ? document.getElementById(opts.clear) : null;
        var emptyMsg = opts.empty ? document.getElementById(opts.empty) : null;
        var countEl = opts.count ? document.getElementById(opts.count) : null;
        var activeCls = opts.activeClass || 'access-filter-on';
        var active = [];   // stackable: a row matches if it has ANY active flag
        var page = 1;
        var pageSize = sizeSel ? (parseInt(sizeSel.value, 10) || 50) : 50;

        function filtered() {
            if (!active.length) { return rows; }
            return rows.filter(function (r) {
                var acc = (r.getAttribute(attr) || '').split(' ');
                for (var i = 0; i < active.length; i++) {
                    if (acc.indexOf(active[i]) !== -1) { return true; }
                }
                return false;
            });
        }
        function ellipsis() {
            ...
            s.textContent = '…';
            s.style.cssText = 'padding:0 4px; color: var(--hr-text-muted);';
            return s;
        }
        function pageBtn(label, target, disabled, current) {
            var b = document.createElement('button');
            b.type = 'button';
            b.className = 'btn btn-sm ' + (current ? 'btn-hr-primary' : 'btn-hr-secondary');
        ...
        function buildPager(pages) {
            if (!pager) { return; }
            pager.innerHTML = '';
            if (pages <= 1) { pager.style.setProperty('display', 'none', 'important'); return; }
            ...
            if (from > 1) {
                pager.appendChild(pageBtn('1', 1, false, page === 1));
                if (from > 2) { pager.appendChild(ellipsis()); }
            }
            for (var i = from; i <= to; i++) { pager.appendChild(pageBtn(String(i), i, false, i === page)); }
            if (to < pages) {
                if (to < pages - 1) { pager.appendChild(ellipsis()); }
                pager.appendChild(pageBtn(String(pages), pages, false, page === pages));
            }
            pager.appendChild(pageBtn('›', page + 1, page >= pages, false));
        }
        function render() {
            var list = filtered(), total = list.length, pages = Math.max(1, Math.ceil(total / pageSize));
            if (page > pages) { page = pages; }
            if (page < 1) { page = 1; }
            var start = (page - 1) * pageSize, end = start + pageSize;
            // Inline !important beats Bootstrap's .d-flex { display:flex !important }
            // no matter where the row sits — no dependency on a scoped CSS class.
            rows.forEach(function (r) { r.style.setProperty('display', 'none', 'important'); });
            list.slice(start, end).forEach(function (r) { r.style.removeProperty('display'); });
            badges.forEach(function (b) {
                if (active.indexOf(b.getAttribute('data-filter')) !== -1) { b.classList.add(activeCls); }
                else { b.classList.remove(activeCls); }
            });
            if (clearBtn) { clearBtn.style.display = active.length ? '' : 'none'; }
            if (emptyMsg) { emptyMsg.style.display = total === 0 ? '' : 'none'; }
            if (countEl) { countEl.textContent = total === 0 ? '' : ((start + 1) + '–' + Math.min(end, total) + ' / ' + total); }
            buildPager(pages);
        }
        badges.forEach(function (b) {
            b.addEventListener('click', function () {
                ...
                if (idx === -1) { active.push(f); } else { active.splice(idx, 1); }
                page = 1;
                render();
            });
        });
        if (clearBtn) { clearBtn.addEventListener('click', function () { active = []; page = 1; render(); }); }
        if (sizeSel) { sizeSel.addEventListener('change', function () { pageSize = parseInt(sizeSel.value, 10) || 50; page = 1; render(); }); }
        render();
    };
    </script>
    ...
    @if($activeTab === 'overview')
                ...
                @else
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover mb-0">
                            ...
                            <tbody>
                                @foreach(($dh['inactive_grouped'] ?? $dh['inactive']) as $gi => $grp)
                                    @php
                                        ...
                                        $grpClass    = $grp['classifier'] ?? ($head['classifier'] ?? null);
                                        $grpUserId   = $grp['user_id'] ?? $head['user_id'];
                                        $mainDiffers = (int) $mainId !== (int) $head['character_id'];
                                        $expandable  = count($chars) > 1 || $mainDiffers;
                                    @endphp
                                    <tr>
                                        <td>
                                            <img src="https://images.evetech.net/characters/{{ $mainId }}/portrait?size=32" style="width:28px;height:28px;border-radius:50%;vertical-align:middle;margin-right:6px;" alt="">
                                            <strong>{{ $mainName }}</strong>
                                            @if($expandable)
                                                <button type="button" class="btn btn-sm btn-link p-0 ml-1" style="color: var(--hr-text-muted); text-decoration:none;" data-toggle="collapse" data-target="#dir-grp-{{ $gi }}">
                                                    <i class="fas fa-chevron-down" style="font-size:0.7rem;"></i>
                                                    {{ trans_choice('hr-manager::corp-health.dir_char_count', count($chars), ['count' => count($chars)]) }}
                                                </button>
                                            @endif
                                        </td>
                                        <td>
                                            <strong class="text-danger">{{ $head['days_since_logon'] !== null ? $head['days_since_logon'] . 'd' : trans('hr-manager::corp-health.unknown') }}</strong>
                                            @if($head['last_logon'])
                                                <small class="text-muted d-block">{{ \Carbon\Carbon::parse($head['last_logon'])->diffForHumans() }}</small>
                                            @endif
                                            @if($mainDiffers)
                                                <small class="text-muted d-block">{{ trans('hr-manager::corp-health.dir_on_char', ['name' => $head['name']]) }}</small>
                                            @endif
                                        </td>
                                        <td>
                                            @if(!$isAuthed)
                                            ...
                                            @elseif($grpClass)
                                                <span class="badge badge-hr">{{ trans('hr-manager::corp-health.cat_' . $grpClass) }}</span>
                                            @endif
                                        </td>
                                        <td>
                                            @if($grpUserId)
                                                <a class="btn btn-sm btn-hr-secondary" href="{{ route('hr-manager.players.show', ['id' => $grpUserId, 'corporation_id' => $corporationId]) }}">{{ trans('hr-manager::corp-health.view_player') }}</a>
                                            @endif
                                        </td>
                                    </tr>
                                    @if($expandable)
                                        <tr>
                                            <td colspan="4" class="p-0" style="border-top:0;">
                                                <div class="collapse" id="dir-grp-{{ $gi }}">
                                                    <div style="background: rgba(255,255,255,0.02); padding: 6px 12px 8px 46px;">
                                                        @foreach($chars as $c)
                                                            <div style="display:flex; align-items:center; gap:8px; padding:3px 0; flex-wrap:wrap;">
                                                                <img src="https://images.evetech.net/characters/{{ $c['character_id'] }}/portrait?size=32" style="width:22px;height:22px;border-radius:50%;" alt="">
                                                                <span style="color: var(--hr-text-light);">{{ $c['name'] }}</span>
                                                                <strong class="text-danger">{{ $c['days_since_logon'] !== null ? $c['days_since_logon'] . 'd' : trans('hr-manager::corp-health.unknown') }}</strong>
                                                                @if($c['last_logon'])<small class="text-muted">{{ \Carbon\Carbon::parse($c['last_logon'])->diffForHumans() }}</small>@endif
                                                                @if(!$c['is_authed'])<span class="badge" style="background: rgba(255,193,7,0.25); color:#ffe08a; font-size:0.6rem;"><i class="fas fa-user-slash"></i> {{ trans('hr-manager::corp-health.not_in_seat') }}</span>@endif
                                                            </div>
                                                        @endforeach
                                                    </div>
                                                </div>
        ...
        <div class="card card-dark mb-3">
            ...
            <div class="card-body p-0">
                ...
                @else
                    @php
                        // Explicit high-contrast colours per flag severity, so the
                        // tiny codes read clearly regardless of the theme / Bootstrap
                        // badge defaults: amber uses DARK text, the rest white.
                        $wfStyle = function ($flag) {
                            return match ($flag) {
                                'silent_wallet_director', 'compliance_very_low' => 'background:#dc3545;color:#fff;',
                                'loyalty_hold', 'blueprint_hold', 'buyback_hold' => 'background:#2fa84f;color:#fff;',
                                'dormant_alts'                                   => 'background:#5a6472;color:#fff;',
                                default                                          => 'background:#e6a700;color:#1a1d21;',
                            };
                        };
                        $wfAll = ['stalled','negative_contribution','compliance_low','compliance_very_low','silent_wallet_director','loyalty_hold','blueprint_hold','buyback_hold','dormant_alts'];
                    @endphp
                    {{-- Legend for the Wallet flags column. --}}
                    <div class="px-3 py-2" style="border-bottom:1px solid var(--hr-border, rgba(255,255,255,0.08)); display:flex; flex-wrap:wrap; gap:6px 16px; align-items:center;">
                        <span style="color:var(--hr-text-muted); font-size:0.72rem; text-transform:uppercase; letter-spacing:0.5px;">{{ trans('hr-manager::corp-health.wf_legend_label') }}</span>
                        @foreach($wfAll as $lf)
                            <span style="display:inline-flex; align-items:center; gap:5px; white-space:nowrap;" title="{{ trans('hr-manager::corp-health.wf_' . $lf) }}">
                                <span class="badge" style="{{ $wfStyle($lf) }} font-weight:600;">{{ trans('hr-manager::corp-health.wf_short_' . $lf) }}</span>
                                <small style="color:var(--hr-text-light); font-size:0.72rem;">{{ trans('hr-manager::corp-health.wf_legend_' . $lf) }}</small>
                            </span>
                        @endforeach
                    </div>
                    <div class="table-responsive">
                        <table class="table table-hover mb-0">
                            ...
                            <tbody>
                                @foreach($atRiskOrWorse as $c)
                                    @php
                                        $cMain = $classifierMains[$c->user_id] ?? null;
                                        $cName = $classifierNames[$c->user_id] ?? ('User #' . $c->user_id);
                                        $cAlts = $classifierAlts[$c->user_id] ?? [];
                                        $cExpandable = count($cAlts) > 1;
                                    @endphp
                                    <tr>
                                        <td>
                                            @if($cMain)
                                                <img src="https://images.evetech.net/characters/{{ $cMain }}/portrait?size=32" class="rounded-circle mr-2" width="28" height="28" alt="" onerror="this.style.display='none'">
                                            @endif
                                            <a href="{{ route('hr-manager.players.show', ['id' => $c->user_id, 'corporation_id' => $corporationId]) }}" style="color: var(--hr-text-white, #fff); text-decoration: none;">{{ $cName }}</a>
                                            @if($cExpandable)
                                                <button type="button" class="btn btn-sm btn-link p-0 ml-1" style="color: var(--hr-text-muted); text-decoration:none;" data-toggle="collapse" data-target="#cls-grp-{{ $loop->index }}">
                                                    <i class="fas fa-chevron-down" style="font-size:0.7rem;"></i>
                                                    {{ trans_choice('hr-manager::corp-health.char_count', count($cAlts), ['count' => count($cAlts)]) }}
                                                </button>
                                            @endif
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            @foreach((array) ($c->wallet_flags ?? []) as $flag)
                                                <span class="badge mr-1" style="{{ $wfStyle($flag) }} font-weight:600;" title="{{ trans('hr-manager::corp-health.wf_' . $flag) }}">
                                    ...
                                    @if($cExpandable)
                                        <tr>
                                            <td colspan="8" class="p-0" style="border-top:0;">
                                                <div class="collapse" id="cls-grp-{{ $loop->index }}">
                                                    <div style="background: rgba(255,255,255,0.02); padding: 6px 12px 8px 46px;">
                                                        @foreach($cAlts as $a)
                                                            <div style="display:flex; align-items:center; gap:8px; padding:3px 0; flex-wrap:wrap;">
                                                                <img src="https://images.evetech.net/characters/{{ $a['character_id'] }}/portrait?size=32" style="width:22px;height:22px;border-radius:50%;" alt="" onerror="this.style.display='none'">
                                                                <span style="color: var(--hr-text-light);">{{ $a['name'] }}</span>
                                                                @if($a['is_main'])
                                                                    <span class="badge badge-hr badge-member" style="font-size:0.58rem;">{{ trans('hr-manager::corp-health.main_badge') }}</span>
                                                                @endif
                                                            </div>
                                                        @endforeach
                                                    </div>
    ...
    @if($activeTab === 'composition')
                                ...
                                <table class="table table-hover mb-0">
                                    ...
                                    <tbody>
                                        @foreach($dr['list'] as $d)
                                            ...
                                            @endphp
                                            @php
                                                $drShowHeldBy = ($d['role_char_count'] ?? 1) > 1 || empty($d['main_holds']);
                                            @endphp
                                            <tr>
                                                <td>
                                                    ...
                                                    <strong>{{ $d['name'] }}</strong>
                                                    @if($drShowHeldBy)
                                                        <div class="small mt-1" style="color: var(--hr-text-muted); padding-left: 34px;">
                                                            <span style="text-transform: uppercase; letter-spacing: 0.3px;">{{ trans('hr-manager::corp-health.held_by') }}</span>
                                                            @foreach($d['role_chars'] as $rc)
                                                                <span class="badge ml-1" style="background: rgba(255,255,255,0.06); color: var(--hr-text-light);">{{ $rc['name'] }}@if(!empty($rc['is_main'])) <i class="fas fa-star" style="color: #ffd479;" title="{{ trans('hr-manager::corp-health.main_badge') }}"></i>@endif</span>
                                                            @endforeach
                                                        </div>
                                                    @endif
                                                </td>
        ...
        {{-- Recruitment access map: who can accept applications in HR vs who holds
             the matching in-game role, so mismatches surface. --}}
        @if(!empty($accessMap['rows']))
            <div class="card card-dark mb-3">
                <div class="card-header" style="border-left: 4px solid {{ ($accessMap['mismatch_count'] ?? 0) > 0 ? 'var(--hr-warning, #ffc107)' : 'var(--hr-success, #28a745)' }};">
                    <h3 class="card-title"><i class="fas fa-user-shield"></i> {{ trans('hr-manager::corp-health.access_map_heading') }}</h3>
                    @if(($accessMap['mismatch_count'] ?? 0) > 0)
                        <div class="card-tools">
                            <span class="badge" style="background: rgba(255,193,7,0.25); color:#ffe08a;">{{ trans('hr-manager::corp-health.access_map_mismatches', ['n' => $accessMap['mismatch_count']]) }}</span>
                        </div>
                    @endif
                </div>
                <div class="card-body p-0">
                    <p class="px-3 pt-3 mb-2" style="color: var(--hr-text-muted); font-size: 0.85rem;">{{ trans('hr-manager::corp-health.access_map_intro') }}</p>
                    <div class="table-responsive">
                        <table class="table table-hover mb-0">
                            <thead>
                                <tr>
                                    <th>{{ trans('hr-manager::corp-health.access_map_col_player') }}</th>
                                    <th>{{ trans('hr-manager::corp-health.access_map_col_hr') }}</th>
                                    <th>{{ trans('hr-manager::corp-health.access_map_col_ingame') }}</th>
                                    <th>{{ trans('hr-manager::corp-health.access_map_col_verdict') }}</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach($accessMap['rows'] as $r)
                                    @php
                                        $vmap = [
                                            'aligned'        => ['#28a745', 'fa-check-circle'],
                                            'no_ingame'      => ['#ffc107', 'fa-exclamation-triangle'],
                                            'no_hr'          => ['#ffc107', 'fa-exclamation-triangle'],
                                            'recruiter_only' => ['#8b95a5', 'fa-user'],
                                        ];
                                        $vc = $vmap[$r['verdict']] ?? $vmap['recruiter_only'];
                                    @endphp
                                    <tr>
                                        <td style="white-space: nowrap;">
                                            <img src="https://images.evetech.net/characters/{{ $r['main_character_id'] }}/portrait?size=32" style="width:24px;height:24px;border-radius:50%;vertical-align:middle;margin-right:6px;" alt="">
                                            <strong>{{ $r['main_name'] }}</strong>
                                        </td>
                                        <td>
                                            @if($r['hr']['admin'])<span class="badge badge-secondary mr-1">{{ trans('hr-manager::permissions.admin_label') }}</span>@endif
                                            @if($r['hr']['director'])<span class="badge badge-secondary mr-1">{{ trans('hr-manager::permissions.director_label') }}</span>@endif
                                            @if($r['hr']['personnel'])<span class="badge mr-1" style="background: rgba(102,126,234,0.25); color:#c3ccf8;">{{ trans('hr-manager::permissions.personnel_label') }}</span>@endif
                                            @if($r['hr']['recruiter'] && !$r['hr']['can_decide'])<span class="badge badge-secondary mr-1">{{ trans('hr-manager::permissions.recruiter_label') }}</span>@endif
                                            @if(!$r['hr']['recruiter'])<span style="color: var(--hr-text-muted);">—</span>@endif
                                        </td>
                                        <td>
                                            @if($r['ingame']['director'])<span class="badge badge-warning mr-1"><i class="fas fa-crown"></i> {{ trans('hr-manager::corp-health.access_ingame_director') }}</span>@endif
                                            @if($r['ingame']['pm'])<span class="badge" style="background: rgba(40,167,69,0.22); color:#6ee7b7;"><i class="fas fa-user-check"></i> {{ trans('hr-manager::corp-health.access_ingame_pm') }}</span>@endif
                                            @if(!$r['ingame']['director'] && !$r['ingame']['pm'])<span style="color: var(--hr-text-muted);">—</span>@endif
                                            @php
                                                $ingameAlts = array_values(array_filter($r['ingame']['chars'] ?? [], fn ($c) => empty($c['is_main'])));
                                            @endphp
                                            @if(!empty($ingameAlts))
                                                <div class="small mt-1" style="color: var(--hr-text-muted);">
                                                    <span style="text-transform: uppercase; letter-spacing: 0.3px;">{{ trans('hr-manager::corp-health.held_by') }}</span>
                                                    @foreach($ingameAlts as $c)
                                                        <span class="badge ml-1" style="background: rgba(255,255,255,0.06); color: var(--hr-text-light);">{{ $c['name'] }}@if($c['director']) <i class="fas fa-crown" style="color: #fcd34d;" title="{{ trans('hr-manager::corp-health.access_ingame_director') }}"></i>@elseif($c['pm']) <i class="fas fa-user-check" style="color: #6ee7b7;" title="{{ trans('hr-manager::corp-health.access_ingame_pm') }}"></i>@endif</span>
                                                    @endforeach
                                                </div>
                                            @endif
                                        </td>
                                        <td>
                                            <span class="badge" style="background: {{ $vc[0] }}22; color: {{ $vc[0] }}; border: 1px solid {{ $vc[0] }}66;" title="{{ trans('hr-manager::corp-health.access_verdict_' . $r['verdict'] . '_hint') }}">
                                                <i class="fas {{ $vc[1] }}"></i> {{ trans('hr-manager::corp-health.access_verdict_' . $r['verdict']) }}
                                            </span>
                                        </td>
                                    </tr>
                                @endforeach
                    ...
                    <div class="row">
                        @foreach($tc['list'] as $i => $entry)
                            <div class="col-md col-6 text-center mb-2">
                                ...
                                @include('hr-manager::corp-health.partials._alt_badge', ['altCount' => $entry['alt_count'] ?? 1, 'alts' => $entry['alts'] ?? [], 'iskFmt' => $iskFmt])
                            ...
                            <div class="card-body">
                                ...
                                @forelse($wi_anom['list'] as $row)
                                    <div class="d-flex align-items-center mb-1" style="gap: 8px; padding: 5px 8px; background: rgba(255,193,7,0.07); border-radius: 4px;">
                                        ...
                                        @else
                                            <a href="{{ route('hr-manager.members.show', $row['character_id']) }}" style="color: var(--hr-text-white); text-decoration: none; font-size: 0.9rem;">{{ $row['name'] }}</a>
                                            @include('hr-manager::corp-health.partials._alt_badge', ['altCount' => $row['alt_count'] ?? 1, 'alts' => $row['alts'] ?? [], 'iskFmt' => $iskFmt])
                        ...
                        <table class="table table-sm" style="margin-top: 8px;">
                            ...
                            <tbody>
                                @foreach($bps['top_requesters'] as $tr)
                                    @php $bpMulti = ($tr['char_count'] ?? 1) > 1; @endphp
                                    <tr>
                                        <td>
                                            <img src="https://images.evetech.net/characters/{{ $tr['character_id'] }}/portrait?size=32" style="width: 22px; height: 22px; border-radius: 50%; vertical-align: middle; margin-right: 6px;" alt="" onerror="this.style.display='none'">
                                            {{ $tr['character_name'] }}
                                            @if($bpMulti)
                                                <button type="button" class="btn btn-sm btn-link p-0 ml-1" style="color: var(--hr-text-muted); text-decoration:none;" data-toggle="collapse" data-target="#bp-req-{{ $loop->index }}">
                                                    <i class="fas fa-chevron-down" style="font-size:0.7rem;"></i>
                                                    {{ trans_choice('hr-manager::corp-health.char_count', $tr['char_count'], ['count' => $tr['char_count']]) }}
                                                </button>
                                            @endif
                                        </td>
                                    ...
                                    @if($bpMulti)
                                        <tr>
                                            <td colspan="4" class="p-0" style="border-top:0;">
                                                <div class="collapse" id="bp-req-{{ $loop->index }}">
                                                    <table class="table table-sm mb-0" style="background: rgba(255,255,255,0.02);">
                                                        <tbody>
                                                            @foreach($tr['chars'] as $c)
                                                                <tr>
                                                                    <td style="padding-left: 46px;">
                                                                        <img src="https://images.evetech.net/characters/{{ $c['character_id'] }}/portrait?size=32" style="width: 18px; height: 18px; border-radius: 50%; vertical-align: middle; margin-right: 6px;" alt="" onerror="this.style.display='none'">
                                                                        {{ $c['character_name'] }}
                                                                        @if(!empty($c['is_main']))<span class="badge badge-hr badge-member ml-1" style="font-size:0.58rem;">{{ trans('hr-manager::corp-health.main_badge') }}</span>@endif
                                                                    </td>
                                                                    <td class="text-right">{{ $c['total_requests'] }}</td>
                                                                    <td class="text-right" style="color: #6ee7b7;">{{ $c['fulfilled'] }}</td>
                                                                    <td class="text-right" style="color: #fca5a5;">{{ $c['rejected'] }}</td>
                                                                </tr>
                                                            @endforeach
                                                        </tbody>
                                                    </table>
    ...
    @if($activeTab === 'membership')
        @php $mem = $membership ?? ['available' => false]; @endphp
        @if(empty($mem['available']))
            <p class="text-muted">{{ trans('hr-manager::corp-health.mem_unavailable') }}</p>
        @else
            @php
                $memNames = $mem['names'] ?? [];
                $memName = fn ($id) => $id ? ($memNames[(int) $id] ?? ('Character #' . $id)) : null;
            @endphp

            {{-- Review queue: joined without a valid application --}}
            <div class="card card-dark mb-3" style="border-left: 4px solid {{ ($mem['review_count'] ?? 0) > 0 ? 'var(--hr-danger, #dc3545)' : 'var(--hr-success, #28a745)' }};">
                <div class="card-header">
                    <h3 class="card-title"><i class="fas fa-user-shield"></i> {{ trans('hr-manager::corp-health.mem_review_heading') }}</h3>
                    <div class="card-tools"><span class="badge {{ ($mem['review_count'] ?? 0) > 0 ? 'badge-danger' : 'badge-success' }}">{{ $mem['review_count'] ?? 0 }}</span></div>
                </div>
                <div class="card-body">
                    @forelse($mem['review_queue'] as $ev)
                        <div class="d-flex flex-wrap align-items-center" style="gap: 10px; padding: 8px 0; border-bottom: 1px solid rgba(255,255,255,0.06);">
                            <img src="https://images.evetech.net/characters/{{ $ev->character_id }}/portrait?size=32" style="width: 32px; height: 32px; border-radius: 50%;" onerror="this.style.display='none'">
                            <div style="flex: 1; min-width: 180px;">
                                <strong style="color: var(--hr-text-white);">{{ $memName($ev->character_id) }}</strong>
                                @if($ev->main_character_id && $ev->main_character_id !== $ev->character_id)
                                    <span class="text-muted">· {{ trans('hr-manager::corp-health.mem_main') }}: {{ $memName($ev->main_character_id) }}</span>
                                @endif
                                <div class="text-muted" style="font-size: 0.78rem;">{{ trans('hr-manager::corp-health.mem_joined_no_app') }} · @hrDate($ev->occurred_at)</div>
                            </div>
                            <a href="https://zkillboard.com/character/{{ $ev->character_id }}/" target="_blank" rel="noopener" class="btn btn-xs btn-hr-secondary"><i class="fas fa-external-link-alt"></i> zKB</a>
                            <form method="POST" action="{{ route('hr-manager.corp-health.membership-review', $ev->id) }}" class="d-flex" style="gap: 6px;">
                                @csrf
                                <input type="text" name="review_note" class="form-control form-control-sm" maxlength="500" placeholder="{{ trans('hr-manager::corp-health.mem_review_note') }}" style="width: 150px;">
                                <button type="submit" class="btn btn-xs btn-hr-primary"><i class="fas fa-check"></i> {{ trans('hr-manager::corp-health.mem_mark_reviewed') }}</button>
                            </form>
                        </div>
                    @empty
                        <p class="text-muted mb-0">{{ trans('hr-manager::corp-health.mem_review_empty') }}</p>
                    @endforelse
                </div>
            </div>

            {{-- Pending registration: current members not linked to SeAT --}}
            <div class="card card-dark mb-3" style="border-left: 4px solid {{ ($mem['pending_count'] ?? 0) > 0 ? 'var(--hr-warning, #ffc107)' : 'var(--hr-success, #28a745)' }};">
                <div class="card-header">
                    <h3 class="card-title"><i class="fas fa-id-badge"></i> {{ trans('hr-manager::corp-health.mem_pending_heading') }}</h3>
                    <div class="card-tools"><span class="badge {{ ($mem['pending_count'] ?? 0) > 0 ? 'badge-warning' : 'badge-success' }}">{{ $mem['pending_count'] ?? 0 }}</span></div>
                </div>
                <div class="card-body">
                    <p class="text-muted" style="font-size: 0.82rem; margin-bottom: 8px;">{{ trans('hr-manager::corp-health.mem_pending_intro') }}</p>
                    @forelse($mem['pending'] as $pm)
                        <div class="d-flex flex-wrap align-items-center" style="gap: 10px; padding: 6px 0; border-bottom: 1px solid rgba(255,255,255,0.05);">
                            <img src="https://images.evetech.net/characters/{{ $pm->character_id }}/portrait?size=32" style="width: 28px; height: 28px; border-radius: 50%;" onerror="this.style.display='none'">
                            <span style="color: var(--hr-text-white); flex: 1; min-width: 140px;">{{ $memName($pm->character_id) }}</span>
                            <span class="text-muted" style="font-size: 0.76rem;">{{ trans('hr-manager::corp-health.mem_pending_since') }} @hrDate($pm->first_seen_at)</span>
                            <a href="https://zkillboard.com/character/{{ $pm->character_id }}/" target="_blank" rel="noopener" class="btn btn-xs btn-hr-secondary"><i class="fas fa-external-link-alt"></i> zKB</a>
                        </div>
                    @empty
                        <p class="text-muted mb-0">{{ trans('hr-manager::corp-health.mem_pending_empty') }}</p>
                    @endforelse
                </div>
            </div>
            ...
            <div class="card card-dark mb-3">
                <div class="card-header">
                    <h3 class="card-title"><i class="fas fa-exchange-alt"></i> {{ trans('hr-manager::corp-health.mem_recent_heading', ['days' => $mem['recent_days']]) }}</h3>
                </div>
                <div class="card-body">
                    @forelse($mem['recent'] as $ev)
                        @php
                            $changeBadge = match ($ev->change_type) {
                                'joined'     => ['badge-success', trans('hr-manager::corp-health.mem_joined')],
                                'left'       => ['badge-secondary', trans('hr-manager::corp-health.mem_left')],
                                'registered' => ['badge-info', trans('hr-manager::corp-health.mem_registered')],
                                default      => ['badge-secondary', $ev->change_type],
                            };
                            $clsBadge = match ($ev->classification) {
                                'no_application' => ['badge-danger', trans('hr-manager::corp-health.mem_cls_no_app')],
                                'unregistered'   => ['badge-danger', trans('hr-manager::corp-health.mem_cls_unregistered')],
                                'known_alt'      => ['badge-info', trans('hr-manager::corp-health.mem_cls_alt')],
                                'applied'        => ['badge-success', trans('hr-manager::corp-health.mem_cls_applied')],
                                default          => [null, null],
                            };
                        @endphp
                        <div class="d-flex flex-wrap align-items-center" style="gap: 8px; padding: 5px 0; border-bottom: 1px solid rgba(255,255,255,0.05);">
                            <img src="https://images.evetech.net/characters/{{ $ev->character_id }}/portrait?size=32" style="width: 24px; height: 24px; border-radius: 50%;" onerror="this.style.display='none'">
                            <span class="badge {{ $changeBadge[0] }}">{{ $changeBadge[1] }}</span>
                            <span style="color: var(--hr-text-white);">{{ $memName($ev->character_id) }}</span>
                            @if($ev->main_character_id && $ev->main_character_id !== $ev->character_id)
                                <span class="text-muted" style="font-size: 0.8rem;">· {{ $memName($ev->main_character_id) }}</span>
                            @endif
                            @if($clsBadge[0])
                                <span class="badge {{ $clsBadge[0] }}">{{ $clsBadge[1] }}</span>
                            @endif
                            @if($ev->change_type === 'left' && $ev->player_still_present)
                                <span class="text-muted" style="font-size: 0.75rem;">({{ trans('hr-manager::corp-health.mem_alt_left') }})</span>
                            @endif
                            @if($ev->reviewed_at)
                                <span class="text-muted" style="font-size: 0.72rem;"><i class="fas fa-check"></i> {{ trans('hr-manager::corp-health.mem_reviewed_tag') }}</span>
                            @endif
                            <span class="ml-auto text-muted" style="font-size: 0.78rem;">@hrDate($ev->occurred_at)</span>
                        </div>
                    @empty
                        <p class="text-muted mb-0">{{ trans('hr-manager::corp-health.mem_recent_empty') }}</p>
                    @endforelse
                </div>
            </div>
        @endif
    ...
    @if($activeTab === 'alignment')
        <div class="card card-dark mb-3">
            <div class="card-header">
                <h3 class="card-title"><i class="fas fa-user-tag"></i> {{ trans('hr-manager::corp-health.align_heading') }}</h3>
                @if(!empty($roleAlignment['available']) && ($roleAlignment['total'] ?? 0) > 0)
                    <div class="card-tools">
                        <span class="badge access-stat-filter" data-filter="aligned" role="button" title="{{ trans('hr-manager::corp-health.access_filter_hint') }}" style="background: rgba(40,167,69,0.2); color:#6ee7b7; cursor: pointer;">{{ $roleAlignment['aligned_count'] }} {{ trans('hr-manager::corp-health.align_count_aligned') }}</span>
                        @if(($roleAlignment['misaligned_count'] ?? 0) > 0)
                            <span class="badge access-stat-filter" data-filter="misaligned" role="button" title="{{ trans('hr-manager::corp-health.access_filter_hint') }}" style="background: rgba(255,193,7,0.25); color:#ffe08a; cursor: pointer;">{{ $roleAlignment['misaligned_count'] }} {{ trans('hr-manager::corp-health.align_count_misaligned') }}</span>
                        @endif
                        <span id="align-filter-clear" class="badge" role="button" title="{{ trans('hr-manager::corp-health.access_filter_clear') }}" style="display: none; background: rgba(255,255,255,0.08); color: var(--hr-text-light); cursor: pointer;"><i class="fas fa-times"></i> {{ trans('hr-manager::corp-health.access_filter_clear') }}</span>
                    </div>
                @endif
            </div>
            <div class="card-body">
                <p style="color: var(--hr-text-muted); font-size: 0.85rem;">{{ trans('hr-manager::corp-health.align_intro') }}</p>
                @if(empty($roleAlignment['available']) || empty($roleAlignment['players']))
                    <p class="text-muted mb-0">{{ trans('hr-manager::corp-health.align_none') }}</p>
                @else
                    @include('hr-manager::corp-health.partials._list_pager_controls', ['prefix' => 'align'])
                    @foreach($roleAlignment['players'] as $p)
                        <div class="align-person-row mb-3" data-align="{{ $p['aligned'] ? 'aligned' : 'misaligned' }}" style="padding: 10px 12px; border-radius: 6px; background: rgba(255,255,255,0.03); border-left: 3px solid {{ $p['aligned'] ? 'var(--hr-success, #28a745)' : 'var(--hr-warning, #ffc107)' }};">
                            <div class="d-flex align-items-center mb-2" style="gap: 8px; flex-wrap: wrap;">
                                <img src="https://images.evetech.net/characters/{{ $p['main_character_id'] }}/portrait?size=32" style="width:26px;height:26px;border-radius:50%;" alt="">
                                <strong style="color: var(--hr-text-white, #fff);">{{ $p['main_name'] }}</strong>
                                <small class="text-muted">{{ trans('hr-manager::corp-health.align_char_count', ['n' => $p['char_count']]) }}</small>
                                <a href="{{ route('hr-manager.players.show', ['id' => $p['user_id'], 'corporation_id' => $corporationId]) }}" class="btn btn-sm btn-hr-secondary ml-auto">{{ trans('hr-manager::corp-health.view_player') }}</a>
                            </div>
                            @include('hr-manager::players.partials._role_alignment', ['alignment' => ['available' => true, 'aligned' => $p['aligned'], 'characters' => $p['characters'], 'ref_character_id' => $p['main_character_id']]])
                        </div>
                    @endforeach
                    <div id="align-filter-empty" style="display: none; color: var(--hr-text-muted); font-size: 0.85rem; padding: 8px;">
                        <i class="fas fa-filter"></i> {{ trans('hr-manager::corp-health.access_filter_none') }}
                    </div>
                    <nav id="align-pager" class="d-flex flex-wrap justify-content-center mt-2" style="gap: 4px; display: none !important;"></nav>
                    <script>
                    hrPaginate({
                        rows: '.align-person-row', badges: '.access-stat-filter', attr: 'data-align',
                        sizeSel: 'align-size', pager: 'align-pager', clear: 'align-filter-clear',
                        empty: 'align-filter-empty', count: 'align-count', activeClass: 'access-filter-on'
                    });
                    </script>
                @endif
            </div>
    ...
    {{-- =================================================================
         ACCESS TAB — corp-wide SeAT + Discord access depth (director-only).
         ================================================================= --}}
    @if($activeTab === 'access' && $corpAccess !== null)
        @php
            $ax = $corpAccess; $axS = $ax['seat']; $axD = $ax['discord'];
            // Human-readable "why does this account have this Discord role" from
            // the connector's set-entity mapping (squad / SeAT role / corp / …).
            $discordSourceTip = function ($src) {
                return match ($src['type'] ?? '') {
                    'squad'       => trans('hr-manager::corp-health.access_src_squad', ['name' => $src['name'] ?? '?']),
                    'seat_role'   => trans('hr-manager::corp-health.access_src_seat_role', ['name' => $src['name'] ?? '?']),
                    'corporation' => trans('hr-manager::corp-health.access_src_corp'),
                    'alliance'    => trans('hr-manager::corp-health.access_src_alliance'),
                    'direct'      => trans('hr-manager::corp-health.access_src_direct'),
                    'public'      => trans('hr-manager::corp-health.access_src_public'),
                    default       => '',
                };
            };
        @endphp
        ...
        <div class="card card-dark mb-3">
            <div class="card-header">
                <h3 class="card-title"><i class="fas fa-key"></i> {{ trans('hr-manager::corp-health.access_heading') }}</h3>
            </div>
            <div class="card-body">
                <p style="color: var(--hr-text-muted); font-size: 0.85rem;">{{ trans('hr-manager::corp-health.access_intro') }}</p>

                {{-- Risk call-outs: deep access on disengaged accounts. Most
                     important thing on the tab, so it leads. --}}
                @if(!empty($ax['mismatches']))
                    <div class="mb-3" style="border: 1px solid rgba(220,53,69,0.35); border-radius: 6px; overflow: hidden;">
                        <div style="padding: 8px 12px; background: rgba(220,53,69,0.15); color: #fca5a5; font-weight: 600; font-size: 0.85rem;">
                            <i class="fas fa-exclamation-triangle"></i> {{ trans('hr-manager::corp-health.access_risk_heading', ['count' => count($ax['mismatches'])]) }}
                        </div>
                        <div style="padding: 6px 12px;">
                            @foreach($ax['mismatches'] as $m)
                                @php $mc = $m['severity'] === 'high' ? '#dc3545' : '#ffc107'; @endphp
                                <div class="d-flex align-items-center" style="gap: 8px; padding: 5px 0; border-bottom: 1px solid rgba(255,255,255,0.05); font-size: 0.85rem;">
                                    <i class="fas fa-user-shield" style="color: {{ $mc }};"></i>
                                    <strong style="color: var(--hr-text-light);">{{ $m['main_name'] }}</strong>
                                    <span style="color: var(--hr-text-muted);">
                                        {{ $m['type'] === 'seat_power_dormant'
                                            ? trans('hr-manager::corp-health.access_risk_seat', ['what' => $m['detail']])
                                            : trans('hr-manager::corp-health.access_risk_discord', ['roles' => $m['detail']]) }}
                                    </span>
                                    @if(!empty($m['category']))
                                        <span class="badge ml-auto" style="background: rgba(220,53,69,0.18); color: #fca5a5;">{{ trans('hr-manager::corp-health.cat_' . $m['category']) }}</span>
                                    @endif
                                </div>
                            @endforeach
                        </div>
                ...
                {{-- Combined access stats (SeAT + Discord) — click a badge to
                     filter the rows below to just those people. --}}
                <div class="d-flex flex-wrap align-items-center mb-2" style="gap: 8px; font-size: 0.85rem;">
                    <span class="badge access-stat-filter" data-filter="superuser" role="button" title="{{ trans('hr-manager::corp-health.access_filter_hint') }}" style="background: rgba(220,53,69,0.2); color: #fca5a5; cursor: pointer;"><i class="fas fa-crown"></i> {{ trans('hr-manager::corp-health.access_stat_superusers', ['n' => $axS['superuser_count']]) }}</span>
                    <span class="badge access-stat-filter" data-filter="ingame_director" role="button" title="{{ trans('hr-manager::corp-health.access_filter_hint') }}" style="background: rgba(245,158,11,0.18); color: #fcd34d; cursor: pointer;">{{ trans('hr-manager::corp-health.access_stat_ingame_directors', ['n' => $axS['ingame_director_count']]) }}</span>
                    <span class="badge access-stat-filter" data-filter="hr_director" role="button" title="{{ trans('hr-manager::corp-health.access_filter_hint') }}" style="background: rgba(102,126,234,0.18); color: #c7d2fe; cursor: pointer;">{{ trans('hr-manager::corp-health.access_stat_hr_directors', ['n' => $axS['hr_director_count']]) }}</span>
                    <span class="badge access-stat-filter" data-filter="hr_admin" role="button" title="{{ trans('hr-manager::corp-health.access_filter_hint') }}" style="background: rgba(102,126,234,0.18); color: #c7d2fe; cursor: pointer;">{{ trans('hr-manager::corp-health.access_stat_hr_admins', ['n' => $axS['hr_admin_count']]) }}</span>
                    <span class="badge access-stat-filter" data-filter="seat_role" role="button" title="{{ trans('hr-manager::corp-health.access_filter_hint') }}" style="background: rgba(23,162,184,0.15); color: #7bd4e0; cursor: pointer;">{{ trans('hr-manager::corp-health.access_stat_seat_roles', ['n' => $axS['seat_role_count']]) }}</span>
                    @if(($axS['squad_role_count'] ?? 0) > 0)
                        <span class="badge access-stat-filter" data-filter="squad_role" role="button" title="{{ trans('hr-manager::corp-health.access_filter_hint') }}" style="background: rgba(23,162,184,0.22); color: #7bd4e0; cursor: pointer;"><i class="fas fa-users"></i> {{ trans('hr-manager::corp-health.access_stat_via_squad', ['n' => $axS['squad_role_count']]) }}</span>
                    @endif
                    @if(($axS['direct_role_count'] ?? 0) > 0)
                        <span class="badge access-stat-filter" data-filter="direct_role" role="button" title="{{ trans('hr-manager::corp-health.access_filter_hint') }}" style="background: rgba(255,255,255,0.1); color: var(--hr-text-light); cursor: pointer;">{{ trans('hr-manager::corp-health.access_stat_direct', ['n' => $axS['direct_role_count']]) }}</span>
                    @endif
                    @if($axD['available'])
                        <span class="badge access-stat-filter" data-filter="discord_linked" role="button" title="{{ trans('hr-manager::corp-health.access_filter_hint') }}" style="background: rgba(88,101,242,0.18); color: #c7d2fe; cursor: pointer;"><i class="fab fa-discord"></i> {{ trans('hr-manager::corp-health.access_stat_linked', ['n' => $axD['linked_count']]) }}</span>
                        @if($axD['configured'])
                            <span class="badge access-stat-filter" data-filter="sensitive" role="button" title="{{ trans('hr-manager::corp-health.access_filter_hint') }}" style="background: rgba(220,53,69,0.2); color: #fca5a5; cursor: pointer;"><i class="fas fa-shield-alt"></i> {{ trans('hr-manager::corp-health.access_stat_sensitive', ['n' => $axD['sensitive_count']]) }}</span>
                        @endif
                        @if(($axD['no_discord_count'] ?? 0) > 0)
                            <span class="badge access-stat-filter hr-blink" data-filter="no_discord" role="button" title="{{ trans('hr-manager::corp-health.access_filter_hint') }}" style="background: rgba(220,53,69,0.2); color: #fca5a5; border: 1px solid rgba(220,53,69,0.5); cursor: pointer;"><i class="fab fa-discord"></i> {{ trans('hr-manager::corp-health.access_stat_no_discord', ['n' => $axD['no_discord_count']]) }}</span>
                        @endif
                    @endif
                    @if($axS['unregistered_count'] > 0)
                        <span class="badge access-stat-filter hr-blink" data-filter="unregistered" role="button" title="{{ trans('hr-manager::corp-health.access_filter_hint') }}" style="background: rgba(220,53,69,0.2); color: #fca5a5; border: 1px solid rgba(220,53,69,0.5); cursor: pointer;"><i class="fas fa-user-slash"></i> {{ trans('hr-manager::corp-health.access_stat_unregistered', ['n' => $axS['unregistered_count']]) }}</span>
                    @endif
                    <span id="access-filter-clear" class="badge" role="button" title="{{ trans('hr-manager::corp-health.access_filter_clear') }}" style="display: none; background: rgba(255,255,255,0.08); color: var(--hr-text-light); cursor: pointer;"><i class="fas fa-times"></i> {{ trans('hr-manager::corp-health.access_filter_clear') }}</span>
                </div>
                <div class="d-flex flex-wrap align-items-center mb-3" style="gap: 8px; color: var(--hr-text-muted); font-size: 0.82rem;">
                    <span>{{ trans('hr-manager::corp-health.access_accounts', ['n' => $axS['account_count']]) }}</span>
                    <span style="opacity: 0.6;">&middot; {{ trans('hr-manager::corp-health.access_chars_registered', ['reg' => $axS['registered_char_count'], 'total' => $axS['member_total']]) }}</span>
                </div>

                @unless($axD['available'])
                    <div class="alert py-2 mb-2" style="background: rgba(88,101,242,0.08); border: 1px solid rgba(88,101,242,0.3); color: var(--hr-text-light); font-size: 0.82rem;">
                        <i class="fas fa-info-circle"></i> {{ trans('hr-manager::corp-health.access_discord_absent') }}
                    </div>
                @endunless
                @if($axD['available'] && !$axD['configured'])
                    <div class="alert py-2 mb-2" style="background: rgba(255,193,7,0.08); border: 1px solid rgba(255,193,7,0.3); color: var(--hr-text-light); font-size: 0.8rem;">
                        <i class="fas fa-info-circle"></i> {{ trans('hr-manager::corp-health.access_discord_unconfigured') }}
                    </div>
                @endif

                @include('hr-manager::corp-health.partials._list_pager_controls', ['prefix' => 'access'])

                {{-- Legend: how SeAT roles were granted. --}}
                <div class="d-flex flex-wrap align-items-center mb-2" style="gap: 10px; font-size: 0.78rem; padding: 6px 10px; border-radius: 6px; background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.06);">
                    <span style="text-transform: uppercase; letter-spacing: 0.4px; color: var(--hr-text-muted);">{{ trans('hr-manager::corp-health.access_role_source') }}</span>
                    <span class="badge" style="background: rgba(23,162,184,0.15); color: #7bd4e0;"><i class="fas fa-users"></i> {{ trans('hr-manager::corp-health.access_legend_squad') }}</span>
                    <span class="badge" style="background: rgba(255,255,255,0.07); color: var(--hr-text-light);">{{ trans('hr-manager::corp-health.access_legend_direct') }}</span>
                </div>

                {{-- Column headers for the unified rows: Member | SeAT | Discord --}}
                <div class="d-none d-md-flex" style="gap: 8px; padding: 0 8px 4px; border-bottom: 1px solid rgba(255,255,255,0.08); margin-bottom: 6px;">
                    <div style="flex: 0 0 210px; color: var(--hr-text-muted); text-transform: uppercase; letter-spacing: 0.4px; font-size: 0.7rem;">{{ trans('hr-manager::corp-health.access_col_member') }}</div>
                    <div style="flex: 1 1 0; color: #f59e0b; text-transform: uppercase; letter-spacing: 0.4px; font-size: 0.7rem;"><i class="fas fa-user-shield"></i> {{ trans('hr-manager::corp-health.access_seat_heading') }}</div>
                    <div style="flex: 1 1 0; color: #8b9cf5; text-transform: uppercase; letter-spacing: 0.4px; font-size: 0.7rem;"><i class="fab fa-discord"></i> {{ trans('hr-manager::corp-health.access_discord_heading') }}</div>
                </div>

                {{-- One row per registered account: SeAT access ↔ Discord access. --}}
                @forelse($ax['people'] as $p)
                    @php
                        $pFlags = [];
                        if ($p['unregistered']) {
                            $pFlags[] = 'unregistered';
                            ...
                            if ($p['superuser'])                { $pFlags[] = 'superuser'; }
                            if ($p['ingame_director'])          { $pFlags[] = 'ingame_director'; }
                            if ($p['hr_director'])              { $pFlags[] = 'hr_director'; }
                            if ($p['hr_admin'])                 { $pFlags[] = 'hr_admin'; }
                            if (!empty($p['seat_roles'])) {
                                $pFlags[] = 'seat_role';
                                $rSquad = false; $rDirect = false;
                                foreach ($p['seat_roles'] as $sr) {
                                    if (!empty($sr['via_squad'])) { $rSquad = true; } else { $rDirect = true; }
                                }
                                if ($rSquad)  { $pFlags[] = 'squad_role'; }
                                if ($rDirect) { $pFlags[] = 'direct_role'; }
                            }
                            if ($p['discord_linked'])           { $pFlags[] = 'discord_linked'; }
                            if ($p['discord_sensitive_count'] > 0) { $pFlags[] = 'sensitive'; }
                            if ($axD['available'] && !$p['discord_linked']) { $pFlags[] = 'no_discord'; }
                        }
                        if ($p['is_risk'])                  { $pFlags[] = 'risk'; }
                    @endphp
                    <div class="access-person-row d-flex flex-column flex-md-row mb-1" data-access="{{ implode(' ', $pFlags) }}" style="gap: 8px; padding: 8px; border-radius: 6px; background: rgba(255,255,255,0.03); {{ $p['is_risk'] ? 'border-left: 3px solid #dc3545;' : '' }}">
                        {{-- Identity + activity tier --}}
                        <div class="d-flex align-items-center" style="gap: 8px; flex: 0 0 210px; min-width: 0;">
                            @if($p['main_character_id'] > 0)
                                <img src="https://images.evetech.net/characters/{{ $p['main_character_id'] }}/portrait?size=32" style="width:26px;height:26px;border-radius:50%; flex-shrink:0;" alt="">
                            @endif
                            <div style="min-width: 0;">
                                @if($p['unregistered'])
                                    <span style="color: var(--hr-text-light); font-weight: 600; display:block; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">{{ $p['main_name'] }}</span>
                                @else
                                    <a href="{{ route('hr-manager.players.show', ['id' => $p['user_id'], 'corporation_id' => $corporationId]) }}" style="color: var(--hr-text-light); font-weight: 600; display:block; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">{{ $p['main_name'] }}</a>
                                @endif
                                <div style="display:flex; gap:4px; flex-wrap:wrap; align-items:center;">
                                    @if($p['unregistered'])
                                        {{-- no tier / no account --}}
                                    @elseif($p['tier_level'] !== null)
                                        <span class="badge badge-hr {{ \HrManager\Support\TierLevel::badgeClass($p['tier_level']) }}" style="font-size:0.62rem;" title="{{ \HrManager\Support\TierLevel::label($p['tier_level']) }}">
                                            {{ \HrManager\Support\TierLevel::shortLabel($p['tier_level']) }}@if($p['tier_threshold']) · {{ $p['tier_threshold'] }}d @endif
                                        </span>
                                    @else
                                        <span class="badge" style="background: rgba(255,255,255,0.05); color: var(--hr-text-muted); font-size:0.62rem;">{{ trans('hr-manager::tiers.unmapped') }}</span>
                                    @endif
                                    @if($p['is_risk'])
                                        <span class="badge" style="background: rgba(220,53,69,0.18); color:#fca5a5; font-size:0.62rem;">{{ trans('hr-manager::corp-health.cat_' . $p['category']) }}</span>
                                    @endif
                                </div>
                            </div>
                        </div>
                        {{-- SeAT access --}}
                        <div style="flex: 1 1 0; min-width: 0; display:flex; flex-wrap:wrap; gap:4px; align-content:flex-start;">
                            @if($p['unregistered'])
                                <span class="badge hr-blink" style="background: rgba(220,53,69,0.2); color: #fca5a5; border: 1px solid rgba(220,53,69,0.5);"><i class="fas fa-user-slash"></i> {{ trans('hr-manager::corp-health.access_no_seat_badge') }}</span>
                            @else
                            @if($p['superuser'])<span class="badge" style="background: rgba(220,53,69,0.9); color:#fff;"><i class="fas fa-crown"></i> {{ trans('hr-manager::corp-health.access_tag_superuser') }}</span>@endif
                            @if($p['ingame_director'])<span class="badge" style="background: rgba(245,158,11,0.2); color:#fcd34d;" title="{{ implode(', ', $p['director_chars']) }}"><i class="fas fa-star"></i> {{ trans('hr-manager::corp-health.access_tag_director') }}</span>@endif
                            @if($p['hr_director'])<span class="badge" style="background: rgba(102,126,234,0.18); color:#c7d2fe;">{{ trans('hr-manager::corp-health.access_tag_hr_director') }}</span>@endif
                            @if($p['hr_admin'])<span class="badge" style="background: rgba(102,126,234,0.18); color:#c7d2fe;">{{ trans('hr-manager::corp-health.access_tag_hr_admin') }}</span>@endif
                            @foreach($p['seat_roles'] as $role)
                                @if($role['via_squad'])
                                    <span class="badge" style="background: rgba(23,162,184,0.15); color: #7bd4e0;" title="{{ trans('hr-manager::corp-health.access_role_via_squad', ['squad' => $role['via_squad']]) }}"><i class="fas fa-users"></i> {{ $role['name'] }}</span>
                                @else
                                    <span class="badge" style="background: rgba(255,255,255,0.07); color: var(--hr-text-light);" title="{{ trans('hr-manager::corp-health.access_role_direct') }}">{{ $role['name'] }}</span>
                                @endif
                            @endforeach
                            @unless($p['has_seat_access'])<span style="color: var(--hr-text-muted);">&mdash;</span>@endunless
                            @endif
                        </div>
                        {{-- Discord access --}}
                        <div style="flex: 1 1 0; min-width: 0; display:flex; flex-wrap:wrap; gap:4px; align-content:flex-start;">
                            ...
                            @elseif(!$p['discord_linked'])
                                <span class="badge hr-blink" style="background: rgba(220,53,69,0.2); color: #fca5a5; border: 1px solid rgba(220,53,69,0.5);"><i class="fab fa-discord"></i> {{ trans('hr-manager::corp-health.access_discord_no_link_badge') }}</span>
                            @elseif(empty($p['discord_roles']))
                                <span style="color: var(--hr-text-muted);">&mdash;</span>
                            @else
                                @foreach($p['discord_roles'] as $role)
                                    @php
                                        $dsrc = $role['source'] ?? null;
                                        $dtip = $dsrc ? $discordSourceTip($dsrc) : '';
                                        $dviaSquad = $dsrc && ($dsrc['type'] ?? '') === 'squad';
                                    @endphp
                                    <span class="badge" style="background: {{ $role['sensitive'] ? 'rgba(220,53,69,0.18)' : 'rgba(255,255,255,0.07)' }}; color: {{ $role['sensitive'] ? '#fca5a5' : 'var(--hr-text-muted)' }};" title="{{ $role['name'] }}{{ $dtip ? ' — ' . $dtip : '' }}">
                                        @if($role['sensitive'])<i class="fas fa-shield-alt"></i> @endif
                                        @if($dviaSquad)<i class="fas fa-users" style="opacity:0.7;"></i> @endif
                                        {{ $role['name'] }}
                                    </span>
                                @endforeach
                            @endif
                ...
                @empty
                    <p class="text-muted mb-0" style="font-size: 0.85rem;">{{ trans('hr-manager::corp-health.access_seat_none') }}</p>
                @endforelse
                <div id="access-filter-empty" style="display: none; color: var(--hr-text-muted); font-size: 0.85rem; padding: 8px;">
                    <i class="fas fa-filter"></i> {{ trans('hr-manager::corp-health.access_filter_none') }}
                </div>
                <nav id="access-pager" class="d-flex flex-wrap justify-content-center mt-2" style="gap: 4px; display: none !important;"></nav>

                <small class="d-block mt-2" style="color: var(--hr-text-muted); font-size: 0.75rem;"><i class="fas fa-info-circle"></i> {{ trans('hr-manager::corp-health.access_footnote') }}</small>

                <script>
                hrPaginate({
                    rows: '.access-person-row', badges: '.access-stat-filter', attr: 'data-access',
                    sizeSel: 'access-size', pager: 'access-pager', clear: 'access-filter-clear',
                    empty: 'access-filter-empty', count: 'access-count', activeClass: 'access-filter-on'
                });
                </script>
            </div>
        </div>
