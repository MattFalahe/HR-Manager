...
@push('head')
<link rel="stylesheet" href="{{ asset('vendor/hr-manager/css/hr-manager.css') }}?v=1.0.10">
@endpush
...
<div class="hr-manager-wrapper">
                    ...
                    <div style="display: flex; align-items: center; gap: 12px; flex-wrap: wrap;">
                        ...
                        @if($tier)
                            <span class="badge badge-hr {{ \HrManager\Support\TierLevel::badgeClass($tier['level']) }}"
                                  @if(!empty($tier['threshold_days'])) title="{{ trans('hr-manager::players.tier_threshold_tip', ['days' => $tier['threshold_days']]) }}" @endif>
                                {{ \HrManager\Support\TierLevel::shortLabel($tier['level']) }}
                                {{ \HrManager\Support\TierLevel::label($tier['level']) }}
                                @if(!empty($tier['threshold_days']))
                                    <small style="opacity: 0.85;">· {{ $tier['threshold_days'] }}d</small>
                                @endif
                            </span>
                        @else
                        ...
                        @if($status && $status->status === 'loa')
                            <span class="badge" style="background: #e0a800; color: #1a1a1a; font-weight: 600;">
                        ...
                        @endif
                        @if(!empty($discord['available']) && !empty($discord['discord_username']))
                            <span class="badge" style="background: rgba(88,101,242,0.18); color: #c7d2fe; border: 1px solid rgba(88,101,242,0.4);" title="{{ trans('hr-manager::members.discord_user') }}">
                                <i class="fab fa-discord"></i> {{ $discord['discord_username'] }}
                            </span>
                        @elseif(!empty($connectorInstalled))
                            <span class="badge" style="background: rgba(255,255,255,0.05); color: var(--hr-text-muted);" title="{{ trans('hr-manager::players.discord_not_linked_help') }}">
                                <i class="fab fa-discord"></i> {{ trans('hr-manager::players.discord_not_linked') }}
                            </span>
                        @endif
                    </div>
    ...
    <style>
        ...
        /* Show-more collapse for long per-character / list sections (the 30-alt
           case). Items must be DIRECT children of .hr-collapse-list; collapsed =
           only the first 8 render, the toggle button reveals the rest. The "All
           Characters" grid is deliberately never collapsed. */
        .hr-collapse-list.is-collapsed > *:nth-child(n+9) { display: none; }
        .hr-collapse-toggle { background: none; border: none; color: #818cf8; font-size: 0.8rem; cursor: pointer; padding: 4px 2px; margin-top: 2px; }
        .hr-collapse-toggle:hover { color: #a5b4fc; text-decoration: underline; }
    </style>
    @endpush
    ...
    {{-- Activity breakdown — the raw numbers behind the role badges, summed
         across all alts. Sits directly under Corp impact. --}}
    @include('hr-manager::players.partials._activity_breakdown', ['activity' => $playerActivity])

    {{-- Mining engagement — ore output + moon mining + (conditional) op
         attendance, summed across alts. --}}
    @include('hr-manager::players.partials._mining_engagement', ['engagement' => $miningEngagement])

    {{-- PvP activity — the account's most active PvP characters (cache-peek,
         warms as each is opened). Quick stats + a jump into each. --}}
    @if(!empty($topPvp))
        <div class="card card-dark mb-3" style="border-left: 4px solid #dc3545;">
            <div class="card-header">
                <h3 class="card-title"><i class="fas fa-skull-crossbones" style="color: #f87171;"></i> {{ trans('hr-manager::players.pvp_heading') }}</h3>
                <div class="card-tools"><small style="color: var(--hr-text-muted);">{{ trans('hr-manager::players.pvp_subtitle') }}</small></div>
            </div>
            <div class="card-body">
                <div class="row">
                    @foreach($topPvp as $pc)
                        @php
                            $k12    = (int) ($pc['windows']['12m']['kills'] ?? 0);
                            $danger = $pc['danger_ratio'] ?? null;
                        @endphp
                        <div class="col-md-4 mb-2">
                            <div style="display:flex; align-items:center; gap:10px; background:rgba(255,255,255,0.03); border:1px solid var(--hr-border, #2c3138); border-radius:6px; padding:10px;">
                                <img src="https://images.evetech.net/characters/{{ $pc['character_id'] }}/portrait?size=64" style="width:44px; height:44px; border-radius:50%;" alt="" onerror="this.style.display='none'">
                                <div style="flex:1; min-width:0;">
                                    <div style="color:var(--hr-text-white); font-weight:600; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">{{ $pc['name'] }}</div>
                                    <div style="font-size:0.78rem; color:var(--hr-text-muted);">
                                        <span style="color:#f87171;">{{ number_format((int) $pc['all_time_kills']) }} {{ trans('hr-manager::players.pvp_kills') }}</span>
                                        · {{ number_format((int) $pc['all_time_losses']) }} {{ trans('hr-manager::players.pvp_losses') }}@if($danger !== null) · {{ $danger }}% {{ trans('hr-manager::players.pvp_danger') }}@endif
                                    </div>
                                    <div style="font-size:0.72rem; color:var(--hr-text-muted);">{{ number_format($k12) }} {{ trans('hr-manager::players.pvp_12mo') }}</div>
                                </div>
                                <a href="{{ route('hr-manager.members.show', $pc['character_id']) }}" class="btn btn-sm btn-hr-secondary" title="{{ trans('hr-manager::players.pvp_view') }}"><i class="fas fa-arrow-right"></i></a>
                            </div>
                        </div>
                    @endforeach
                </div>
    ...
    {{-- In-game title/role alignment across the account's characters in this
         corp (optional feature — Settings → Features). Only renders when enabled
         and the person has 2+ characters here to compare. --}}
    @if(!empty($roleAlignment['available']))
        <div class="card card-dark mb-3" style="border-left: 4px solid {{ empty($roleAlignment['aligned']) ? 'var(--hr-warning, #ffc107)' : 'var(--hr-success, #28a745)' }};">
            <div class="card-header">
                <h3 class="card-title"><i class="fas fa-user-tag"></i> {{ trans('hr-manager::corp-health.align_panel_heading') }}</h3>
            </div>
            <div class="card-body">
                @include('hr-manager::players.partials._role_alignment', ['alignment' => $roleAlignment])
            </div>
        </div>
    @endif
                    ...
                    <div class="col-md-6">
                        ...
                        @if(!empty($adI['by_character']) && count($adI['by_character']) > 1)
                            <div class="mt-2" style="border-top: 1px solid rgba(255,255,255,0.06); padding-top: 6px;">
                                ...
                                <div class="hr-collapse-list {{ count($adI['by_character']) > 8 ? 'is-collapsed' : '' }}" id="collapse-adchars">
                                ...
                                @if(count($adI['by_character']) > 8)
                                    <button type="button" class="hr-collapse-toggle" data-target="collapse-adchars"
                                            data-more="{{ trans('hr-manager::players.show_all', ['count' => count($adI['by_character'])]) }}"
                                            data-less="{{ trans('hr-manager::players.show_less') }}">
                                        <i class="fas fa-chevron-down"></i> {{ trans('hr-manager::players.show_all', ['count' => count($adI['by_character'])]) }}
                                    </button>
                                @endif
                            </div>
                        @endif
    ...
    {{-- Discord access depth (via seat-connector): account-level Discord roles,
         split by the operator-curated "sensitive" list. Companion to the SeAT
         access depth box above. Only the connector-MANAGED roles are visible to
         SeAT — manually-assigned Discord roles and channel permissions are not,
         so this is a floor on real Discord power, never the ceiling. --}}
    @if(!empty($discord['available']) && !empty($discordAccess['roles']))
        @php $daSens = $discordAccess['sensitive_count'] > 0; @endphp
        <div class="card card-dark mb-3" style="border-left: 4px solid {{ $daSens ? '#dc3545' : '#5865f2' }};">
            <div class="card-header">
                <h3 class="card-title"><i class="fab fa-discord"></i> {{ trans('hr-manager::players.da_heading') }}</h3>
                <div class="card-tools">
                    @if(!empty($discord['discord_username']))<small style="color: var(--hr-text-muted);">{{ $discord['discord_username'] }} &middot; </small>@endif
                    <small style="color: var(--hr-text-muted);">{{ trans('hr-manager::players.da_role_count', ['count' => $discordAccess['total']]) }}</small>
                    @if($daSens)
                        <span class="badge ml-1" style="background: rgba(220,53,69,0.2); color: #fca5a5; border: 1px solid rgba(220,53,69,0.5);"><i class="fas fa-shield-alt"></i> {{ trans('hr-manager::players.da_sensitive_badge', ['count' => $discordAccess['sensitive_count']]) }}</span>
                    @endif
                </div>
            </div>
            <div class="card-body">
                @if($daSens)
                    {{-- Sensitive / leadership roles first, called out. --}}
                    <div class="mb-2">
                        <small style="color: var(--hr-text-muted); text-transform: uppercase; letter-spacing: 0.4px; font-size: 0.7rem;"><i class="fas fa-shield-alt" style="color: #fca5a5;"></i> {{ trans('hr-manager::players.da_sensitive_heading') }}</small>
                        <div style="margin-top: 4px;">
                            @foreach($discordAccess['sensitive'] as $role)
                                <span class="badge mr-1 mb-1" style="background: rgba(220,53,69,0.2); color: #fca5a5; border: 1px solid rgba(220,53,69,0.5);">
                ...
                @php $daOther = array_filter($discordAccess['roles'], fn ($r) => empty($r['sensitive'])); @endphp
                @if(!empty($daOther))
                    <div>
                        @if($daSens)<small style="color: var(--hr-text-muted); text-transform: uppercase; letter-spacing: 0.4px; font-size: 0.7rem;">{{ trans('hr-manager::players.da_other_heading') }}</small><br>@endif
                        @foreach($daOther as $role)
                ...
                @unless($discordAccess['configured'])
                    <small class="d-block mt-2" style="color: var(--hr-text-muted); font-size: 0.75rem;"><i class="fas fa-info-circle"></i> {{ trans('hr-manager::players.da_unconfigured') }}</small>
                @endunless
                <small class="d-block mt-2" style="color: var(--hr-text-muted); font-size: 0.72rem;"><i class="fas fa-info-circle"></i> {{ trans('hr-manager::players.da_footnote') }}</small>
            </div>
        </div>
    @endif
        ...
        <div class="col-md-8">
            ...
            <div class="card card-dark mb-3">
                ...
                <div class="card-body">
                    {{-- Account activity roll-up: how many of this human's
                         ...
                         (dormant = idle > 90 days or no signal). Per-character
                         only — the account's single classification is unaffected. --}}
                    @php
                        $actCount = 0; $idleCount = 0; $dormCount = 0;
                        foreach ($altSummaries as $__a) {
                            $__la = $__a['last_activity_at'] ?? null;
                            if ($__la === null) { $dormCount++; continue; }
                            $__d = (int) abs($__la->diffInDays(now()));
                            if ($__d <= 30) { $actCount++; }
                            elseif ($__d <= 90) { $idleCount++; }
                            else { $dormCount++; }
                        }
                    @endphp
                    @if(count($altSummaries) > 1)
                        <div class="d-flex flex-wrap align-items-center mb-3" style="gap: 12px; font-size: 0.82rem;">
                            <span style="color: var(--hr-text-muted); text-transform: uppercase; letter-spacing: 0.3px;">{{ trans('hr-manager::players.activity_legend') }}</span>
                            <span style="color: #3fb950;"><i class="fas fa-bolt"></i> {{ $actCount }} {{ trans('hr-manager::players.activity_active_short') }}</span>
                            @if($idleCount > 0)<span style="color: #e0a800;"><i class="fas fa-moon"></i> {{ $idleCount }} {{ trans('hr-manager::players.activity_idle_short') }}</span>@endif
                            @if($dormCount > 0)<span style="color: #f87171;"><i class="fas fa-bed"></i> {{ $dormCount }} {{ trans('hr-manager::players.activity_dormant_short') }}</span>@endif
                        </div>
                        @if($dormCount >= 2 && $actCount >= 1)
                            <div class="mb-3" style="padding: 7px 11px; border-left: 3px solid #e0a800; background: rgba(224,168,0,0.08); border-radius: 4px; font-size: 0.82rem; color: var(--hr-text-light);">
                                <i class="fas fa-info-circle" style="color: #e0a800;"></i>
                                {{ trans('hr-manager::players.dormant_alts_note', ['active' => $actCount, 'dormant' => $dormCount]) }}
                            </div>
                        @endif
                    @endif
                    <div class="row">
                            ...
                            <div class="col-md-6 col-lg-4 mb-3">
                                <div style="background: var(--hr-dark-card); border: 1px solid {{ $alt['in_corp_now'] ? 'rgba(40,167,69,0.4)' : 'var(--hr-border)' }}; border-radius: 6px; padding: 10px;">
                                    <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 6px;">
                                        ...
                                        @endif
                                        {{-- Token-status badge (tinted, so it reads as a distinct set from the
                                             solid corp badge above): active / expired / missing. --}}
                                        @php
                                            $ts = $alt['token_status'] ?? 'missing';
                                            $tsMeta = [
                                                'active'  => ['rgba(40,167,69,0.18)',   '#3fb950', trans('hr-manager::players.token_active')],
                                                'expired' => ['rgba(224,168,0,0.18)',   '#e0a800', trans('hr-manager::players.token_expired')],
                                                'missing' => ['rgba(108,117,125,0.20)', '#adb5bd', trans('hr-manager::players.token_missing')],
                                            ][$ts] ?? ['rgba(108,117,125,0.20)', '#adb5bd', trans('hr-manager::players.token_missing')];
                                        @endphp
                                        <span class="badge badge-hr" style="background: {{ $tsMeta[0] }}; color: {{ $tsMeta[1] }};" title="{{ $tsMeta[2] }}">
                                            <i class="fas fa-key"></i>
                                        </span>
                                        {{-- Per-character activity badge: is THIS character actually
                                             used? Login/wallet recency banded into active / idle /
                                             inactive so a director can spot the dormant alts an account
                                             is carrying (the classifier is per-account, so these never
                                             each count as "dead weight" — this is the per-alt view). --}}
                                        @php
                                            $altLa = $alt['last_activity_at'];
                                            $altLaDays = $altLa ? (int) abs($altLa->diffInDays(now())) : null;
                                            $actMeta = $altLaDays === null
                                                ? ['rgba(108,117,125,0.20)', '#adb5bd', 'fa-question', trans('hr-manager::players.activity_unknown')]
                                                : ($altLaDays <= 30
                                                    ? ['rgba(40,167,69,0.18)',  '#3fb950', 'fa-bolt', trans('hr-manager::players.activity_active')]
                                                    : ($altLaDays <= 90
                                                        ? ['rgba(224,168,0,0.18)', '#e0a800', 'fa-moon', trans('hr-manager::players.activity_idle')]
                                                        : ['rgba(220,53,69,0.18)', '#f87171', 'fa-bed', trans('hr-manager::players.activity_inactive')]));
                                        @endphp
                                        <span class="badge badge-hr" style="background: {{ $actMeta[0] }}; color: {{ $actMeta[1] }};" title="{{ $actMeta[3] }}{{ $altLaDays !== null ? ' · ' . $altLaDays . 'd' : '' }}">
                                            <i class="fas {{ $actMeta[2] }}"></i>
                                        </span>
                                    </div>
            ...
            @php
                ...
                $eventIcons = [
                    ...
                    // Watchlist / intel screening
                    'watchlist_application_hit'          => ['fa-user-secret',      'var(--hr-danger, #dc3545)'],
                    'watchlist_added'                    => ['fa-user-lock',        'var(--hr-warning, #ffc107)'],
                    'watchlist_cleared'                  => ['fa-user-check',       'var(--hr-info, #17a2b8)'],
                    'hr.player.blacklist_override'       => ['fa-ban',              'var(--hr-danger, #dc3545)'],
                ];
                ...
                $eventLabels = [
                    ...
                    'hr.player.blacklist_override'            => 'Accepted despite blacklist',
                    ...
                    'watchlist_application_hit'               => 'Flagged character on a new application',
                    'watchlist_added'                         => 'Added to watchlist',
                    'watchlist_cleared'                       => 'Cleared from watchlist',
                ];
            ...
            <div class="card card-dark mb-3">
                ...
                <div class="card-body">
                    <div class="hr-collapse-list {{ $history->count() > 8 ? 'is-collapsed' : '' }}" id="collapse-history">
                        ...
                        <div class="d-flex align-items-start mb-2" style="gap: 8px;">
                            ...
                            <div style="flex: 1; min-width: 0;">
                                ...
                                <small style="color: var(--hr-text-muted);">
                                    ...
                                    @if($event->character_id && isset($historySubjectNames[$event->character_id]))
                                        &middot;
                                        <img src="https://images.evetech.net/characters/{{ $event->character_id }}/portrait?size=32" style="width:14px;height:14px;border-radius:50%;vertical-align:middle;" alt="" onerror="this.style.display='none'">
                                        <span style="color: var(--hr-text-light);">{{ $historySubjectNames[$event->character_id] }}</span>
                                    @endif
                                    @if($actorName)
                    ...
                    @if($history->count() > 8)
                        <button type="button" class="hr-collapse-toggle" data-target="collapse-history"
                                data-more="{{ trans('hr-manager::players.show_all', ['count' => $history->count()]) }}"
                                data-less="{{ trans('hr-manager::players.show_less') }}">
                            <i class="fas fa-chevron-down"></i> {{ trans('hr-manager::players.show_all', ['count' => $history->count()]) }}
                        </button>
                    @endif
                </div>
            </div>
            ...
            <div class="card card-dark mb-3">
                ...
                <div class="card-body">
                    <div class="hr-collapse-list {{ $notes->count() > 8 ? 'is-collapsed' : '' }}" id="collapse-notes">
                    ...
                    @if($notes->count() > 8)
                        <button type="button" class="hr-collapse-toggle" data-target="collapse-notes"
                                data-more="{{ trans('hr-manager::players.show_all', ['count' => $notes->count()]) }}"
                                data-less="{{ trans('hr-manager::players.show_less') }}">
                            <i class="fas fa-chevron-down"></i> {{ trans('hr-manager::players.show_all', ['count' => $notes->count()]) }}
                        </button>
                    @endif
                </div>
            </div>
        ...
        <div class="col-md-4">
            ...
            <div class="card card-dark mb-3">
                ...
                <div class="card-body">
                    ...
                    {{-- Clear status. Overriding a security AUTO-purge (token
                         loss) needs a written reason — it's an accountable
                         director override of the Watchdog, noted on the player. --}}
                    @if($status && $status->status !== 'active')
                        @php $isAutoPurge = $status->status === 'marked_for_purge' && $status->purge_origin === \HrManager\Models\PlayerStatus::ORIGIN_TOKEN_LOSS; @endphp
                        <form method="POST" action="{{ route('hr-manager.players.clear-status', $user->id) }}" class="mb-2">
                            @csrf
                            <input type="hidden" name="corporation_id" value="{{ $corporationId }}">
                            @if($isAutoPurge)
                                <div class="form-group mb-1">
                                    <label class="mb-1" style="font-size: 0.8rem; color: var(--hr-text-muted);">
                                        <i class="fas fa-shield-alt text-danger"></i> {{ trans('hr-manager::players.override_reason_label') }}
                                    </label>
                                    <textarea name="cancel_reason" class="form-control form-control-sm" rows="2" required
                                              placeholder="{{ trans('hr-manager::players.override_reason_placeholder') }}">{{ old('cancel_reason') }}</textarea>
                                </div>
                            @endif
        ...
        @if(!empty($identity))
            <div class="card card-dark mb-3">
                ...
                <div class="card-body">
                    ...
                    @else
                        <div class="hr-collapse-list {{ $identity->mappings->count() > 8 ? 'is-collapsed' : '' }}" id="collapse-identity">
                        @foreach($identity->mappings as $mapping)
                            ...
                            @if(!$mapping->effective_to)
                                @push('hr-modals')
                                {{-- Reassign modal per active mapping (rendered in the modal
                                     stack so it isn't counted as a collapse-list child). --}}
                        ...
                        @if($identity->mappings->count() > 8)
                            <button type="button" class="hr-collapse-toggle" data-target="collapse-identity"
                                    data-more="{{ trans('hr-manager::players.show_all', ['count' => $identity->mappings->count()]) }}"
                                    data-less="{{ trans('hr-manager::players.show_less') }}">
                                <i class="fas fa-chevron-down"></i> {{ trans('hr-manager::players.show_all', ['count' => $identity->mappings->count()]) }}
                            </button>
                        @endif
                    @endif
...
<script>
$(function () {
    // Show-more toggles for the long player-profile lists (.hr-collapse-list).
    // Collapsed shows the first 8; the button reveals the rest and back.
    $(document).on('click', '.hr-collapse-toggle', function () {
        var $btn = $(this);
        var $list = $('#' + $btn.attr('data-target'));
        var collapsed = $list.toggleClass('is-collapsed').hasClass('is-collapsed');
        $btn.html('<i class="fas fa-chevron-' + (collapsed ? 'down' : 'up') + '"></i> '
            + (collapsed ? $btn.attr('data-more') : $btn.attr('data-less')));
    });
});
</script>
@endpush
