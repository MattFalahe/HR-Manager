...
<div class="hr-manager-wrapper">
            ...
            <div style="display: flex; align-items: flex-start; gap: 18px;">
                ...
                <div style="flex: 1;">
                    ...
                    <small style="opacity: 0.85;">
                        ...
                        <strong>@if($watchlistMatch->is_system_added)<i class="fas fa-robot"></i> @endif{{ $watchlistMatch->added_by_label }}</strong>
                    ...
                    <div class="mt-2">
                        @foreach($intelNotes->take(3) as $note)
                            <div class="mb-2" style="padding: 8px 12px; background: rgba(0,0,0,0.25); border-left: 3px solid var(--hr-primary-start); border-radius: 4px;">
                                <div class="mb-1">
                                    @php $noteCharName = $intelCharNames[$note->character_id] ?? ('Character #' . $note->character_id); @endphp
                                    <strong style="color: var(--hr-text-white); font-size: 0.86rem;">{{ $noteCharName }}</strong>
                                    @if((int) $note->character_id !== (int) $application->character_id)
                                        <span class="badge badge-warning ml-1" style="font-size: 0.62rem;"><i class="fas fa-link"></i> {{ trans('hr-manager::watchlist.app_match_alt') }}</span>
                                    @endif
                                    @foreach(($note->tags ?? []) as $tag)
                                        <span class="badge mr-1 ml-1" style="background: rgba(255,255,255,0.05); color: var(--hr-text-light); border: 1px solid var(--hr-border); font-size: 0.7rem;">{{ $tag }}</span>
                                    @endforeach
                                </div>
    ...
    <div class="row">
        ...
        <div class="col-md-8">
            ...
            {{-- Director convenience: SeAT native-page deep-links for the applied
                 character. No grant needed — directors already hold the SeAT
                 permissions. Recruiters get the grant-based Recruiter Access panel
                 instead (when that feature is enabled). Relabelled + signed with the
                 character so it reads distinctly from the temporary recruiter panel. --}}
            @can('hr-manager.director')
                @include('hr-manager::members.partials._seat_links', [
                    'characterId' => (int) $application->character_id,
                    'characters'  => $directorLinkCharacters ?? [],
                    'heading'     => trans('hr-manager::applications.director_deeplink_heading'),
                    'note'        => trans('hr-manager::applications.director_deeplink_footnote'),
                ])
            @endcan

            {{-- zKillboard PvP activity — rolling 1/3/6/12-month kill windows per
                 account character, most-active flagged. Lazy-loaded on click so it
                 only fetches zKill on demand (one request per character). --}}
            <div class="card card-dark mb-3">
                <div class="card-header">
                    <h3 class="card-title"><i class="fas fa-crosshairs"></i> {{ trans('hr-manager::applications.pvp_panel_heading') }}</h3>
                </div>
                <div class="card-body">
                    <div id="pvpBreakdownTarget">
                        <button type="button" id="pvpLoadBtn" class="btn btn-sm btn-hr-secondary btn-icon"
                                data-url="{{ route('hr-manager.applications.pvp', $application->id, false) }}">
                            <i class="fas fa-crosshairs"></i> {{ trans('hr-manager::applications.pvp_load_btn') }}
                        </button>
                        <small class="d-block mt-2" style="color: var(--hr-text-muted);">{{ trans('hr-manager::applications.pvp_load_hint') }}</small>
                    </div>
                </div>
            </div>
            <script>
            (function () {
                var btn = document.getElementById('pvpLoadBtn');
                if (!btn) return;
                btn.addEventListener('click', function () {
                    var target = document.getElementById('pvpBreakdownTarget');
                    var url = btn.getAttribute('data-url');
                    btn.disabled = true;
                    btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> ' + @js(trans('hr-manager::applications.pvp_loading'));
                    fetch(url, { credentials: 'same-origin', headers: { 'X-Requested-With': 'XMLHttpRequest', 'Accept': 'text/html' } })
                        ...
                        .catch(function () {
                            btn.disabled = false;
                            btn.innerHTML = '<i class="fas fa-crosshairs"></i> ' + @js(trans('hr-manager::applications.pvp_load_btn'));
                            target.insertAdjacentHTML('beforeend', '<div class="text-danger mt-2"><small>' + @js(trans('hr-manager::applications.pvp_error')) + '</small></div>');
                        });
                });
            })();
            </script>
        ...
        <div class="col-md-4">
            ...
            @if(!$application->isTerminal())
                <div class="card card-dark mb-3">
                    ...
                    <div class="card-body">
                        <form method="POST" action="{{ route('hr-manager.applications.status', $application->id) }}" id="acceptGuardStatusForm">
                            @csrf
                            ...
                            <div class="form-group" id="statusCommentGroup">
                                <label>{{ trans('hr-manager::applications.comment') }}</label>
                                <textarea name="comment" class="form-control" rows="2" maxlength="1000"></textarea>
                            </div>
                            @if($blacklistBlock)
                                {{-- Acceptance guard: shown (via JS) only when "Accepted" is picked.
                                     A HIGH blacklist match must be cleared, or the director overrides
                                     here with a written justification (logged + notified). --}}
                                <div id="acceptGuardBlock" style="display:none; margin-bottom:1rem; padding:10px 12px; border-radius:6px; background:rgba(220,53,69,0.12); border:1px solid rgba(220,53,69,0.45);">
                                    <div style="color:#ff9aa5; font-weight:600; margin-bottom:6px;">
                                        <i class="fas fa-ban"></i> {{ trans('hr-manager::applications.accept_guard_heading') }}
                                    </div>
                                    <p style="color:var(--hr-text-light); font-size:0.85rem; margin-bottom:8px;">
                                        {{ trans('hr-manager::applications.accept_guard_body', ['name' => $blacklistBlock->character_name ?: ('Character #' . $blacklistBlock->character_id)]) }}
                                    </p>
                                    <div class="form-check mb-2">
                                        <input type="checkbox" name="blacklist_override" value="1" class="form-check-input" id="blacklistOverrideChk">
                                        <label class="form-check-label" for="blacklistOverrideChk" style="color:var(--hr-text-light);">
                                            {{ trans('hr-manager::applications.accept_override_label') }}
                                        </label>
                                    </div>
                                    <textarea name="blacklist_override_reason" id="blacklistOverrideReason" class="form-control" rows="2" maxlength="1000"
                                              placeholder="{{ trans('hr-manager::applications.accept_override_placeholder') }}"></textarea>
                                    <small class="d-block mt-1" style="color:var(--hr-text-muted);">{{ trans('hr-manager::applications.accept_override_hint') }}</small>
                                </div>
                            @endif
                        ...
                        @if($blacklistBlock)
                            <script>
                            (function () {
                                var form = document.getElementById('acceptGuardStatusForm');
                                var block = document.getElementById('acceptGuardBlock');
                                if (!form || !block) return;
                                var sel = form.querySelector('select[name="status"]');
                                var commentGroup = document.getElementById('statusCommentGroup');
                                var sync = function () {
                                    var on = sel && sel.value === 'accepted';
                                    block.style.display = on ? 'block' : 'none';
                                    // The override justification replaces the normal comment when a
                                    // blacklisted applicant is being accepted, so hide the comment box.
                                    if (commentGroup) commentGroup.style.display = on ? 'none' : 'block';
                                };
            ...
            {{-- Undo last status change (director-only correction). Sits OUTSIDE
                 the Change Status card above because that one hides once the
                 application is terminal — and a mistaken reject / accept (a
                 terminal state) is the main thing to undo. Voids the last
                 transition, restores the prior status, hides it from the
                 applicant's tracking page, keeps it internally. --}}
            @can('hr-manager.director')
                @if($revertTarget)
                    <div class="card card-dark mb-3">
                        <div class="card-header">
                            <h3 class="card-title">
                                <i class="fas fa-undo"></i> {{ trans('hr-manager::applications.revert_heading') }}
                            </h3>
                        </div>
                        <div class="card-body">
                            <p style="color: var(--hr-text-muted); font-size: 0.9rem;">
                                {!! trans('hr-manager::applications.revert_intro', [
                                    'voided'   => trans('hr-manager::applications.status_' . $revertTarget->new_status),
                                    'restored' => trans('hr-manager::applications.status_' . $revertTarget->old_status),
                                ]) !!}
                            </p>
                            @if($revertTarget->new_status === 'accepted')
                                <div style="margin-bottom:1rem; padding:10px 12px; border-radius:6px; background:rgba(253,126,20,0.12); border:1px solid rgba(253,126,20,0.45);">
                                    <span style="color:#ffbf8a; font-weight:600;"><i class="fas fa-exclamation-triangle"></i> {{ trans('hr-manager::applications.revert_accepted_warning') }}</span>
                                </div>
                            @endif
                            <form method="POST" action="{{ route('hr-manager.applications.revert-status', $application->id) }}"
                                  onsubmit="return confirm(@js(trans('hr-manager::applications.revert_confirm')))">
                                @csrf
                                <div class="form-group">
                                    <label>{{ trans('hr-manager::applications.revert_reason_label') }}</label>
                                    <textarea name="reason" class="form-control" rows="2" maxlength="1000" required
                                              placeholder="{{ trans('hr-manager::applications.revert_reason_placeholder') }}"></textarea>
                                </div>
                                <button type="submit" class="btn btn-hr-secondary btn-block btn-icon">
                                    <i class="fas fa-undo"></i>
                                    {{ trans('hr-manager::applications.revert_button', [
                                        'voided'   => trans('hr-manager::applications.status_' . $revertTarget->new_status),
                                        'restored' => trans('hr-manager::applications.status_' . $revertTarget->old_status),
                                    ]) }}
                                </button>
