                    ...
                    @if(!empty($traceResult['timeline']))
                        ...
                        <div class="card card-dark mb-0">
                            @foreach($traceResult['timeline'] as $ev)
                                <div class="diag-row" style="align-items:flex-start;{{ !empty($ev['voided']) ? ' opacity:0.75;' : '' }}">
                                    ...
                                    <span style="flex:1; font-size:0.85rem;">
                                        <span style="color:#e6e9ef; font-weight:500;{{ !empty($ev['voided']) ? ' text-decoration:line-through;' : '' }}">{{ $ev['title'] }}</span>
                                        ...
                                        @if(!empty($ev['voided']))
                                            {{-- Undone by a director as a mistake: still shown here (the trace
                                                 shows the truth) but flagged, and hidden from the applicant. --}}
                                            <div style="color:#ffc107; font-size:0.8rem; margin-top:3px;">
                                                <i class="fas fa-undo"></i> Reverted as a mistake by <strong>{{ $ev['void_by'] }}</strong>
                                                @if(!empty($ev['void_admin']))<span class="badge ml-1" style="background:#ffc107; color:#1a1a1a; font-weight:600;" title="SeAT superuser (global admin)">ADMIN</span>@endif
                                                <span style="color:#7f8694;">&middot; {{ $ev['void_at'] }}</span>
                                                @if(!empty($ev['void_reason']))<div style="color:#9aa1ad; font-style:italic;">"{{ $ev['void_reason'] }}"</div>@endif
                                            </div>
                                        @endif
                                    </span>
