...
<div class="hr-manager-wrapper">
    ...
    <div class="row">
        ...
        <div class="col-md-8">
            ...
            {{-- SeAT native-page deep links (director convenience). Registered
                 characters only — SeAT has no character pages for a character it
                 never synced. No grant is attached; directors already hold the
                 SeAT permissions. --}}
            @if($isRegistered)
                @include('hr-manager::members.partials._seat_links', ['characterId' => $affiliation->character_id])
            @endif

            {{-- Activity breakdown — the raw numbers behind this character's role
                 badges (this character only). Shares the player-view partial. --}}
            @include('hr-manager::players.partials._activity_breakdown', ['activity' => $characterActivity])
            ...
            @if($assessment)
                <div class="card card-dark mb-3">
                    ...
                    <div class="card-body">
                        @php
                            // Short ISK (M / B / T) so the tiles don't show a 10-digit
                            // number; the exact figure is on the hover title.
                            $iskShort = function ($v) {
                                $a = abs((float) $v);
                        ...
                        <div class="stats-grid">
                            <div class="stat-item">
                                <div class="stat-value" title="{{ number_format($assessment->total_mining_value, 0) }} ISK">{{ $iskShort($assessment->total_mining_value) }}</div>
                                <div class="stat-label">{{ trans('hr-manager::members.total_mining_value') }} (ISK)</div>
                            </div>
                            <div class="stat-item">
                                <div class="stat-value" title="{{ number_format($assessment->total_mining_tax, 0) }} ISK">{{ $iskShort($assessment->total_mining_tax) }}</div>
                            ...
                            <div class="stat-item">
                                <div class="stat-value" title="{{ number_format($assessment->total_ratting_income, 0) }} ISK">{{ $iskShort($assessment->total_ratting_income) }}</div>
        ...
        <div class="col-md-4">
            ...
            <div class="card card-dark mb-3">
                ...
                <div class="card-body">
                    <dl class="row mb-0">
                        ...
                        @if($lastLogin)
                            <dt class="col-sm-6" style="color: var(--hr-text-muted);"><i class="fas fa-clock"></i> {{ trans('hr-manager::members.last_online') }}</dt>
                            <dd class="col-sm-6">@hrDateShort($lastLogin)</dd>
                        @endif

                        @if(!empty($discord['available']) && $discord['discord_username'])
                        ...
                        @elseif(!empty($discord['available']) && $discord['connector_id'])
                        ...
                        @elseif(!empty($connectorInstalled))
                            <dt class="col-sm-6" style="color: var(--hr-text-muted);"><i class="fab fa-discord"></i> {{ trans('hr-manager::members.discord_user') }}</dt>
                            <dd class="col-sm-6" style="color: var(--hr-text-muted);" title="{{ trans('hr-manager::players.discord_not_linked_help') }}">{{ trans('hr-manager::players.discord_not_linked') }}</dd>
                        @endif
            ...
            {{-- Mining engagement — ore output + moon mining + (conditional) op
                 attendance for this character. Shared with the player view. --}}
