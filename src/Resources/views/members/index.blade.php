...
<div class="hr-manager-wrapper">
    ...
    @if(!empty($rosterStatus) && $rosterStatus['needs_director_token'])
        @if(!empty($rosterStatus['evewho_active']))
            {{-- Roster back-filled from EveWho (opt-in). Softer, purple info box
                 so it reads as "we filled the gap from a third party" rather
                 than the plain "you're missing a token" warning. --}}
            <div class="warning-box mb-3" style="border-left-color: #8b5cf6; background: rgba(139,92,246,0.08);">
                <i class="fas fa-satellite-dish" style="color: #8b5cf6;"></i>
                <div>
                    <strong>{{ trans('hr-manager::members.roster_evewho_heading') }}</strong>
                    <p class="mb-0">{{ trans('hr-manager::members.roster_evewho_body', ['count' => $rosterStatus['evewho_count'], 'when' => $rosterStatus['evewho_fetched_at'] ?? '—']) }}</p>
                </div>
            </div>
        @else
                            ...
                            @foreach($members as $member)
                                <tr>
                                    <td>
                                        ...
                                        @if(!empty($member->from_evewho))
                                            <span class="badge ml-2" style="background: rgba(139,92,246,0.18); color: #c4b5fd; border: 1px solid rgba(139,92,246,0.4);" title="{{ trans('hr-manager::members.source_evewho_help') }}">
                                                <i class="fas fa-satellite-dish"></i> {{ trans('hr-manager::members.source_evewho') }}
                                            </span>
                                        @endif
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        ...
                                        @else
                                            <span style="color: var(--hr-text-muted); font-size: 0.8rem;" title="{{ trans('hr-manager::members.no_seat_page_help') }}">
                                                <i class="fas fa-ban"></i> {{ trans('hr-manager::members.no_seat_page') }}
                                            </span>
                                        @endif
                                    </td>
                                </tr>
