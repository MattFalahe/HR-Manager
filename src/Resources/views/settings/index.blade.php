                ...
                <div class="tab-pane" id="webhooks">
                            ...
                            <table class="table table-hover mb-0">
                                ...
                                <tbody>
                                    @foreach($webhooks as $webhook)
                                        @php
                                            $whFlagKeys = ['notify_application_submitted','notify_application_accepted','notify_application_rejected','notify_status_change','notify_inactive_director','notify_dead_weight','notify_purge_reminder','notify_player_status','notify_token_revoked','notify_token_coverage','notify_member_joined','notify_member_left','notify_join_no_application','notify_member_unregistered','notify_wallet_stalled','notify_wallet_compliance_dropped','notify_wallet_milestone'];
                                        ...
                                        <tr>
                                            ...
                                            <td style="max-width: 320px;">
                                                @forelse($whOn as $k)
                                                    <span class="badge badge-secondary mb-1" style="font-weight: normal;">@if(in_array($k, $fastPollKeys, true))<i class="fas fa-bolt" style="opacity: 0.7;" title="{{ $fastPollActive ? trans('hr-manager::settings.fast_poll_badge_active') : trans('hr-manager::settings.fast_poll_badge_inactive') }}"></i> @endif{{ trans('hr-manager::settings.' . $k) }}</span>
                                                @empty
                    ...
                    <form method="POST" action="{{ route('hr-manager.settings.webhooks.store') }}">
                        ...
                        {{-- Shared field partial — SAME source as the inline edit
                             ...
                             Add and Edit again. No $webhook = add-mode defaults. --}}
                        @include('hr-manager::settings.partials._webhook_form_fields')
