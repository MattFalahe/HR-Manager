...
class WatchlistEntry extends Model
{
    ...
    /**
     * True when HR itself created this entry (the applicant-screening
     * auto-flag passes added_by = 0), rather than a person.
     */
    public function getIsSystemAddedAttribute(): bool
    {
        return (int) $this->added_by === 0;
    }

    /**
     * Who added this entry, for display. A system-generated entry
     * (added_by = 0) shows a named HR actor instead of the meaningless
     * "User #0"; a person shows their character name (or a numeric
     * fallback if the user record has since gone).
     */
    public function getAddedByLabelAttribute(): string
    {
        if ($this->is_system_added) {
            return trans('hr-manager::watchlist.system_actor');
        }

        return $this->addedByUser->name ?? ('User #' . $this->added_by);
    }
}
