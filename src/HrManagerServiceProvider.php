...
class HrManagerServiceProvider extends AbstractSeatPlugin
{
    public function boot()
    {
        ...
        // Route-middleware alias for the audit view-logger. Attached to the
        // `/hr-manager` route group (see routes.php) so every internal GET is
        // recorded when the opt-in audit log is enabled; a no-op otherwise.
        try {
            $this->app['router']->aliasMiddleware(
                'hr-audit',
                \HrManager\Http\Middleware\AuditViewLogger::class
            );
        } catch (\Throwable $e) {
            Log::warning('[HR Manager] could not register hr-audit middleware: ' . $e->getMessage());
        }
    ...
    private function registerPluginBridgeCapabilities()
    {
        ...
        try {
            ...
            // Manager Core ESI fast-poll: register the membership notification
            // handler so corp joins (CorpAppAcceptMsg) + voluntary leaves
            // (CharLeftCorpMsg) are picked up ~2 min from the director feed MC
            // polls — far faster than the 30-min roster-diff. Kicks send no EVE
            // notification, so the roster-diff stays the backstop for those, and
            // it dedups the fast path via recentlyHandled(). Resolve the registry
            // via the class constant (Laravel 11 binding), as Structure Manager
            // does for its structure handler.
            if (class_exists('ManagerCore\\Services\\ESI\\EsiNotificationRegistry')) {
                try {
                    app(\ManagerCore\Services\ESI\EsiNotificationRegistry::class)->register(
                        \HrManager\Services\MembershipNotificationHandler::registeredTypes(),
                        \HrManager\Services\MembershipNotificationHandler::class,
                        'hr-manager'
                    );
                } catch (\Throwable $e) {
                    Log::warning('[HR Manager] Could not register membership notification handler with Manager Core: ' . $e->getMessage());
                }
            }
        } catch (\Exception $e) {
