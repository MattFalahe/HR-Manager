...
class DiagnoseCommand extends Command
{
     ...
     * blueprint / structure panels. Mirror the web DiagnosticController list.
     */
    private const CONSUMED_CAPABILITIES = [
        ...
        // --- Blueprint Manager (blueprint panels + engagement signal) ---
        'blueprint-manager:blueprint.getCharacterStats',
        'blueprint-manager:blueprint.getCorpSummary',
        // --- Structure Manager (Corp Health Structure Compliance tab) ---
    ...
    private const OWNED_SUBSCRIPTIONS = [
        ...
        // Blueprint Manager request lifecycle (timeline + engagement)
        'blueprint.request.*',
        // Structure Manager alerts (Structure Health incident counting)
        'structure.alert.*',
        // Buyback Manager offers + completed contracts (contribution)
    ...
    private function checkPluginBridge(): void
    {
        ...
        // Every sibling plugin HR takes data from (events and/or capabilities).
        // Detect by installed composer package — reliable for event-only
        // publishers (Buyback, Broadcast) that don't register a bridge
        // capability, and matches the web diagnostic's detection.
        $siblings = [
            'mattfalahe/mining-manager'      => 'Mining Manager',
            'mattfalahe/corp-wallet-manager' => 'Corp Wallet Manager',
            'mattfalahe/blueprint-manager'   => 'Blueprint Manager',
            'mattfalahe/structure-manager'   => 'Structure Manager',
            'mattfalahe/buyback-manager'     => 'Buyback Manager',
            'mattfalahe/seat-discord-pings'  => 'SeAT Broadcast',
        ];
        $hasApi = class_exists('\\Composer\\InstalledVersions');
        foreach ($siblings as $pkg => $label) {
            $installed = false;
            if ($hasApi) {
                try {
                    $installed = \Composer\InstalledVersions::isInstalled($pkg);
                } catch (\Throwable $e) {
                    $installed = false;
                }
            }
            $this->line("  {$label}: " . ($installed ? '<fg=green>Detected</>' : '<fg=yellow>Not found</>'));
        }
        // SeAT Connector isn't a suite plugin — detect via its table.
    ...
    {
        $this->info('Consumed Capabilities (Wallet / Blueprint / Structure via PluginBridge):');
        if (!class_exists('ManagerCore\Services\PluginBridge')) {
