...
class BackfillBuybackCommand extends Command
{
    ...
    /**
     * Contract states Buyback Manager treats as completed. BB's own status enum
     * is `completed` (see BuybackContract::completed scope); the ESI-style
     * `finished*` values are kept as a defensive superset in case a BB variant
     * stores the raw ESI contract status. Matching neither is harmless (seeds 0).
     */
    private const COMPLETED_STATES = ['completed', 'finished', 'finished_issuer', 'finished_contractor'];
    ...
    private function backfillCompletions(bool $dry): int
    {
        ...
        // An older / different Buyback Manager schema may not carry the status
        // column this filter needs. Skip cleanly instead of erroring the run —
        // live completions still flow via the buyback.contract.completed event.
        if (!Schema::hasColumn('buyback_contracts', 'status')) {
            $this->warn('buyback_contracts has no status column (incompatible Buyback Manager version) — skipping completion backfill.');
            return 0;
        }
        ...
        try {
            ...
            $this->warn('Completion backfill skipped — Buyback Manager schema not readable: ' . $e->getMessage());
        }
