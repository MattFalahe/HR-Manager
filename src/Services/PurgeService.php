...
class PurgeService
{
    ...
    public function removeSquadsForPurge(PlayerStatus $status, string $via = 'purge_manual'): array
    {
        ...
        // purge_auto and token_loss are automated (cron / detection pass), so
        // force a NULL actor; purge_manual is the director who clicked (auth is
        // captured, but pass it explicitly for clarity).
        $actorUserId = in_array($via, ['purge_auto', 'token_loss'], true)
            ? null
