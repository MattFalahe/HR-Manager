...
{
    private const CACHE_TTL = 21600; // 6h — a "what is this character used for" profile (6-month activity) barely moves hour to hour, and the longer cache means a profile pre-warm / rebuild reuses it instead of re-running the per-character cross-plugin fan-out
     ...
     */
    public function classify(int $characterId, int $corporationId, ?int $months = null): array
    {
        // Operator-tunable activity window (config / env HR_ACTIVITY_WINDOW_MONTHS);
        // shorter = cheaper cross-plugin queries. An explicit caller value wins.
    ...
    private function build(int $characterId, int $corporationId, int $months): array
    {
        ...
        // --- Ratting / missions (SeAT core wallet journals, direct) ---
        // Computed locally from character_wallet_journals rather than via CWM's
        // ratting.getCharacterBreakdown: that capability is ~1s PER CHARACTER
        // (it scans the journal in a heavy way), which on a 30-alt account was
        // ~30s of the profile-build cost. A plain indexed sum over the same
        // journal is a few ms per character and needs no CWM — bounty_prizes for
        // the Ratter badge, agent-mission rewards for the Mission Runner badge.
        try {
            if (Schema::hasTable('character_wallet_journals')) {
                $row = DB::table('character_wallet_journals')
                ...
                $bounty  = (float) ($row->bounty ?? 0);
                $mission = (float) ($row->mission ?? 0);
