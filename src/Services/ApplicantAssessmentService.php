...
class ApplicantAssessmentService
{
     ...
     * }
     */
    public function assess(int $characterId, ?array $allowedCorpIds = null, ?int $applicantCorpId = null, ?int $applicantAllianceId = null): array
    {
        ...
        $signals['watchlist']    = $this->watchlistSignal($characterId, $allowedCorpIds, $char, $flags, $applicantCorpId, $applicantAllianceId);
     ...
     */
    private function watchlistSignal(int $characterId, ?array $allowedCorpIds, CharacterInfo $char, array &$flags, ?int $applicantCorpId = null, ?int $applicantAllianceId = null): array
    {
        try {
            // Scope the match to the RECRUITING corp context (passed from the
            // application) so a corp/alliance-scoped blacklist entry matches on the
            // corp being applied to. Fall back to the character's own current corp
            // when called without that context (e.g. a member-view check).
            $scopeCorpId = $applicantCorpId ?: (int) ($char->corporation_id ?? 0);
            $match = $this->watchlist->findMatch($characterId, $allowedCorpIds, $scopeCorpId ?: null, $applicantAllianceId ?: null);
        } catch (\Throwable $e) {
