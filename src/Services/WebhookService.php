...
{
    /**
     * @param ?string $mentionUserId Discord snowflake of a specific person to
     *   @-mention (e.g. the player just marked for purge).
     * @param bool $suppressRole When true, skip the configured role ping and
     *   only @-mention the person — used by the personal purge notice, whose
     *   whole point is to reach the affected player, not the leadership role.
     */
    public function sendDiscordWebhook(WebhookConfiguration $config, array $embed, ?string $mentionUserId = null, bool $suppressRole = false): bool
    {
        $payload = [
        ...
        $mentions = [];
        if ($config->discord_role_id && !$suppressRole) {
            $mentions[] = "<@&{$config->discord_role_id}>";
        }
        if ($mentionUserId) {
    ...
    public function buildApplicationEmbed(string $event, array $data): array
    {
        $colors = [
            ...
            'purge_personal_notice' => 0xe83e8c,
            'player_status_status_cleared' => 0x28a745,
            'token_revoked' => 0xdc3545,
            'token_revoked_director' => 0x8b0000,
            'token_restored' => 0x28a745,
            'token_coverage' => 0x17a2b8,
        ];
        ...
        $titles = [
            ...
            'purge_personal_notice'          => 'Purge notice',
            'player_status_status_cleared'   => 'Player status cleared',
            'token_revoked'                  => 'Security: SeAT token revoked',
            'token_revoked_director'         => 'SECURITY BREACH: Director token revoked',
            'token_restored'                 => 'Resolved: SeAT token restored',
            'token_coverage'                 => 'Token coverage digest',
        ];
        $title = $titles[$event]
            ?? ucfirst(trim(str_replace(['player_status_', '_'], ['', ' '], $event)));

        // Deep link to whatever the alert is about (the application, the player
        // profile, the character page…). Discord makes the embed title a
        // hyperlink when `url` is set; we also append an explicit line so the
        // link is obvious rather than something you have to discover by hover.
        $link = $data['link'] ?? null;
        $description = $data['description'] ?? '';
        if ($link) {
            $description = trim($description) . "\n\n[" . ($data['link_label'] ?? 'Open in HR Manager') . "]({$link})";
        }
