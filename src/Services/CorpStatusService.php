...
class CorpStatusService
{
    ...
    public const TABS = ['overview', 'composition', 'economy', 'structure-compliance', 'membership', 'alignment', 'access', 'recruitment', 'purge'];
    ...
    private function buildTab(int $corporationId, string $tab): array
    {
        return match ($tab) {
            ...
            // Title/role alignment is built separately (RoleAlignmentService) — no
            // $corpStatus sections here, so don't fall through to the overview build.
            'alignment' => [],
            default => [ // overview
    ...
    private function roleDistribution(int $corpId): array
    {
        ...
        // --- Ratters from SeAT wallet journals (bounty income across the whole
        // roster). Direct roster scan — like PI / industry / trading below —
        // rather than HR's assessment cache, which only covers members someone
        // has opened, so this reflects the whole corp instead of a sparse subset.
        $ratterIds = [];
        if (Schema::hasTable('character_wallet_journals')) {
            $ratterIds = DB::table('character_wallet_journals')
                ->whereIn('character_id', $rosterIds)
                ->where('date', '>=', $since)
                ->where('ref_type', 'like', 'bounty%')
                ->groupBy('character_id')
                ->havingRaw('SUM(amount) >= ?', [$floor])
                ->pluck('character_id')->map(fn($id) => (int) $id)->all();
        }

        // --- Miners from Mining Manager's mining_ledger (distinct roster chars
        // who mined in the window). Needs Mining Manager — it populates the
        // table; empty / absent → no miners counted (bar reads 0).
        $minerIds = [];
    ...
    private function memberFinancialRows(int $corpId): array
    {
        ...
        if (is_array($payload) && !empty($payload['available']) && !empty($payload['members'])) {
            ...
            return $this->memberRowsMemo[$corpId] = ['source' => 'corp-wide', 'rows' => $this->aggregateByPlayer($rows)];
        }
        ...
        if (Schema::hasColumn('hr_manager_member_assessments', 'lifetime_contribution')) {
            ...
            return $this->memberRowsMemo[$corpId] = ['source' => 'registered', 'rows' => $this->aggregateByPlayer($rows)];
        }
    ...
    /**
     * Fold per-character financial rows into per-PLAYER rows so each human
     * appears once across every Wallet Insights card — a member's alts are the
     * same person, and showing them separately double-counts and scatters them
     * (e.g. the same human landing in both Top Contributors and the anomaly
     * board). Each account is keyed to its MAIN character (the display
     * identity); characters with no linked SeAT account (unregistered members,
     * or corp / holding-corp entities that aren't a person) pass through as
     * their own single-entry row.
     *
     * Sums contribution / net / earned across the account's characters, takes
     * the best active-months, ORs is_registered, and rolls compliance up
     * WEIGHTED by each character's income (so a player's tax % reflects tax paid
     * on their total earnings; null when the account earned nothing). Carries an
     * `alts` breakdown for the expandable per-character view.
     *
     * @param \Illuminate\Support\Collection $charRows
     * @return \Illuminate\Support\Collection
     */
    private function aggregateByPlayer($charRows)
    {
        if ($charRows->isEmpty()) {
            return $charRows;
        }

        $charIds = $charRows->pluck('character_id')->map(fn ($id) => (int) $id)->filter()->all();

        $charToUser = !empty($charIds) && Schema::hasTable('refresh_tokens')
        ...
        $userToMain = !empty($userIds) && Schema::hasTable('users')
            ? DB::table('users')->whereIn('id', $userIds)->pluck('main_character_id', 'id')->toArray()
            : [];

        // Group each character row under its account main (or itself when unlinked).
        $groups = [];
        foreach ($charRows as $row) {
            $cid  = (int) $row->character_id;
            $uid  = $charToUser[$cid] ?? null;
            $main = $uid !== null ? (int) ($userToMain[(int) $uid] ?? 0) : 0;
            $key  = $main > 0 ? $main : $cid;
            $groups[$key][] = $row;
        }

        $out = collect();
        foreach ($groups as $mainId => $rows) {
            // A single character that IS the account main (or an unlinked
            // character / corp entity) passes through unchanged.
            if (count($rows) === 1 && (int) $rows[0]->character_id === (int) $mainId) {
                $r = clone $rows[0];
                $r->alt_count = 1;
                $r->alts = [];
                $out->push($r);
            ...
            $contribution = 0.0; $net = 0.0; $earned = 0.0; $activeMonths = 0;
            $complianceNum = 0.0; $complianceDen = 0.0; $anyCompliance = false;
            $isRegistered = false;
            $alts = [];
            foreach ($rows as $r) {
                ...
                $activeMonths  = max($activeMonths, (int) $r->active_months);
                $isRegistered  = $isRegistered || (bool) $r->is_registered;
                if ($r->compliance !== null) {
                    $anyCompliance = true;
                    $w = max(0.0, (float) $r->earned);
                    $complianceNum += (float) $r->compliance * $w;
                    $complianceDen += $w;
                }
                $alts[] = [
            ...
            // Income-weighted compliance; if nobody earned, a simple mean of the
            // present readings still conveys the account's tax posture.
            $compliance = null;
            if ($anyCompliance) {
                if ($complianceDen > 0) {
                    $compliance = $complianceNum / $complianceDen;
                } else {
                    $present = array_values(array_filter(array_map(fn ($a) => $a['compliance'], $alts), fn ($c) => $c !== null));
                    $compliance = !empty($present) ? array_sum($present) / count($present) : null;
                }
            }

            $out->push((object) [
                'character_id'  => (int) $mainId,
    ...
    private function walletFreeloaders(int $corpId, float $threshold = 1_000_000): array
    {
        ...
        $names = $this->namesFor($this->allCardCharIds($rows));
        ...
        $list = $rows->map(function ($r) use ($names, $nonChar, $threshold) {
            ...
            return [
                ...
                'alt_count'    => $r->alt_count ?? 1,
                'alts'         => $this->namedAlts($r->alts ?? [], $names),
            ];
        })->all();
    ...
    private function walletAnomalies(int $corpId): array
    {
        ...
        // A whole account is alliance-tax-exempt if its main OR any alt is listed.
        $exemptFor = fn ($r) => isset($exempt[$r->character_id])
            || collect($r->alts ?? [])->contains(fn ($a) => isset($exempt[(int) $a['character_id']]));

        // NEG is only meaningful when the member took corp ISK AND gave little
        // back. Net-negative alone catches every director who moves corp ISK and
        // every SRP recipient — legitimate, not anomalies — so we require the
        // account's total contribution to be below a floor. That keeps real
        // contributors (and you) off the board and surfaces the actual takers.
        $contribFloor = 1_000_000_000; // 1B ISK
        $isTaker = fn ($r) => $r->net < 0 && $r->contribution < $contribFloor;

        $rows = $data['rows']
            ->filter(fn ($r) => $isTaker($r)
                || ($r->compliance !== null && $r->compliance < 50 && !$exemptFor($r)))
            ->sortBy('net')
            ->take(15)
            ->values();
        ...
        $list = $rows->map(function ($r) use ($names, $nonChar, $exemptFor, $isTaker) {
            $isExempt = $exemptFor($r);
            $flags = [];
            if ($isTaker($r)) $flags[] = 'NEG';
    ...
    private function corpWalletFlaggedMembers(int $corpId): array
    {
        ...
        // Only characters CURRENTLY in this corp. A history event keeps the
        // corp_id from when it fired, so a member who has since left (or a
        // character CWM reported on that was never really in this corp) would
        // otherwise linger here. Filter against SeAT's live character->corp map.
        if (!empty($byChar) && Schema::hasTable('character_affiliations')) {
            $current = DB::table('character_affiliations')
                ->whereIn('character_id', array_keys($byChar))
                ->where('corporation_id', $corpId)
                ->pluck('character_id')->map(fn ($id) => (int) $id)->flip()->toArray();
            $byChar = array_intersect_key($byChar, $current);
        }
        if (empty($byChar)) {
            return ['available' => true, 'list' => [], 'total' => 0];
        }
    ...
    /**
     * Every character id a set of aggregated rows references — the main
     * (character_id) plus every alt in the breakdown — so a card can resolve
     * names for the row AND its expandable per-alt list in one batch.
     */
    private function allCardCharIds($rows): array
    {
        $ids = $rows->pluck('character_id')->all();
        foreach ($rows as $r) {
            foreach (($r->alts ?? []) as $a) {
                $ids[] = (int) $a['character_id'];
            }
        }
    ...
    /**
     * Attach resolved names to an alts breakdown (from aggregateByPlayer), for
     * the expandable per-character view. Sorted by contribution desc.
     */
    private function namedAlts(array $alts, array $names): array
    {
        $out = array_map(fn ($a) => array_merge($a, [
            'name' => $names[(int) $a['character_id']] ?? ('#' . $a['character_id']),
        ]), $alts);
        usort($out, fn ($x, $y) => $y['contribution'] <=> $x['contribution']);
        return $out;
    }
    ...
    private function roleRoster(int $corpId, string $role): array
    {
        ...
        // Fold role-holding characters into accounts (main + role-holding alts)
        // so one human running several director alts is a single row. Holders
        // with no SeAT account can't be grouped, so each stands alone.
        $groups = $this->groupRoleHoldersByAccount($list);

        return [
            'available'     => true,
            'list'          => $groups,
            'count'         => count($list),   // role-holding characters
            'account_count' => count($groups), // distinct humans / unlinked chars
        ];
    }

    /**
     * Fold a flat per-character role-holder list into per-account rows. The
     * header is the account MAIN (a consistent human identity even when only an
     * alt holds the role); role_chars lists the character(s) that actually hold
     * it, with the main flagged. Characters with no SeAT account pass through
     * as their own single-character row.
     *
     * @param  array<int,array<string,mixed>>  $list
     * @return array<int,array<string,mixed>>
     */
    private function groupRoleHoldersByAccount(array $list): array
    {
        $byUser = [];
        $singletons = [];
        foreach ($list as $entry) {
            if (!empty($entry['user_id'])) {
                $byUser[(int) $entry['user_id']][] = $entry;
            } else {
                $singletons[] = $entry;
            }
        }
        ...
        if (!empty($byUser)) {
            $mains = DB::table('users')->whereIn('id', array_keys($byUser))
                ->pluck('main_character_id', 'id')->toArray();
            $mainIds = array_values(array_filter(array_map('intval', $mains)));
            $mainNames = !empty($mainIds)
                ? app(NameResolutionService::class)->getCharacterNamesWithFallback($mainIds)
                : [];

            foreach ($byUser as $uid => $entries) {
                $mainId  = (int) ($mains[$uid] ?? 0);
                $mainRow = null;
                foreach ($entries as $e) {
                    if ($e['character_id'] === $mainId) { $mainRow = $e; break; }
                }

                $lastLogon = null;
                foreach ($entries as $e) {
                    if (!empty($e['last_logon']) && ($lastLogon === null || $e['last_logon'] > $lastLogon)) {
                        $lastLogon = $e['last_logon'];
                    }
                }

                $roleChars = array_map(function ($e) use ($mainId) {
                    $e['is_main'] = ($e['character_id'] === $mainId);
                    return $e;
                }, $entries);
                usort($roleChars, fn ($a, $b) => ($b['is_main'] <=> $a['is_main']) ?: strcmp((string) $a['name'], (string) $b['name']));

                $groups[] = [
                    'character_id'    => $mainId ?: (int) $entries[0]['character_id'],
                    'name'            => $mainNames[$mainId] ?? ($mainRow['name'] ?? $entries[0]['name']),
                    'user_id'         => (int) $uid,
                    'classifier'      => $entries[0]['classifier'],
                    'last_logon'      => $lastLogon,
                    'security_status' => $mainRow['security_status'] ?? $entries[0]['security_status'],
                    'role_char_count' => count($entries),
                    'role_chars'      => array_values($roleChars),
                    'main_holds'      => $mainRow !== null,
                ];
            }
        }
        ...
        foreach ($singletons as $s) {
            ...
            $char['is_main']      = true;
            $s['role_char_count'] = 1;
            $s['main_holds']      = true;   // unlinked: the character is its own identity
            $s['role_chars']      = [$char];
            $groups[] = $s;
        }

        usort($groups, fn ($a, $b) => strcmp((string) $a['name'], (string) $b['name']));
    ...
    private function buildDirectorHealth(int $corpId): array
    {
        ...
        // Group the inactive directors by HUMAN account so the UI shows one row
        // per person (with their dark director characters as expandable detail),
        // instead of a flat character list where you can't tell one human from
        // many. Unauthed directors can't be tied to an account, so each stands
        // alone.
        $groupsByUser  = [];
        $unauthedGroups = [];
        foreach ($inactive as $e) {
            if ($e['is_authed'] && $e['user_id']) {
                $groupsByUser[(int) $e['user_id']][] = $e;
            } else {
                $unauthedGroups[] = [
                    'user_id'           => null,
                    'is_authed'         => false,
                    'main_character_id' => $e['character_id'],
                    'main_name'         => $e['name'],
                    'classifier'        => null,
                    'characters'        => [$e],
                ];
        ...
        $authedUserIds = array_keys($groupsByUser);
        $mainIdByUser  = !empty($authedUserIds)
            ? DB::table('users')->whereIn('id', $authedUserIds)->pluck('main_character_id', 'id')->toArray()
            : [];
        ...
        foreach ($groupsByUser as $uid => $chars) {
            ...
            $authedGroups[] = [
                ...
                'main_name'         => $mainNames[$mainId] ?? ($chars[0]['name'] ?? ('#' . $mainId)),
                'classifier'        => $chars[0]['classifier'] ?? null,
                'characters'        => $chars,
            ];
        }
