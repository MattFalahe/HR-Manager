...
class ApplicantAccessService
{
    ...
    public function grant(Application $application, int $recruiterUserId): ?RecruiterAccessGrant
    {
        ...
        try {
            ...
            $grant = DB::transaction(function () use ($application, $recruiterUserId, $characterIds, $permissions, $expiresAt) {
            ...
            // Audit trail: who was granted temporary applicant access, over which
            // characters, with what permissions and until when. Actor auto-resolves
            // to the current user (or null/system for retroactive/cron grants).
            app(AuditService::class)->action('access.grant', AuditService::CAT_PRIVILEGE, [
                'target_type' => 'application',
                'target_id'   => (int) $application->id,
                'summary'     => 'Granted temporary applicant access',
                'corporation_id' => $application->corporation_id ? (int) $application->corporation_id : null,
                'context'     => [
                    'subject_user_id' => $recruiterUserId,
                    'permission_set'  => array_values($permissions),
                    'characters'      => count($characterIds),
     ...
     * included; alts pulled when settings allow.
     *
     * Primary resolution is the live shared-account lookup via
     * refresh_tokens.user_id — the exact same canonical resolver the
     * assessment + screening services use. It reflects the account the
     * instant an alt registers a token, so the grant (and therefore the
     * temporary-access panel + the SeAT role filters) is alt-aware
     * immediately, without waiting for HR's own identity-mapping sync to
     * catch up. The PlayerIdentity mapping is kept as a secondary union so
     * historically-tracked alts whose tokens have since been removed are
     * still covered.
     */
    private function resolveCharacterIds(Application $application): array
    {
        $mainId = (int) $application->character_id;
        $ids = [$mainId];
        ...
        // Primary: live account resolution via shared refresh_tokens.user_id.
        try {
            if (Schema::hasTable('refresh_tokens')) {
                $userId = (int) DB::table('refresh_tokens')
                    ->where('character_id', $mainId)
                    ->whereNull('deleted_at')
                    ->value('user_id');
                if ($userId > 0) {
                    $altIds = DB::table('refresh_tokens')
                        ->where('user_id', $userId)
                        ->whereNull('deleted_at')
                        ->pluck('character_id')
                        ->map(fn ($id) => (int) $id)
            ...
            Log::info('[HR Manager] token-based alt resolution failed: ' . $e->getMessage());
        }

        // Secondary: HR's own PlayerIdentity mapping (union — catches alts we
        // still track whose live tokens have since been removed).
        try {
            $identity = $this->playerIdentity->forCharacter($mainId);
    ...
    {
        $ok = DB::transaction(function () use ($grant, $reason) {
        ...
        // Audit trail: privilege lost, and WHY (manual / application_closed /
        // expired). Actor auto-resolves; cron sweeps have no auth user → System.
        if ($ok) {
            app(AuditService::class)->action('access.revoke', AuditService::CAT_PRIVILEGE, [
                'target_type' => 'application',
                'target_id'   => (int) $grant->application_id,
                'summary'     => 'Revoked applicant access',
                'context'     => [
                    'subject_user_id' => (int) $grant->user_id,
                    'reason'          => $reason,
                ],
            ]);
        }
