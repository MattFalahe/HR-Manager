...
class ApplicantConnectorAccessService
{
    ...
    public function grant(Application $application, ?int $applicantUserId = null): ?ApplicantConnectorGrant
    {
        ...
        try {
            ...
            $grant = DB::transaction(function () use ($application, $userId, $permission, $expiresAt) {
            ...
            // Audit trail: applicant granted temporary Connector (Discord) link
            // access so they can self-link before joining.
            app(AuditService::class)->action('connector_access.grant', AuditService::CAT_PRIVILEGE, [
                'target_type' => 'application',
                'target_id'   => (int) $application->id,
                'summary'     => 'Granted applicant Connector link access',
    ...
    private function revokeGrant(ApplicantConnectorGrant $grant, string $reason): bool
    {
        ...
        if ($ok) {
            app(AuditService::class)->action('connector_access.revoke', AuditService::CAT_PRIVILEGE, [
                'target_type' => 'application',
                'target_id'   => (int) $grant->application_id,
                'summary'     => 'Revoked applicant Connector link access',
                'context'     => [
