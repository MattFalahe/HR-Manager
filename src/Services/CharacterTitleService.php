...
class CharacterTitleService
{
    ...
    /**
     * Does this character hold the in-game Director role in the corp? The
     * highest-impact role (full access), so the Token-loss Watchdog treats a
     * director losing their SeAT token as a top-severity security event.
     */
    public function holdsDirector(int $characterId, int $corporationId): bool
    {
        return in_array('Director', $this->directRolesForCharacter($characterId, $corporationId), true);
    }
