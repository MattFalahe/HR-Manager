...
class PlayerService
{
    ...
    private function lastActivityFor(int $characterId): ?Carbon
    {
        ...
        // SeAT corp member tracking — the character's last in-game LOGIN. The
        // most reliable "is this member active" signal, and the same source the
        // Corp-wide activity panel buckets on, so a member who logs in but
        // doesn't rat / mine / trade still reads as active (not dead weight).
        if (Schema::hasTable('corporation_member_trackings')) {
            try {
                $login = DB::table('corporation_member_trackings')
                    ->where('character_id', $characterId)
                    ->max('logon_date');
                if ($login) {
                    $candidates[] = Carbon::parse($login);
                }
            } catch (\Throwable $e) {
                \Illuminate\Support\Facades\Log::warning('[HR Manager] PlayerService: member-tracking login query failed: ' . $e->getMessage());
            }
        }
