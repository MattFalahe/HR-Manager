...
{
    /**
     * Cross-request cache TTL (seconds) for successful bridge responses. Every
     * cross-plugin read (ratting / mining / contribution / blueprint …) funnels
     * through callPluginBridge, and a player profile with many alts fans out to
     * dozens of these per load. They are all multi-month AGGREGATES that barely
     * move minute-to-minute, so a short read-through cache turns repeat loads
     * (and the per-alt fan-out) from dozens of MC dispatches into cache hits,
     * while staying fresh enough for HR's purposes. Only successful responses
     * are cached — "absent / not-registered" states are cheap and must stay
     * live so an install/uninstall reflects immediately.
     */
    private const BRIDGE_CACHE_TTL = 300;
    ...
    /**
     * Combined mining-engagement picture for one or many characters (pass a
     * whole account's ids to sum across alts). Batched — one query per metric —
     * so it's cheap even for a 30-alt account.
     *
     * Deliberately broader than event attendance alone, because a corp that
     * doesn't run scheduled ops made "attended 0 of 2 ops" read like a red flag
     * when it was really "we barely use events". So this also surfaces:
     *   - total ore value + active mining months (are they mining at all?),
     *   - scheduled-op attendance ONLY when the corp actually runs ops,
     *   - moon-mining: how many chunks the CORP pulled, and the moon-ore value
     *     these characters mined at the corp's own moon structures
     *     (mining_ledger.is_moon_ore at the corp's moon_extractions structures).
     *
     * has_ops / has_moons let the view show each dimension only when it's real.
     *
     * @param  array<int>  $characterIds
     * @return array{available:bool,has_data:bool,has_ops:bool,has_moons:bool,ore_value:float,moon_ore_value:float,active_months:int,ops_total:int,ops_attended:int,ops_rate:?int,moon_pulls_total:int,months:int}
     */
    public function getMiningEngagement(array $characterIds, int $corporationId, int $months = 6): array
    {
        $out = [
            'available' => false, 'has_data' => false, 'has_ops' => false, 'has_moons' => false,
            'ore_value' => 0.0, 'moon_ore_value' => 0.0, 'active_months' => 0,
            'ops_total' => 0, 'ops_attended' => 0, 'ops_rate' => null,
            'moon_pulls_total' => 0, 'months' => $months,
        ];
        ...
        try {
            // Total ore value + active months from Mining Manager's MONTHLY
            // SUMMARY — the same source the Activity breakdown and the Assessment
            // Data card use, so all three ore-value figures agree. (The raw
            // mining_ledger is live and drifts a bit ahead of MM's rolled-up
            // summary, which is what caused the mismatch.) Falls back to the raw
            // ledger only if the summary model isn't present.
            $summaryClass = 'MiningManager\\Models\\MiningLedgerMonthlySummary';
            if (class_exists($summaryClass)) {
                $rows = $summaryClass::whereIn('character_id', $characterIds)
                    ->where('month', '>=', now()->subMonths($months)->startOfMonth())
                    ->get(['month', 'total_value']);
                $out['ore_value']     = (float) $rows->sum('total_value');
                $out['active_months'] = $rows->pluck('month')->map(fn ($m) => (string) $m)->unique()->count();
            } else {
                $agg = DB::table('mining_ledger')
                    ->whereIn('character_id', $characterIds)
                    ->where('date', '>=', $since)
                    ->selectRaw("SUM(total_value) AS val, COUNT(DISTINCT DATE_FORMAT(date, '%Y-%m')) AS months_active")
                    ->first();
                $out['ore_value']     = (float) ($agg->val ?? 0);
                $out['active_months'] = (int) ($agg->months_active ?? 0);
            }

            // Scheduled-op attendance — only meaningful when the corp runs ops.
            if (Schema::hasTable('mining_events') && Schema::hasTable('event_participants')) {
                $real = ['completed', 'active'];
                $out['ops_total'] = (int) DB::table('mining_events')
                    ->where('corporation_id', $corporationId)
                    ->whereIn('status', $real)
                    ->where('start_time', '>=', $since)
                    ->count();
                if ($out['ops_total'] > 0) {
                    $out['ops_attended'] = (int) DB::table('event_participants as ep')
                        ->join('mining_events as me', 'me.id', '=', 'ep.event_id')
                        ->whereIn('ep.character_id', $characterIds)
                        ->where('me.corporation_id', $corporationId)
                        ->whereIn('me.status', $real)
                        ->where('me.start_time', '>=', $since)
                        ->distinct()
                        ->count('me.id');
                    $out['ops_rate'] = (int) round($out['ops_attended'] / $out['ops_total'] * 100);
                    $out['has_ops']  = true;
                }
            }

            // Moon mining — only when the corp actually runs moon extractions.
            if (Schema::hasTable('moon_extractions')) {
                $out['moon_pulls_total'] = (int) DB::table('moon_extractions')
                    ->where('corporation_id', $corporationId)
                    ->where('chunk_arrival_time', '>=', $since)
                    ->count();
                if ($out['moon_pulls_total'] > 0) {
                    $out['has_moons'] = true;
                    $moonStructures = DB::table('moon_extractions')
                        ->where('corporation_id', $corporationId)
                        ->distinct()
                        ->pluck('structure_id')
                        ->map(fn ($s) => (int) $s)
                        ->all();
                    if (!empty($moonStructures)) {
                        $out['moon_ore_value'] = (float) DB::table('mining_ledger')
                            ->whereIn('character_id', $characterIds)
                            ->whereIn('observer_id', $moonStructures)
                            ->where('is_moon_ore', 1)
                            ->where('date', '>=', $since)
                            ->sum('total_value');
                    }
            ...
            $out['has_data'] = $out['ore_value'] > 0 || $out['ops_total'] > 0 || $out['moon_pulls_total'] > 0;
        } catch (\Throwable $e) {
            Log::warning('[HR Manager] getMiningEngagement failed: ' . $e->getMessage());
        }
    ...
    private function callPluginBridge(string $plugin, string $capabilityName, array $args): array
    {
        ...
        // Read-through cache on the successful payload only. Keyed by
        // plugin + capability + args so each (character, corp, window) tuple
        // caches independently. A cache hit skips the MC dispatch + the target
        // plugin's own query entirely — the bulk of a many-alt profile's cost.
        $cacheKey = 'hr-bridge-' . md5($plugin . '|' . $capabilityName . '|' . serialize($args));
        ...
        try {
            ...
            Cache::put($cacheKey, $payload, self::BRIDGE_CACHE_TTL);
            return $payload;
        } catch (\Exception $e) {
