...
class BlueprintActivityService
{
    ...
    public function getCorpSummary(int $corporationId): array
    {
        ...
        $data = $res['data'];
        // Blueprint Manager returns the top requesters per CHARACTER. Fold them
        // into accounts (main + requesting alts) so one human isn't listed once
        // per alt, keeping a per-character split for the expandable detail.
        if (!empty($data['top_requesters']) && is_array($data['top_requesters'])) {
            $data['top_requesters'] = $this->groupRequestersByAccount($data['top_requesters']);
        }
        return ['available' => true] + $data;
    }

    /**
     * Fold Blueprint Manager's per-character top-requester list into per-account
     * rows: sum the request / fulfil / reject counts across a human's
     * characters, head the row with their MAIN, and keep the per-character split
     * in `chars` for the expandable detail. Requesters with no SeAT account
     * stand alone as their own single-character row.
     *
     * Note: Blueprint Manager truncates to its top-10 CHARACTERS upstream, so
     * this regroups the returned set — it does not re-rank the whole corp.
     *
     * @param  array<int,array<string,mixed>>  $requesters
     * @return array<int,array<string,mixed>>
     */
    private function groupRequestersByAccount(array $requesters): array
    {
        $charIds = array_values(array_filter(array_map(
            fn ($r) => (int) ($r['character_id'] ?? 0),
            $requesters
        )));
        if (empty($charIds)) {
            return $requesters;
        ...
        // character -> user, then user -> main character.
        $userByChar = DB::table('refresh_tokens')
            ->whereIn('character_id', $charIds)
            ->whereNull('deleted_at')
            ->pluck('user_id', 'character_id')
            ->toArray();
        $userIds = array_values(array_unique(array_map('intval', $userByChar)));
        $mainByUser = !empty($userIds)
            ? DB::table('users')->whereIn('id', $userIds)->pluck('main_character_id', 'id')->toArray()
            : [];

        // A main may not itself appear among the requesters, so resolve its name.
        ...
        $groups = [];   // keyed "u{userId}" (linked) or "c{charId}" (unlinked)
        foreach ($requesters as $r) {
            $cid = (int) ($r['character_id'] ?? 0);
            $uid = (int) ($userByChar[$cid] ?? 0);

            if ($uid > 0) {
                $mainId = (int) ($mainByUser[$uid] ?? 0) ?: $cid;
                $key    = 'u' . $uid;
                $headNm = $mainNames[$mainId] ?? null;
            } else {
                $mainId = $cid;
                $key    = 'c' . $cid;
                $headNm = null;
            ...
            if (!isset($groups[$key])) {
                $groups[$key] = [
                    'character_id'   => $mainId,
                    'character_name' => $headNm ?? ($r['character_name'] ?? ('#' . $mainId)),
                    'total_requests' => 0,
                    'fulfilled'      => 0,
                    'rejected'       => 0,
                    'chars'          => [],
            ...
            $groups[$key]['total_requests'] += (int) ($r['total_requests'] ?? 0);
            $groups[$key]['fulfilled']      += (int) ($r['fulfilled'] ?? 0);
            $groups[$key]['rejected']       += (int) ($r['rejected'] ?? 0);
            $groups[$key]['chars'][] = [
                'character_id'   => $cid,
                'character_name' => $r['character_name'] ?? ('#' . $cid),
                'total_requests' => (int) ($r['total_requests'] ?? 0),
                'fulfilled'      => (int) ($r['fulfilled'] ?? 0),
                'rejected'       => (int) ($r['rejected'] ?? 0),
                'is_main'        => ($cid === (int) $groups[$key]['_main_id']),
            ];
        }

        $out = [];
        foreach ($groups as $g) {
            usort($g['chars'], fn ($a, $b) => ($b['is_main'] <=> $a['is_main']) ?: ($b['total_requests'] <=> $a['total_requests']));
            $g['char_count'] = count($g['chars']);
            unset($g['_main_id']);
            $out[] = $g;
        }

        // Re-rank accounts by combined request volume (grouping changed totals).
        usort($out, fn ($a, $b) => $b['total_requests'] <=> $a['total_requests']);
