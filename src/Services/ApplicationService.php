...
class ApplicationService
{
    ...
    public function submitApplication(int $characterId, int $templateId, ?int $corporationId, array $answers, ?int $submitterUserId = null): Application
    {
        ...
        // Screen the WHOLE applying account against the blacklist + intel: a hit
        // notifies directors, records the discovered link on the history timeline,
        // and (opt-in) auto-flags the rest of the account. Best-effort — a
        // screening failure must never block the submit.
        try {
            app(\HrManager\Services\ApplicantScreeningService::class)->screen($application, (int) $userId);
        } catch (\Throwable $e) {
            Log::warning('[HR Manager] applicant screening on submit failed: ' . $e->getMessage());
        }
    ...
    /**
     * Undo the most recent status change as an accidental mistake. The erroneous
     * status_history row is VOIDED (kept internally, hidden from the applicant's
     * tracking timeline) and the application is restored to the status it held
     * before that change. Director-gated at the controller; a written reason is
     * required. Deliberately bypasses canTransition (a mistaken rejected ->
     * interview is not a normal transition) and creates NO new public history row
     * (the void IS the correction).
     *
     * NOTE: this reverses the STATUS and its applicant-facing visibility only. It
     * does NOT undo side effects that already fired on the voided transition (the
     * recruiter / Connector access revoked on a close, Discord onboarding on an
     * accept, the original webhook that already went out). The controller warns
     * on 'accepted'.
     *
     * @return array{voided:string, restored:string}|false false when there is
     *         nothing revertable (no rows, or only the initial 'applied' row,
     *         which has no prior state to restore to).
     */
    public function revertLastStatus(Application $application, int $userId, string $reason)
    {
        $result = DB::transaction(function () use ($application, $userId, $reason) {
            $locked = Application::lockForUpdate()->find($application->id);
            if (!$locked) {
                return false;
            }

            $last = ApplicationStatusHistory::where('application_id', $locked->id)
                ->whereNull('voided_at')
                ->orderByDesc('id')
                ->first();

            if (!$last || $last->old_status === null) {
                return false;
            }

            $voided   = (string) $last->new_status;
            $restored = (string) $last->old_status;
            ...
            // Undo the decision stamp when reverting an accept / reject.
            if (in_array($voided, ['accepted', 'rejected'], true)) {
            ...
            return ['voided' => $voided, 'restored' => $restored];
        });

        if ($result !== false) {
            $this->safelyNotify(fn() => $this->notifications->notifyStatusReverted(
                $application,
                $result['voided'],
                $result['restored'],
                $userId,
    ...
    /**
     * The transition a revert would undo (the last non-voided row that has a
     * prior state), or null when nothing is revertable. Lets the director view
     * show / label the Undo control and warn when it would undo an 'accepted'.
     */
    public function revertableLastTransition(Application $application): ?ApplicationStatusHistory
    {
        $last = ApplicationStatusHistory::where('application_id', $application->id)
            ->whereNull('voided_at')
            ->orderByDesc('id')
            ->first();

        return ($last && $last->old_status !== null) ? $last : null;
    }
     ...
     */
    public function applicantWithdraw(Application $application, ?string $reason = null): bool
    {
        ...
        // Compose the status-history comment. The applicant's reason (when the
        // operator asks for one) rides in the comment field — directors see it
        // in the timeline; it is never echoed on the public tracking page.
        $reason  = $reason !== null ? trim($reason) : '';
        $reason  = $reason !== '' ? mb_substr($reason, 0, 1000) : '';
        $comment = $reason !== ''
            ? 'Withdrawn by the applicant. Reason: ' . $reason
            : 'Withdrawn by the applicant.';

        $oldStatus = null;
        $success = DB::transaction(function () use ($application, $actorUserId, $comment, &$oldStatus) {
            $locked = Application::lockForUpdate()->find($application->id);
        ...
        if ($success && $oldStatus !== null) {
            $this->safelyNotify(fn() => $this->notifications->notifyStatusChange($application, $oldStatus, 'withdrawn', $actorUserId, $comment));
            $this->publishApplicationEvent('hr.application.withdrawn', $application, $oldStatus, $comment);
