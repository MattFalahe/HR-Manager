...
class IntelService
{
    ...
    /**
     * Notes for ANY of the given characters — the applicant's whole account
     * (main + registered alts) — that the viewer can see. This is what the
     * application detail uses, so an intel note recorded against a flagged ALT
     * still surfaces, matching the alt-aware watchlist banner.
     *
     * @param array<int>      $characterIds
     * @param array<int>|null $allowedCorpIds  null = admin (all)
     */
    public function notesForCharacters(array $characterIds, int $viewerUserId, ?array $allowedCorpIds, string $viewerTier): Collection
    {
        ...
        $query = IntelNote::whereIn('character_id', $characterIds)
            ->active()
            ->with('author')
            ->orderByDesc('created_at');
