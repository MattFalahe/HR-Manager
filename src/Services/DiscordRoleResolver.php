 ...
 *
 * Implementation mirrors Structure Manager's DiscordRoleResolver, the
 * role-picker pattern shared across the suite.
 */
class DiscordRoleResolver
{
     ...
     *     'color'          => '#2ecc71' | null,
     *     'source'         => 'discord-roles-table',
     *     'sources'        => ['discord-roles-table', 'seat-connector'],
     *   ]
     *
    ...
    public static function listRoles(): array
    {
        ...
        foreach ($providers as $provider) {
            try {
                ...
                Log::warning('[HR Manager] DiscordRoleResolver: failed listing roles from ' . $provider . ': ' . $e->getMessage());
                continue;
            }
     ...
     *
     * Display names follow the suite's naming convention: the
     * seat-discord-pings package is displayed as "SeAT Broadcast" in
     * user-facing UI. The internal table
    ...
    /**
     * Legacy warlof/seat-discord-connector tables. Best-effort.
     */
