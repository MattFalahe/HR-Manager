...
class WatchlistService
{
    ...
    public function addEntry(
        ...
        // Preserve the lifecycle on the player timeline: WHEN a character was
        // added, by whom, and why. Character-keyed so it shows on the dossier +
    ...
    /**
     * Append a watchlist lifecycle event to the player history timeline.
     * Best-effort: a history failure never breaks the add/clear itself.
     */
    private function recordLifecycle(string $eventType, WatchlistEntry $entry, int $actorUserId, array $payload): void
    {
        try {
            app(HistoryEventService::class)->record($eventType, $payload, [
                'character_id'   => (int) $entry->character_id,
                'corporation_id' => $entry->scope_corporation_id !== null ? (int) $entry->scope_corporation_id : null,
                'actor_user_id'  => $actorUserId > 0 ? $actorUserId : null,
                'occurred_at'    => now(),
            ]);
        } catch (\Throwable $e) {
            \Illuminate\Support\Facades\Log::warning('[HR] watchlist history record failed: ' . $e->getMessage());
        }
    }
    ...
    /**
     * The first ACTIVE blacklist entry, at the given severity, across ANY of the
     * supplied characters (the whole applying account), scoped to the viewer's
     * corps / alliances. Drives the acceptance guard — returns null when nothing
     * at that severity blocks acceptance.
     *
     * @param array<int> $characterIds
     */
    public function firstBlockingBlacklist(array $characterIds, string $severity, ?array $allowedCorpIds = null, ?int $applicantCorpId = null, ?int $applicantAllianceId = null): ?WatchlistEntry
    {
        foreach (array_values(array_unique(array_filter(array_map('intval', $characterIds)))) as $cid) {
            $entry = $this->buildMatchQuery($cid, $allowedCorpIds, $applicantCorpId, $applicantAllianceId)
                ->active()
                ->where('list_type', WatchlistEntry::TYPE_BLACKLIST)
    ...
    public function clearEntry(int $id, int $clearedByUserId, ?string $reason): bool
    {
        ...
        // Idempotent: an already-cleared entry (re-submitted, or swept twice in
        // an account clear) is a no-op — no re-clear, no duplicate history event.
        ...
        // Preserve the clear on the player timeline (when + who + why), so the
        // dossier keeps the full add → hit → cleared story even though the entry
        // itself drops off the active list.
        $this->recordLifecycle('watchlist_cleared', $entry, $clearedByUserId, [
            'list_type' => $entry->list_type,
            'reason'    => $reason ? mb_substr($reason, 0, 300) : null,
            'label'     => 'Cleared from ' . $entry->list_type,
        ]);
