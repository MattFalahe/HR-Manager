...
class WatchlistMonitorService
{
    ...
    public function scanAllianceJoins(): int
    {
        ...
        foreach ($entries as $entry) {
            ...
            $relevantCorps = array_map('intval', array_keys(array_filter(
                $corpToAlliance,
                fn($aid) => in_array($aid, $allianceIds, true)
            )));
            // De-spam: when the corp pass already covers this entry, drop the
            // corps it handles so a blacklisted character in one of OUR corps
            // gets a single corp ALERT, not that plus a redundant alliance
            // WARNING. The alliance warning then only fires for the OTHER corps
            // in the alliance (the case it's actually meant for).
            if ($entry->notify_on_corp_match) {
                $relevantCorps = array_values(array_diff($relevantCorps, $this->scopedCorpIds($entry)));
            }
    ...
    public function checkEntryNow(WatchlistEntry $entry): int
    {
        ...
        if ($entry->notify_on_alliance_match
            ...
            if (!empty($allianceIds)) {
                ...
                // Same de-spam as the periodic pass: don't warn at the alliance
                // level for a corp the corp pass already alerted on.
                if ($entry->notify_on_corp_match) {
                    $relevantCorps = array_values(array_diff($relevantCorps, $this->scopedCorpIds($entry)));
                }
                if (!empty($relevantCorps)) {
