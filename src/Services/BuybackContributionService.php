...
class BuybackContributionService
{
    ...
    public function forCharacters(array $characterIds): array
    {
        ...
        foreach ($completed as $r) {
            $pol = $this->effectivePolicyForRow($r, $policies);
    ...
    /**
     * Recent counted buyback contribution for an account, for the classifier.
     * Only completions the per-corp policy actually credits count (personal /
     * uncounted buyback never rescues an at-risk member). has_recent is true
     * only when there is a counted completion inside the window.
     */
    public function recentContribution(array $characterIds, int $days): array
    {
        $characterIds = array_values(array_filter(array_map('intval', $characterIds)));
        if (empty($characterIds) || !$this->isAvailable()) {
            return ['available' => false];
        ...
        $rows = BuybackActivity::whereIn('character_id', $characterIds)
            ->where('stage', BuybackActivity::STAGE_COMPLETED)
            ->where('occurred_at', '>=', $since)
            ->get();

        if ($rows->isEmpty()) {
            return ['available' => true, 'has_recent' => false];
        }
        ...
        foreach ($rows as $r) {
            ...
            $weighted += (float) $r->total_value * $pol['weight'];
            $counted++;
        }

        return [
            'available'       => true,
            'has_recent'      => $counted > 0 && $weighted > 0,
    ...
    public function detectedProgrammes(): array
    {
        ...
        foreach ($corpIds as $corpId) {
            ...
            // Era split: completions frozen at a past policy vs still valued at
            // the current one. Lets the settings UI show "N frozen, M at current".
            $frozenCompleted = BuybackActivity::where('corporation_id', $corpId)
                ->where('stage', BuybackActivity::STAGE_COMPLETED)
                ->whereNotNull('frozen_at')->count();
            ...
            $out[] = [
                ...
                'frozen_count'          => $frozenCompleted,
                'live_count'            => max(0, $completed - $frozenCompleted),
                'policy'                => $policy,
    ...
    /**
     * The valuation to apply to ONE activity row. A frozen row (frozen_at set)
     * keeps the tier / weight / attribution snapshotted when its era ended — so a
     * later policy change never re-values it. An un-frozen row (legacy, or the
     * current era) falls back to the corp's current policy, exactly as before.
     *
     * @param  array<int,array>  $cache  per-corp current-policy cache (unfrozen path)
     */
    private function effectivePolicyForRow($row, array &$cache): array
    {
        if ($row->frozen_at !== null) {
            return [
                'counted'                   => (bool) $row->frozen_counted,
                'tier'                      => $row->frozen_tier ?: BuybackPolicy::TIER_COMMUNITY,
                'weight'                    => (float) $row->frozen_weight,
                'attributed_corporation_id' => (int) ($row->frozen_attributed_corporation_id ?: $row->corporation_id),
            ];
        }
        return $this->cachedPolicy((int) $row->corporation_id, $cache);
    }

    /**
     * Freeze a corp's activity rows at the corp's CURRENT effective policy —
     * snapshotting tier / weight / counted / attribution onto each row so later
     * policy changes leave them untouched.
     *
     * Call it BEFORE saving a new policy with $onlyUnfrozen=true to freeze the
     * ending era at the OLD policy (past stays put, new policy applies going
     * forward); call it AFTER saving with $onlyUnfrozen=false to re-value EVERY
     * row at the new policy (initial setup / deliberate correction).
     *
     * @return int rows frozen
     */
    public function applyPolicySnapshot(int $corporationId, bool $onlyUnfrozen): int
    {
        if (!$this->isAvailable()) {
            return 0;
        }

        $pol = $this->policyFor($corporationId);

        $query = BuybackActivity::where('corporation_id', $corporationId);
        if ($onlyUnfrozen) {
            $query->whereNull('frozen_at');
        }

        return $query->update([
            'frozen_tier'                      => $pol['tier'],
            'frozen_weight'                    => $pol['weight'],
            'frozen_counted'                   => $pol['counted'],
            'frozen_attributed_corporation_id' => $pol['attributed_corporation_id'],
            'frozen_at'                        => now(),
        ]);
    }
