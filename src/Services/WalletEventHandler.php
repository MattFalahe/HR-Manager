...
class WalletEventHandler
{
     ...
     *
     * Records a 'wallet_stalled' history event and busts the assessment cache.
     * Deliberately does NOT auto-downgrade the classification: a single alt's
     * stall is a weak, per-character signal, and letting it flip the whole
     * account to at_risk drags an active main down whenever a dormant alt
     * stalls. The category is left to the nightly classifier's holistic,
     * account-level pass (freshest contribution across all alts). The stall is
     * still recorded for visibility and alerted (throttled) so directors see it.
     */
    public function handleStalled(array $payload): ?array
    {
        ...
        try {
            ...
            // NO auto-nudge to at_risk: see the method docblock. One alt's stall
            // must not downgrade the account — the nightly classifier weighs the
            // account holistically. We still record + alert for visibility.
     ...
     * before they fully stall. Records a 'wallet_contribution_drop' history
     * event; like handleStalled it does NOT auto-downgrade the account (a
     * per-alt signal shouldn't drag the main) — the nightly classifier decides
     * the category holistically.
     *
    ...
    public function handleContributionDropDetected(array $payload): ?array
    {
        ...
        try {
            ...
            $this->bustAssessmentCache($characterId, $corporationId);
            // Per-alt signal — no auto-downgrade (see handleStalled). The
            // nightly classifier weighs the account holistically.
            $assessmentUpdated = false;
