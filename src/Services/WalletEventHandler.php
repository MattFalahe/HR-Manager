...
class WalletEventHandler
{
    ...
    public function handleStalled(array $payload): ?array
    {
        ...
        try {
            ...
            // Fire webhook notification (gated by per-webhook
            // notify_wallet_stalled toggle inside NotificationService).
        ...
        try {
            $historyEvent = app(HistoryEventService::class)->record(
                ...
                [
                    ...
                    'occurred_at'     => $payload['detected_at'] ?? now(),
                    'source_plugin'   => 'corp-wallet-manager',
                    'idempotency_key' => $this->idempotencyKey('wallet_milestone', $payload),
                ]
            );

            // Publish hr.player.milestone_reached for downstream subscribers
            // (SeAT Broadcast can render "1B contributor club" pings etc.).
            // Only fires when the history insert succeeded (i.e. wasn't an
            // idempotent replay) so duplicate publishes don't fire.
            if ($historyEvent !== null) {
                $this->publishMilestoneEvent($characterId, $corporationId, $payload);
            }

            // Fire webhook notification (gated by per-webhook
            // notify_wallet_milestone toggle).
            $this->dispatchNotification(fn() => app(NotificationService::class)
