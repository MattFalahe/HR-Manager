...
Route::group([
    'namespace'  => 'HrManager\Http\Controllers',
    'prefix'     => 'hr-manager',
    'middleware' => ['web', 'auth', 'locale', 'hr-audit'],
], function () {
    ...
    Route::group(['prefix' => 'applications', 'middleware' => 'can:hr-manager.recruiter'], function () {
        ...
        Route::get('/{id}/pvp',   ['as' => 'hr-manager.applications.pvp',   'uses' => 'ApplicationController@pvpBreakdown']);
        Route::post('/{id}/status', ['as' => 'hr-manager.applications.status', 'uses' => 'ApplicationController@updateStatus']);
        // Undo an accidental status change (director-only, checked in-controller).
        Route::post('/{id}/revert-status', ['as' => 'hr-manager.applications.revert-status', 'uses' => 'ApplicationController@revertStatus']);
    ...
    Route::group(['prefix' => 'watchlist', 'middleware' => 'can:hr-manager.recruiter'], function () {
        ...
        Route::get('/character/{id}',   ['as' => 'hr-manager.watchlist.dossier', 'uses' => 'WatchlistController@dossier']);
    ...
    Route::group(['middleware' => 'can:hr-manager.director'], function () {
        ...
        // Acknowledge a "joined without application" flag from the Membership tab.
        Route::post('/corp-health/membership/{id}/review', [
            'as' => 'hr-manager.corp-health.membership-review', 'uses' => 'CorpHealthController@reviewMembershipEvent',
        ]);
        ...
        // Activity / audit log — director-only. index renders the filtered
        // trail; export streams the current filter as CSV. Both are excluded
        // from view-logging (AuditService::VIEW_SKIP) so reading the log
        // doesn't spam the log.
        Route::get('/audit',        ['as' => 'hr-manager.audit.index',  'uses' => 'AuditController@index']);
        Route::get('/audit/export', ['as' => 'hr-manager.audit.export', 'uses' => 'AuditController@export']);
    });
