...
class MemberController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Opt-in EveWho back-fill (Settings → Features, off by default). Only
        // when SeAT has NO authoritative roster (we fell through to the sparse
        // character_affiliations): read the corp's public EveWho member list as
        // just another roster source so the page isn't near-empty.
        //
        // The page only SEEDS on the very first view (short budget, so a big
        // corp may show just the first page at first); the daily
        // hr-manager:sync-external-rosters cron then does the full multi-page
        // pull + keeps it fresh, so no later view ever blocks on EveWho.
        $eveWho = app(\HrManager\Services\EveWhoRosterService::class);
        $eveWhoActive = false;
        if ($rosterSource === 'character_affiliations' && $eveWho->isEnabled()) {
            if ($eveWho->count($corporationId) === 0) {
                $eveWho->sync($corporationId);
            }
            if ($eveWho->count($corporationId) > 0) {
                $rosterSource = 'hr_manager_external_roster';
                $eveWhoActive = true;
            }
        }
        ...
        $query = DB::table($rosterSource . ' as cm')
            ...
            ->select([
                ...
                'ca.alliance_id',
                // SeAT's own affiliation for this char (any corp). Lets the
                // EveWho badge suppress rows SeAT already knew were in THIS corp
                // before the EveWho fill.
                'ca.corporation_id as seat_corp_id',
            ]);
        ...
        // EveWho-sourced characters may not be in character_infos yet; use the
        // aggregator's own name as a display fallback so they don't all read as
        // "Character #id" before ESI name-resolution catches up.
        if ($eveWhoActive) {
            $ext = $eveWho->storedRoster($corporationId);
            foreach ($charIds as $cid) {
                if (empty($universeNames[$cid]) && !empty($ext[$cid])) {
                    $universeNames[$cid] = $ext[$cid];
                }
            }
        }
        ...
        $resolver = app(NameResolutionService::class);
        $members->getCollection()->transform(function ($row) use ($universeNames, $registeredSet, $corpName, $allianceNames, $corpAllianceId, $resolver, $tokenHealth, $tokenRequired, $tokenMap, $eveWhoActive, $corporationId) {
            ...
            // Provenance flag: badge a row "EveWho" ONLY when this row exists
            // solely because EveWho listed it — i.e. an EveWho-backed list, not
            // registered, and SeAT had no affiliation putting the character in
            // THIS corp already. A char SeAT already knew (present before the
            // fill) is not "from EveWho" and gets no badge.
            $seatKnewHere = ((int) ($row->seat_corp_id ?? 0)) === (int) $corporationId;
            $row->from_evewho = $eveWhoActive && !$row->is_registered && !$seatKnewHere;
            // A member profile only resolves when SeAT actually knows the
            // character (registered, or in character_affiliations for ANY corp —
            // members.show falls back through those). An EveWho-only row has no
            // SeAT record, so its View button would 404; the view hides it.
            // Non-EveWho rows always come from a SeAT source, so they resolve.
            $seatKnowsChar = $row->is_registered || ((int) ($row->seat_corp_id ?? 0)) > 0;
            $row->has_seat_page = !$eveWhoActive || $seatKnowsChar;
        ...
        $rosterStatus = [
            ...
            'evewho_active'           => $eveWhoActive,
            'evewho_count'            => $eveWhoActive ? $eveWho->count($corporationId) : 0,
            'evewho_fetched_at'       => $eveWhoActive ? optional($eveWho->lastFetchedAt($corporationId))->diffForHumans() : null,
        ];
    ...
    public function show(int $characterId)
    {
        ...
        // Mining engagement — ore output + moon mining + op attendance (only when
        // the corp runs ops), for this character. Replaces the old attendance-only
        // headline, which read as a red 0% for corps that don't run scheduled ops.
        $miningEngagement = $corporationId
            ? $cross->getMiningEngagement([$characterId], $corporationId, (int) config('hr-manager.performance.activity_window_months', 6))
            : ['available' => false, 'has_data' => false];

        // Last in-game login for this character (from SeAT member tracking).
        $lastLogin = Schema::hasTable('corporation_member_trackings')
            ? DB::table('corporation_member_trackings')->where('character_id', $characterId)->max('logon_date')
            : null;
        ...
        // Framework installed (vs this character linked): lets the profile show
        // a muted "not on Discord" state instead of silently rendering nothing.
        $connectorInstalled = app(SeatConnectorService::class)->isAvailable();
        ...
        // Activity breakdown — the raw numbers behind those badges, for THIS
        // character only (no userId, so no account-level FC count).
        $characterActivity = app(\HrManager\Services\PlayerActivityService::class)
            ->forCharacters([$characterId], $corporationId);
