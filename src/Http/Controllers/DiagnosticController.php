...
class DiagnosticController extends Controller
{
    ...
    /** EventBus subscriptions HR owns. Mirror HrManagerServiceProvider::subscribe. */
    private const OWNED_SUBSCRIPTIONS = [
        ...
        // Blueprint / Structure / Buyback (added after the initial list was written)
        'blueprint.request.*', 'structure.alert.*',
        'buyback.offer.published', 'buyback.contract.completed',
    ];
    ...
    public function index(Request $request)
    {
        ...
        $master     = $activeTab === 'master'     ? $this->masterTest()       : null;
        $ecosystem  = $activeTab === 'ecosystem'  ? $this->dataExchangeMap()  : null;
    ...
    private function dataIntegrity(): array
    {
        ...
        // Activity log (opt-in). Show enabled/disabled + volume, and the oldest
        // entry so retention pruning (180d) is visibly working.
        if (Schema::hasTable('hr_manager_audit_log')) {
            $auditOn = (bool) \HrManager\Models\Setting::getValue('enable_audit_log', config('hr-manager.features.enable_audit_log', false));
            $auditCount = DB::table('hr_manager_audit_log')->count();
            $auditOldest = DB::table('hr_manager_audit_log')->min('occurred_at');
            $out[] = $this->result(
                'Activity log',
                'ok',
                $auditOn
                    ? ($auditCount . ' entries' . ($auditOldest ? ', oldest ' . \Illuminate\Support\Carbon::parse($auditOldest)->diffForHumans() : ''))
                    : 'disabled (opt-in — Settings → Features)'
            );
        }
    ...
    private function traceApplication(int $applicationId): array
    {
        ...
        $revertedCount = $history->filter(fn ($h) => !empty($h->voided_at ?? null))->count();
        $steps[] = ['label' => 'Status transitions', 'status' => 'ok',
            'detail' => $history->count() . ' recorded'
                . ($revertedCount > 0 ? ' (' . $revertedCount . ' reverted as a mistake)' : '')];
        ...
        $actorIds = array_values(array_unique(array_filter(array_map('intval', array_merge(
            ...
            // Directors who reverted a transition, so the trace can name them too.
            $history->pluck('voided_by')->all(),
        ...
        foreach ($history as $h) {
            ...
            $entry = array_merge($fmt($h->created_at), [
            ...
            // A transition a director undid as a mistake: still recorded here (the
            // trace shows the truth) but flagged, and hidden from the applicant's
            // tracking page. `?? null` keeps this safe before migration 000038.
            $voidedAt = $h->voided_at ?? null;
            if ($voidedAt) {
                $v = $actor($h->voided_by ?? 0);
                $entry['voided']       = true;
                $entry['void_by']      = $v['name'];
                $entry['void_admin']   = $v['admin'];
                $entry['void_at']      = \Illuminate\Support\Carbon::parse($voidedAt)->format('Y-m-d H:i');
                $entry['void_reason']  = (string) ($h->void_reason ?? '');
            }
    ...
    /**
     * Master Test — runs every check group at once for a single combined verdict
     * (the "is everything healthy?" pass other suite plugins ship). Heavier than
     * the per-tab views since it computes all of them in one request.
     */
    private function masterTest(): array
    {
        $v = $this->systemValidation();
        $groups = [
            'Health Checks'     => $this->healthChecks(),
            'System Validation' => array_merge($v['plugins'], $v['capabilities'], $v['direct'], $v['subscriptions'], $v['topics']),
            'Settings Health'   => $this->settingsHealth(),
            'Data Integrity'    => $this->dataIntegrity(),
        ];
        ...
        return ['groups' => $groups, 'summary' => $this->summarise($all)];
    }

    /**
     * Ecosystem data-exchange map: for each suite plugin, what HR RECEIVES from
     * it, what HR SENDS to it, and which HR surfaces the exchange feeds —
     * annotated with live installed status. A read-only architecture view, not a
     * health check.
     */
    private function dataExchangeMap(): array
    {
        $detected = $this->detectSuitePlugins();

        $map = [
            'manager-core' => [
                'receives' => ['The hub itself: EventBus + PluginBridge route every cross-plugin exchange below', 'Pricing, SDE lookups, ESI fast-poll notifications, the capability registry'],
                'sends'    => ['hr.application.* (submitted / accepted / rejected / withdrawn / status_changed / joined_corp)', 'hr.player.* (flagged / recovered / milestone ladder), hr.purge.* (reminders + executed)'],
                'surfaces' => ['Everything cross-plugin — without MC, all rows below drop to local-only'],
            ],
            'corp-wallet-manager' => [
                'receives' => ['EventBus: member.contribution.{stalled,milestone,drop_detected}, member.tax.compliance_dropped, wallet.unusual_recipient_detected', 'Bridge: wallet.getCorpSummary + per-character lifetime / net-position / tax-compliance'],
                'sends'    => [],
                'surfaces' => ['Classifier wallet-signal layer, member Wallet Activity + Audit panels, 5 Corp Health wallet-insight cards, the Economy Financial Pulse'],
            ],
            'mining-manager' => [
                'receives' => ['EventBus: mining.* (tax + activity)', 'Bridge: mining history / tax / ore breakdown'],
                'sends'    => [],
                'surfaces' => ['Member assessment mining columns: favourite ores + systems, corp ore-op attendance'],
            ],
            'blueprint-manager' => [
                'receives' => ['EventBus: blueprint.request.* (created / approved / rejected / fulfilled)', 'Bridge: blueprint.getCharacterStats / getCorpSummary'],
                'sends'    => [],
                'surfaces' => ['Player Blueprint Activity panel, Corp Health Blueprint Engagement card, Industrialist role badge, positive classifier modifier'],
            ],
            'structure-manager' => [
                'receives' => ['Bridge: compliance.getForCorporation'],
                'sends'    => [],
                'surfaces' => ['Corp Health Structure Compliance tab — SM owns the doctrines, HR renders the verdict + Copy / Appraise'],
            ],
            'buyback-manager' => [
                'receives' => ['EventBus: buyback.offer.published + buyback.contract.completed'],
                'sends'    => ['Doctrine-fit hand-off to the buyback appraiser (Structure Compliance tab)'],
                'surfaces' => ['Player Buyback panel, Corp Health Economy buyback card, per-corp contribution policy, the buyback_hold classifier modifier'],
            ],
            'seat-discord-pings' => [
                'receives' => ['EventBus: pings.broadcast.sent + pings.formup.scheduled'],
                'sends'    => ['hr.* events SeAT Broadcast can consume'],
                'surfaces' => ['Player FC Activity panel, Corp Health fleet-commander roster + Organizers, FC role badge'],
            ],
        ];

        $flows = [];
        foreach ($map as $slug => $flow) {
            $plugin = $detected[$slug] ?? ['label' => $slug, 'installed' => false, 'detail' => 'unknown'];
            $flows[] = [
                'label'     => $plugin['label'],
                'installed' => (bool) $plugin['installed'],
                'detail'    => $plugin['detail'],
                'receives'  => $flow['receives'],
                'sends'     => $flow['sends'],
        ...
        return [
            'flows'     => $flows,
            // Fast-poll join/leave detection rides MC's ESI notification registry.
            // Single source of truth shared with the Settings webhook badge.
            'fast_poll' => \HrManager\Services\MembershipNotificationHandler::fastPollAvailable(),
        ];
    }
