...
- **"View in SeAT" deep-links** on the per-character Members profile: one-click buttons (Sheet / Wallet / Mail / Assets / Skills) that jump straight into SeAT's own character pages in a new tab. Unlike the recruiter-access grant on applications, no temporary role is attached — the Members profile is already director-tier and SeAT's own permissions govern each page on click.
